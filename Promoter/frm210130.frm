...
Begin VB.Form frm210130 
   ...
   End
End
Attribute VB_Name = "frm210130"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'Memo by Amy 2021/08/02 Form2.0已修改 Text2/Label10/GrdDataList
...
'Modify by Amy 2023/09/05 +IsRelation
...
grdDataList.ColWidth(2) = 3800
grdDataList.CellAlignment = flexAlignCenterCenter
grdDataList.col = 3: grdDataList.Text = "國籍"
grdDataList.ColWidth(3) = 1200
grdDataList.CellAlignment = flexAlignCenterCenter
grdDataList.col = 4: grdDataList.Text = "智權人員" '"智權人員/開發人員"
...
'Modify by Amy 2023/09/05 避免沒改到,從strMenu1搬過來
'關聯企業
...
Public Sub PubShowNextData()
   ...
   , , m_bolPrintRight, , strRepCon)
   If cmdState = 5 Then cmdOK(cmdState).Enabled = True
End Sub

'Mark by Amy 2023/09/05 改抓共用(Memo by Amy 發現與其他支不同,改與國外潛在客戶同[4支查詢都一致]-秀玲)
...
''            frm100101_20.Show
''            frm100101_20.CRdateF = Text6
''            frm100101_20.CRdateT = IIf(Len(Text7) <= 0, "", IIf(Len(Text7) <= 0, (ServerDate - 19110000), Text7))
...
'            If Left(Trim(strTmp), 1) = "平" Then '國外
...
Private Sub cmdSearch_Click()
    Dim strCheckWay As String, strFields As String
    Dim stSQL1 As String, stSQL2 As String, stSQL3 As String, stSQL4 As String, stSQL5 As String
    Dim strNo As String, Str01 As String, s As Integer
    Dim IsDevelop As Boolean, bCancel As Boolean
...
On Error GoTo ErrHnd
    ...
    If Option2(4).Value = True Then
        If Len(Trim(Text4)) = 0 And Len(Trim(Text5)) Then
            s = MsgBox("條件不可空白", , "輸入條件錯誤")
            Text4.SetFocus
        End If
    End If
   ...
   End If
    ...
    ElseIf Option2(2).Value = True Then
        ...
        strSql = strSql & " Union All Select ' ' AS V,PCC01||'0-'||PCC02 AS 編號,Nvl(PCC05,Nvl(PCC03,PCC04)) AS 名稱,NA03 AS 國籍,' ' AS 智權人員,' ' AS 狀態,PCC13 AS 備註,' ' AS 申請國家,'' AS 總收文號,'' AS 案件性質,'' AS 收文日" & strFields & " From PotCustCont,PotCustomer,Nation Where (InStr(NLS_Upper(PCC08),'" & UCase(ChgSQL(Trim(Text9))) & "') > 0 ) And pcc01=PCU01 And PCU09=NA01(+) "
        strSql = strSql & " Union All Select ' ' AS V,PCC01||'0-'||PCC02 AS 編號,Nvl(PCC05,Nvl(PCC03,PCC04)) AS 名稱,NA03 AS 國籍,' ' AS 智權人員,' ' AS 狀態,PCC13 AS 備註,' ' AS 申請國家,'' AS 總收文號,'' AS 案件性質,'' AS 收文日" & strFields & " From PotCustCont,PotCustomer1,Nation Where (InStr(NLS_Upper(PCC08),'" & UCase(ChgSQL(Trim(Text9))) & "') > 0 ) And pcc01=POC01 And POC04=NA01(+) "
    ...
    ElseIf Option2(3).Value = True Then
        ...
        strSql = strSql & " Union All Select ' ' AS V,PCC01||'0-'||PCC02 AS 編號,Nvl(PCC05,Nvl(PCC03,PCC04)) AS 名稱,' ' AS 國籍,' ' AS 智權人員,' ' AS 狀態,PCC13 AS 備註,' ' AS 申請國家,'' AS 總收文號,'' AS 案件性質,'' AS 收文日" & strFields & " From PotCustCont Where InStr(PCC12,'" & ChgSQL(Text3) & "') > 0  And SubStr(PCC01,1,1)='R' "
        ...
        strSql = strSql & " Union All Select ' ' AS V,FA01||FA02||Decode(FA02,'0','','＊')||Decode(FA77,'Y','$','') AS 編號,Nvl(FA04,Decode(FA05,null,FA06,FA05||' '||FA63||' '||FA64||' '||FA65)) AS 名稱,NA03 AS 國籍,' ' AS 智權人員,Decode(FA103,null,FA69,GetDizhang(FA103,'Y')) AS 狀態, FA29 AS 備註,' ' AS 申請國家,'' AS 總收文號,'' AS 案件性質,'' AS 收文日" & strFields & " From Fagent,Nation,PotCustomer Where (InStr(FA94,'" & ChgSQL(Text3) & "')> 0) And FA10=NA01(+) "
    ...
    ElseIf Option2(4).Value = True Then
        ...
        strSql = strSql & " Union All Select ' ' AS V,PCC01||'0-'||PCC02 AS 編號,Nvl(PCC05,Nvl(PCC03,PCC04)) AS 名稱,NA03 AS 國籍,' ' AS 智權人員,' ' AS 狀態,PCC13 AS 備註,' ' AS 申請國家,'' AS 總收文號,'' AS 案件性質,'' AS 收文日" & strFields & " From PotCustCont,PotCustomer,Nation Where PCC11 >='" & ChangeTStringToWString(Text4) & "' And  PCC11 <='" & ChangeTStringToWString(Text5) & "'  And PCC01=PCU01 And PCU09=NA01(+)"
        strSql = strSql & " Union All Select ' ' AS V,PCC01||'0-'||PCC02 AS 編號,Nvl(PCC05,Nvl(PCC03,PCC04)) AS 名稱,NA03 AS 國籍,' ' AS 智權人員,' ' AS 狀態,PCC13 AS 備註,' ' AS 申請國家,'' AS 總收文號,'' AS 案件性質,'' AS 收文日" & strFields & " From PotCustCont,PotCustomer1,Nation Where PCC11 >='" & ChangeTStringToWString(Text4) & "' And  PCC11 <='" & ChangeTStringToWString(Text5) & "'  And PCC01=POC01 And POC04=NA01(+)"
    End If
...
'                'Modify by Amy 2023/09/05 變色改為共用函數
...
'                End If
    ...
    'Modify by Amy 2023/09/05 原程式寫至共用SetGridOneData
...
'If Option2(0).Value = True Then
'   pub_QL05 = pub_QL05 & ";" & Option2(0).Caption & Trim(Text1) 'Add By Sindy 2010/12/23
...
'      'Add By Sindy 2012/3/21
'      'Modified by Lydia 2017/02/14 + strfields 'Modfiy by Amy 2019/08/27 補程式未加strFields
'      strSql = strSql & " union all select ' ' as V,NT01||Decode(NT21,null,'⊕','') AS 編號,NVL(NT02,DECODE(NT03,null,NT07,NT03||' '||NT04||' '||NT05||' '||NT06)) as 名稱,NA03 AS 國籍,ST02 AS 智權人員,Decode(NT21,null,'不得代理','') AS 狀態, Decode(NT21,null,'','撤銷日期：'||sqldatet(NT21)||'；')||NT20 AS 備註,' ' as 申請國家,'' as 總收文號,'' as 案件性質,'' as 收文日" & strFields & "  from notagent,nation,STAFF where nt08=na01(+) and nt01='" & IIf(Len(Trim(Text1)) >= 3, Trim(Text1), Right("000" & Trim(Text1), 3)) & "' AND nt18=ST01(+)"
'   End If
'
''潛在客戶名稱
'ElseIf Option2(1).Value = True Then
'   pub_QL05 = pub_QL05 & ";" & Option2(1).Caption & Trim(Text2) 'Add By Sindy 2010/12/23
'   '以編號或名稱
'   '模糊比對
'   If Option3(0).Value = False Then
'      pub_QL05 = pub_QL05 & ";" & Option3(1).Caption 'Add By Sindy 2010/12/23
'      strCheckWay = ">0"
'   '字首比對
'   Else
'      pub_QL05 = pub_QL05 & ";" & Option3(0).Caption  'Add By Sindy 2010/12/23
'      strCheckWay = "=1"
'   End If
'   'Add by Amy 2013/12/04
...
'    'end 2013/12/04
'
''Modify by Amy 2013/12/04 拿掉中英日 +查對造 +申請國家,總收文號,案件性質,收文日 欄位
''   '以中文名稱查詢
''   If Option1(0).Value = True Then
''      pub_QL05 = pub_QL05 & ";" & "以" & Option1(0).Caption & "查詢" 'Add By Sindy 2010/12/23
''      strSql = "SELECT ' ' as V,CU01||CU02||Decode(CU02,'0','','＊')||decode(cu111,'Y','$','')||decode(cu121,'Y','●','') AS 編號,CU04 AS 名稱,NA03 AS 國籍,ST02 AS 智權人員,Decode(CU142,null,CU80,Decode(CU142,'A','同意抵帳中',Decode(CU142,'B','宣告破產','帳款處理中'))) AS 狀態,CU79 AS 備註 FROM CUSTOMER,NATION,STAFF, (Select Distinct CU01 As A1 From Customer Where instr(CU04,'" & ChgSQL(Trim(Text2)) & "')" & strCheckWay & " ) A WHERE CU10=NA01(+) AND CU01=A.A1 AND CU13=ST01(+) "
''      strSql = strSql & " union all select ' ' as V,FA01||FA02||Decode(FA02,'0','','＊')||decode(fa77,'Y','$','') AS 編號,fa04 as 名稱,NA03 AS 國籍,' ' AS 智權人員,Decode(FA103,null,FA69,Decode(FA103,'A','同意抵帳中',Decode(FA103,'B','宣告破產','帳款處理中'))) AS 狀態, FA29 AS 備註 from fagent,nation, (Select Distinct FA01 As A1 From Fagent Where instr(fa04,'" & ChgSQL(Trim(Text2)) & "')" & strCheckWay & " ) A where fa10=na01(+) AND FA01=A.A1 " & StrSQLa
''      '查潛在客戶
''      strSql = strSql & " union all SELECT ' ' AS V,PCU01||PCU02||Decode(PCU02,'0','','＊') AS 編號,PCU08 AS 名稱,NA03 AS 國籍,ST02 AS 智權人員,PCU39 AS 狀態,PCU40 AS 備註 from potcustomer,nation,staff, (Select Distinct pcu01 As A1 From potcustomer Where instr(pcu08,'" & ChgSQL(Trim(Text2)) & "')" & strCheckWay & ") A where pcu09=na01(+) and pcu01=A.A1 and substr(LTrim(PCU38),1,5)=ST01(+) "
''      '查國內潛在客戶
''      strSql = strSql & " union all SELECT ' ' AS V,POC01||POC02||Decode(POC02,'0','','＊') AS 編號,POC03 AS 名稱,NA03 AS 國籍,ST02 AS 智權人員,POC14 AS 狀態,POC15 AS 備註 from potcustomer1,nation,staff, (Select Distinct poc01 As A1 From potcustomer1 Where instr(poc03,'" & ChgSQL(Trim(Text2)) & "')" & strCheckWay & ") A where poc04=na01(+) and poc01=A.A1 AND poc13=ST01(+) "
''      '查聯絡人
''      strSql = strSql & " union all select ' ' as V,PCC01||'0-'||PCC02 AS 編號,PCC05 AS 名稱,NA03 AS 國籍,ST02 AS 智權人員,Decode(CU142,null,CU80,Decode(CU142,'A','同意抵帳中',Decode(CU142,'B','宣告破產','帳款處理中'))) AS 狀態,PCC13 AS 備註 from (Select * From potcustcont Where instr(pcc05,'" & ChgSQL(Trim(Text2)) & "')" & strCheckWay & ") A,CUSTOMER,NATION,STAFF WHERE CU10=NA01(+) AND  CU01=PCC01 AND CU02='0' AND CU13=ST01(+) "
''      strSql = strSql & " union all select ' ' as V,PCC01||'0-'||PCC02 AS 編號,PCC05 AS 名稱,NA03 AS 國籍,ST02 AS 智權人員,PCU39 AS 狀態,PCC13 AS 備註 from (Select * From potcustcont Where instr(pcc05,'" & ChgSQL(Trim(Text2)) & "')" & strCheckWay & ") A,potcustomer,nation,staff where pcu09=na01(+)  AND PCU01=PCC01 AND PCU02='0' and substr(LTrim(PCU38),1,5)=ST01(+) "
''      strSql = strSql & " union all select ' ' as V,PCC01||'0-'||PCC02 AS 編號,PCC05 AS 名稱,NA03 AS 國籍,' ' AS 智權人員,Decode(FA103,null,FA69,Decode(FA103,'A','同意抵帳中',Decode(FA103,'B','宣告破產','帳款處理中'))) AS 狀態, PCC13 AS 備註 from (Select * From potcustcont Where instr(pcc05,'" & ChgSQL(Trim(Text2)) & "')" & strCheckWay & ") A,fagent,nation where fa10=na01(+) AND FA01=PCC01 AND FA02='0' " & StrSQLa
''      strSql = strSql & " union all select ' ' as V,PCC01||'0-'||PCC02 AS 編號,PCC05 AS 名稱,NA03 AS 國籍,ST02 AS 智權人員,POC14 AS 狀態,PCC13 AS 備註 from (Select * From potcustcont Where instr(pcc05,'" & ChgSQL(Trim(Text2)) & "')" & strCheckWay & ") A,potcustomer1,nation,staff where poc04=na01(+)  AND POC01=PCC01 AND POC02='0' and poc13=ST01(+) "
''      'Add By Sindy 2012/3/21
...
''      pub_QL05 = pub_QL05 & ";" & "以" & Option1(1).Caption & "查詢" 'Add By Sindy 2010/12/23
''      strSql = "SELECT ' ' as V,CU01||CU02||Decode(CU02,'0','','＊')||decode(cu111,'Y','$','')||decode(cu121,'Y','●','') AS 編號,cu05||' '||cu88||' '||cu89||' '||cu90 AS 名稱,NA03 AS 國籍,ST02 AS 智權人員,Decode(CU142,null,CU80,Decode(CU142,'A','同意抵帳中',Decode(CU142,'B','宣告破產','帳款處理中'))) AS 狀態,CU79 AS 備註 FROM CUSTOMER,NATION,STAFF, (Select Distinct CU01 As A1 From Customer Where instr(upper(cu05||' '||cu88||' '||cu89||' '||cu90),'" & UCase(ChgSQL(Trim(Text2))) & "')" & strCheckWay & " ) A WHERE CU10=NA01(+) AND CU01=A.A1 AND CU13=ST01(+) "
''      strSql = strSql & " union all select ' ' as V,FA01||FA02||Decode(FA02,'0','','＊')||decode(fa77,'Y','$','') AS 編號,FA05||' '||FA63||' '||FA64||' '||FA65 as 名稱,NA03 AS 國籍,' ' AS 智權人員,Decode(FA103,null,FA69,Decode(FA103,'A','同意抵帳中',Decode(FA103,'B','宣告破產','帳款處理中'))) AS 狀態, FA29 AS 備註 from fagent,nation, (Select Distinct FA01 As A1 From Fagent Where instr(upper(fa05||' '||fa63||' '||fa64||' '||fa65),'" & UCase(ChgSQL(Trim(Text2))) & "')" & strCheckWay & " ) A where fa10=na01(+) AND FA01=A.A1 " & StrSQLa
''      '查潛在客戶
...
''      '查國內潛在客戶
''      strSql = strSql & " union all SELECT ' ' AS V,POC01||POC02||Decode(POC02,'0','','＊') AS 編號,rtrim(poc23||' '||poc24||' '||poc25||' '||poc26) AS 名稱,NA03 AS 國籍,ST02 AS 智權人員,POC14 AS 狀態,POC15 AS 備註 from potcustomer1,nation,staff, (Select Distinct poc01 As A1 From potcustomer1 Where instr(upper(poc23||' '||poc24||' '||poc25||' '||poc26),'" & UCase(ChgSQL(Trim(Text2))) & "')" & strCheckWay & ") A where poc04=na01(+) and poc01=A.A1 AND poc13=ST01(+) "
''      '查聯絡人
''      strSql = strSql & " union all select ' ' as V,PCC01||'0-'||PCC02 AS 編號,PCC03 AS 名稱,NA03 AS 國籍,ST02 AS 智權人員,Decode(CU142,null,CU80,Decode(CU142,'A','同意抵帳中',Decode(CU142,'B','宣告破產','帳款處理中'))) AS 狀態,PCC13 AS 備註 from (Select * From potcustcont Where instr(upper(pcc03),'" & UCase(ChgSQL(Trim(Text2))) & "')" & strCheckWay & ") A,CUSTOMER,NATION,STAFF WHERE CU10=NA01(+) AND  CU01=PCC01 AND CU02='0' AND CU13=ST01(+) "
''      strSql = strSql & " union all select ' ' as V,PCC01||'0-'||PCC02 AS 編號,PCC03 AS 名稱,NA03 AS 國籍,ST02 AS 智權人員,PCU39 AS 狀態,PCC13 AS 備註 from (Select * From potcustcont Where instr(upper(pcc03),'" & UCase(ChgSQL(Trim(Text2))) & "')" & strCheckWay & ") A,potcustomer,nation,staff where pcu09=na01(+)  AND PCU01=PCC01 AND PCU02='0' and substr(LTrim(PCU38),1,5)=ST01(+) "
''      strSql = strSql & " union all select ' ' as V,PCC01||'0-'||PCC02 AS 編號,PCC03 AS 名稱,NA03 AS 國籍,' ' AS 智權人員,Decode(FA103,null,FA69,Decode(FA103,'A','同意抵帳中',Decode(FA103,'B','宣告破產','帳款處理中'))) AS 狀態, PCC13 AS 備註 from (Select * From potcustcont Where instr(upper(pcc03),'" & UCase(ChgSQL(Trim(Text2))) & "')" & strCheckWay & ") A,fagent,nation where fa10=na01(+) AND FA01=PCC01 AND FA02='0' " & StrSQLa
''      strSql = strSql & " union all select ' ' as V,PCC01||'0-'||PCC02 AS 編號,PCC03 AS 名稱,NA03 AS 國籍,ST02 AS 智權人員,POC14 AS 狀態,PCC13 AS 備註 from (Select * From potcustcont Where instr(upper(pcc03),'" & UCase(ChgSQL(Trim(Text2))) & "')" & strCheckWay & ") A,potcustomer1,nation,staff where poc04=na01(+)  AND POC01=PCC01 AND POC02='0' and poc13=ST01(+) "
''      'Add By Sindy 2012/3/21
...
''   '以日文名稱查詢
''   Else
''      pub_QL05 = pub_QL05 & ";" & "以" & Option1(2).Caption & "查詢" 'Add By Sindy 2010/12/23
''      strSql = "SELECT ' ' as V,CU01||CU02||Decode(CU02,'0','','＊')||decode(cu111,'Y','$','')||decode(cu121,'Y','●','') AS 編號,cu06 AS 名稱,NA03 AS 國籍,ST02 AS 智權人員,Decode(CU142,null,CU80,Decode(CU142,'A','同意抵帳中',Decode(CU142,'B','宣告破產','帳款處理中'))) AS 狀態,CU79 AS 備註 FROM CUSTOMER,NATION,STAFF, (Select Distinct CU01 As A1 From Customer Where instr(CU06,'" & ChgSQL(Trim(Text2)) & "')" & strCheckWay & " ) A WHERE CU10=NA01(+) AND CU01=A.A1 AND CU13=ST01(+) "
''      strSql = strSql & " union all select ' ' as V,FA01||FA02||Decode(FA02,'0','','＊')||decode(fa77,'Y','$','') AS 編號,FA06 as 名稱,NA03 AS 國籍,' ' AS 智權人員,Decode(FA103,null,FA69,Decode(FA103,'A','同意抵帳中',Decode(FA103,'B','宣告破產','帳款處理中'))) AS 狀態, FA29 AS 備註 from fagent,nation, (Select Distinct FA01 As A1 From Fagent Where instr(fa06,'" & ChgSQL(Trim(Text2)) & "')" & strCheckWay & " ) A where fa10=na01(+) AND FA01=A.A1 " & StrSQLa
''      '查潛在客戶
''      strSql = strSql & " union all SELECT ' ' AS V,PCU01||PCU02||Decode(PCU02,'0','','＊') AS 編號,pcu07 AS 名稱,NA03 AS 國籍,ST02 AS 智權人員,PCU39 AS 狀態,PCU40 AS 備註 from potcustomer,nation,staff, (Select Distinct pcu01 As A1 From potcustomer Where instr(pCU07,'" & ChgSQL(Trim(Text2)) & "')" & strCheckWay & ") A where pcu09=na01(+) and pcu01=A.A1 and substr(LTrim(PCU38),1,5)=ST01(+) "
''      '查國內潛在客戶
''      strSql = strSql & " union all SELECT ' ' AS V,POC01||POC02||Decode(POC02,'0','','＊') AS 編號,poc27 AS 名稱,NA03 AS 國籍,ST02 AS 智權人員,POC14 AS 狀態,POC15 AS 備註 from potcustomer1,nation,staff, (Select Distinct poc01 As A1 From potcustomer1 Where instr(POC27,'" & ChgSQL(Trim(Text2)) & "')" & strCheckWay & ") A where poc04=na01(+) and poc01=A.A1 AND poc13=ST01(+) "
''      '查聯絡人
''      strSql = strSql & " union all select ' ' as V,PCC01||'0-'||PCC02 AS 編號,PCC04 AS 名稱,NA03 AS 國籍,ST02 AS 智權人員,Decode(CU142,null,CU80,Decode(CU142,'A','同意抵帳中',Decode(CU142,'B','宣告破產','帳款處理中'))) AS 狀態,PCC13 AS 備註 from (Select * From potcustcont Where instr(pcc04,'" & ChgSQL(Trim(Text2)) & "')" & strCheckWay & ") A,CUSTOMER,NATION,STAFF WHERE CU10=NA01(+) AND  CU01=PCC01 AND CU02='0' AND CU13=ST01(+) "
''      strSql = strSql & " union all select ' ' as V,PCC01||'0-'||PCC02 AS 編號,PCC04 AS 名稱,NA03 AS 國籍,ST02 AS 智權人員,PCU39 AS 狀態,PCC13 AS 備註 from (Select * From potcustcont Where instr(pcc04,'" & ChgSQL(Trim(Text2)) & "')" & strCheckWay & ") A,potcustomer,nation,staff where pcu09=na01(+)  AND PCU01=PCC01 AND PCU02='0' and substr(LTrim(PCU38),1,5)=ST01(+) "
''      strSql = strSql & " union all select ' ' as V,PCC01||'0-'||PCC02 AS 編號,PCC04 AS 名稱,NA03 AS 國籍,' ' AS 智權人員,Decode(FA103,null,FA69,Decode(FA103,'A','同意抵帳中',Decode(FA103,'B','宣告破產','帳款處理中'))) AS 狀態, PCC13 AS 備註 from (Select * From potcustcont Where instr(pcc04,'" & ChgSQL(Trim(Text2)) & "')" & strCheckWay & ") A,fagent,nation where fa10=na01(+) AND FA01=PCC01 AND FA02='0' " & StrSQLa
''      strSql = strSql & " union all select ' ' as V,PCC01||'0-'||PCC02 AS 編號,PCC04 AS 名稱,NA03 AS 國籍,ST02 AS 智權人員,POC14 AS 狀態,PCC13 AS 備註 from (Select * From potcustcont Where instr(pcc04,'" & ChgSQL(Trim(Text2)) & "')" & strCheckWay & ") A,potcustomer1,nation,staff where poc04=na01(+)  AND POC01=PCC01 AND POC02='0' and poc13=ST01(+) "
''      'Add By Sindy 2012/3/21
...
''      'end 2014/02/25
''
''      'Add by Amy 2014/03/17 所有商標案InStr(R021014,'T')且案件性質為1202(核准通知)者狀態改為 其他相關人
...
'   '開拓客戶資料
'   If Check1.Value = 1 Then
'      'Modify by Amy 2013/09/30 原只顯示ecd03,ecd04 改兩個語法
'      'strSql = strSql & " union all SELECT ' ' AS V,ecd02||'-'||LPAD(ecd01,6,'0') AS 編號,ecd03||' '||ecd04 AS 名稱,NA03 AS 國籍,' ' AS 智權人員,ecd15 AS 狀態,ecd16 AS 備註 from expandcusdetail,expandcusattr,nation,(Select Distinct nvl(ecd01,'')||nvl(ecd02,'') As A1 From expandcusdetail Where instr(ecd03,'" & ChgSQL(Trim(Text2)) & "') " & strCheckWay & " or instr(ecd04,'" & ChgSQL(Trim(Text2)) & "') " & strCheckWay & " or instr(ecd11,'" & ChgSQL(Trim(Text2)) & "') " & strCheckWay & " or instr(ecd12,'" & ChgSQL(Trim(Text2)) & "') " & strCheckWay & ") A Where ecd10=na01(+) and ecd02=eca01(+) and nvl(ecd01,'')||nvl(ecd02,'')=A.A1 "
'      'Modified by Lydia 2017/02/14 + strfields
...
''開發者
'ElseIf Option2(3).Value = True Then
'     pub_QL05 = pub_QL05 & ";" & Option2(3).Caption & Text3 & Label10 'Add By Sindy 2010/12/23
'     'Modify by Amy 2013/12/04 +申請國家/總收文號/案件性質/收文日
'     '查潛在客戶
'     'Modify by Amy 2020/03/16 智權人員 原:st02 ,因開發人員可能多人
'     'Modified by Lydia 2017/02/14 + strfields
...
'
'     'Modified by Lydia 2017/02/14 + strfields
'     strSql = strSql & " union all select ' ' as V,FA01||FA02||Decode(FA02,'0','','＊')||decode(fa77,'Y','$','') AS 編號,NVL(fa04,DECODE(fa05,null,fa06,fa05||' '||fa63||' '||fa64||' '||fa65)) as 名稱,NA03 AS 國籍,' ' AS 智權人員,Decode(FA103,null,FA69,GetDizhang(FA103,'Y')) AS 狀態, FA29 AS 備註,' ' as 申請國家,'' as 總收文號,'' as 案件性質,'' as 收文日" & strFields & " FROM fagent,nation,PotCustomer Where (instr(fa94,'" & ChgSQL(Text3) & "')> 0) and fa10=na01(+) " & StrSQLa
'
''開發日期
'ElseIf Option2(4).Value = True Then
'   pub_QL05 = pub_QL05 & ";" & Option2(4).Caption & Text4 & "-" & Text5 'Add By Sindy 2010/12/23
'   'Modify by Amy 2013/12/04 +申請國家/總收文號/案件性質/收文日
'   'Modified by Lydia 2017/02/14 + strfields
'   strSql = "SELECT ' ' as V,CU01||CU02||Decode(CU02,'0','','＊')||decode(cu111,'Y','$','')||decode(cu121,'Y','●','') AS 編號,NVL(CU04,DECODE(cu05,null,CU06,cu05||' '||cu88||' '||cu89||' '||cu90)) AS 名稱,NA03 AS 國籍,ST02 AS 智權人員,Decode(CU142,null,CU80,GetDizhang(CU142,'Y')) AS 狀態,CU79 AS 備註,' ' as 申請國家,'' as 總收文號,'' as 案件性質,'' as 收文日" & strFields & " FROM CUSTOMER,NATION,STAFF Where cu14 >='" & ChangeTStringToWString(Text4) & "' and cu14 <='" & ChangeTStringToWString(Text5) & "' and CU10=NA01(+) and CU13=ST01(+) "
'
...
'
'If Trim(Text6) <> "" Or Trim(Text7) <> "" Then
'   pub_QL05 = pub_QL05 & ";" & Left(Label5, 5) & Text6 & "-" & Text7 'Add By Sindy 2010/12/23
'End If
'If Check1.Value = 1 Then
'   pub_QL05 = pub_QL05 & ";" & Check1.Caption 'Add By Sindy 2010/12/23
'End If
'
...
Private Sub Form_Activate()
   'Add by Amy 2023/09/05 +if 否則跳至別表單回來會錯
...
Private Sub Form_Load()
   ...
   Label2(0).Caption = Label2(0).Caption & vbCrLf & "　　紫底為風險警示" 'Modify by Amy 2024/01/31 +風險檢查對象,拿掉風險警示啟用日
   ...
   bolToEndByNick = False
   m_bolPrintRight = IsUserHasRightOfFunction("frm210130", strPrint, False)
...
'Modify by Amy 2013/12/10 +申請國家/總收文號/案件性質/收文日
'Added by Lydia 2018/05/24 改由啟用日控制
If strSrvDate(1) < 國外部關聯企業啟用日 Then
    'Modify by Amy 2019/10/05 +4個''->關聯編號/名稱/關係/說明 避免加欄位困難
    'Modified by Lydia 2020/05/07 +'00' as R11401
    strSql = "SELECT '' AS V,R06001 AS 編號,R06002 AS 名稱,R06003 AS 國籍,ST02 AS 智權人員,CU80 AS 狀態,CU79 AS 備註,' ' as 申請國家,'' as 總收文號,'' as 案件性質,'' as 收文日,'' as 關聯編號,'' as 關聯名稱,'' as 關聯關係,'' as 關聯說明,'00' as R11401 FROM R140407,CUSTOMER,STAFF where id='" & strUserNum & "' AND SUBSTR(R06001,1,1)='X' AND  SUBSTR(R06001,1,8)=CU01(+) AND SUBSTR(R06001,9,1)=CU02(+) and CU13=ST01(+) "
     'Modify by Amy 2019/10/05 原:Union All 把All  拿掉 ex:X29973 有兩筆(一筆為更名)->兩筆勾選->按「關係企業」->不應出現四筆
    'Modify by Amy 2020/03/16 智權人員 原:st02 ,因開發人員可能多人
    strSql = strSql & "UNION SELECT '' AS V,R06001 AS 編號,R06002 AS 名稱,R06003 AS 國籍,pcu38 AS 智權人員,PCU39 AS 狀態,PCU40 AS 備註,' ' as 申請國家,'' as 總收文號,'' as 案件性質,'' as 收文日,'' as 關聯編號,'' as 關聯名稱,'' as 關聯關係,'' as 關聯說明,'00' as R11401 FROM R140407,POTCUSTOMER,staff where id='" & strUserNum & "' AND SUBSTR(R06001,1,1)='R' AND SUBSTR(R06001,1,8)=PCU01 AND SUBSTR(R06001,9,1)=PCU02 and substr(LTrim(PCU38),1,5)=ST01(+) "
    strSql = strSql & "UNION SELECT '' AS V,R06001 AS 編號,R06002 AS 名稱,R06003 AS 國籍,poc13 AS 智權人員,POC14 AS 狀態,POC15 AS 備註,' ' as 申請國家,'' as 總收文號,'' as 案件性質,'' as 收文日,'' as 關聯編號,'' as 關聯名稱,'' as 關聯關係,'' as 關聯說明,'00' as R11401 FROM R140407,POTCUSTOMER1,STAFF where id='" & strUserNum & "' AND SUBSTR(R06001,1,1)='R' AND SUBSTR(R06001,1,8)=POC01 AND SUBSTR(R06001,9,1)=POC02 and POC13=ST01(+) "
    'end 2020/03/16
    strSql = strSql & "UNION SELECT '' AS V,R06001 AS 編號,R06002 AS 名稱,R06003 AS 國籍,' ' AS 智權人員,FA69 AS 狀態,FA29 AS 備註,' ' as 申請國家,'' as 總收文號,'' as 案件性質,'' as 收文日,'' as 關聯編號,'' as 關聯名稱,'' as 關聯關係,'' as 關聯說明,'00' as R11401 FROM R140407,FAGENT where id='" & strUserNum & "' AND SUBSTR(R06001,1,1)='Y' AND SUBSTR(R06001,1,8)=FA01(+) AND SUBSTR(R06001,9,1)=FA02(+)  "
...
If adoRecordset.RecordCount <> 0 Then
    Set grdDataList.Recordset = adoRecordset
    'Modify by Amy 2023/09/05 原程式搬至SetDataListWidth
...
'Add by Amy 2019/10/05 +所有顏色顯示
   ...
   If grdDataList.Rows > 0 Then
        For i = 1 To grdDataList.Rows - 1
            ...
            'Modify by Amy 2023/09/05 變色改共用函數
            'Modify by Amy 2023/09/26 依狀態更新智權人員改為共用函數
            Call UpdQuerySales(Me.Name, grdDataList, strField)
            'end 2023/09/26
            Call SetMSGridColorQCus(0, Me.Name, grdDataList, strField, IIf(Check3.Value = vbChecked, True, False))
   ...
   '若只有一筆資料 , 則直接設定為點選此筆資料
   'Modify by Amy 2023/09/05 原程式改至共用SetGridOneData,避免有沒改到的
   cmdOK(4).BackColor = &H8000000F
   Call SetGridOneData
   'end 2023/09/05
...
If grdDataList.col = 1 Then
    ...
    If strCopyTxt <> "" Then
        ...
        '設回原本顏色
        'Modify by Amy 2023/09/05 改寫至共用函數
'        'Add by Amy 2019/09/17 活化客戶顯示整列黃,若有呆帳編號底為紅其他欄為黃
'        If grdDataList.TextMatrix(grdDataList.row, GetValue("待活化客戶")) = "0" And Right(grdDataList.TextMatrix(grdDataList.row, GetValue("編號")), 1) <> "＊" Then
'            '呆帳
...
If grdDataList.row <> 0 Then
    If grdDataList.Text = "V" Then
         grdDataList.Text = ""
         'Modify by Amy 2023/09/05 改抓共用函數
'         'Add By Sindy 2012/3/21
'         grdDataList.col = 1
'         'Add by Amy 2019/09/17 活化客戶顯示整列黃,若有呆帳編號底為紅其他欄為黃
'         If grdDataList.TextMatrix(grdDataList.row, GetValue("待活化客戶")) = "0" And Right(grdDataList.TextMatrix(grdDataList.row, GetValue("編號")), 1) <> "＊" Then
...
'         Next i
    ...
    'Add by Amy 2020/10/15 勾選時判斷有往來記錄,往來記錄鈕變色
    'Modify by Amy 2023/03/08 欄位改動態
    'Modify by Amy 2023/09/05 bug-聯絡人也會有往來記錄,故拿掉編號只取8碼
...
Private Sub Text2_GotFocus()
   ...
   OpenIme 'Modify by Amy 2013/12/04
End Sub
...
Private Sub ChkContactRecordBT(ByVal stChk As String, ByVal stKey As String)
    'Memo by Amy 2023/09/27 原2023/09/05 將按鈕鎖住,有資料才可按,User 按此鈕新增,故不鎖
    cmdOK(4).BackColor = &H8000000F
...
'Add by Amy 2023/09/05 查詢只有一筆資料Grid顏色設定

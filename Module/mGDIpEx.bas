...
Type BITMAPFILEHEADER 'BMP檔案 檔頭資訊
    ...
    bfOffBits   As Long         'Specifies the offset, in bytes, from the BITMAPFILEHEADER structure to the bitmap bits.
End Type
...
'add by nickc 2006/01/05
Public Function LoadPictureExWmf(ByVal sFileName As String, tmpPB As PictureBox) As StdPicture
    Set MSF = New MSGrphFilter
    MSF.MsoLoadPicture sFileName, tmpPB
    Set LoadPictureExWmf = tmpPB.Image
End Function

'add by nickc 2006/01/06
Public Function ChgToWmf(tmpPB As PictureBox) As StdPicture
    'Set MSF = New MSGrphFilter
    MSF.MsoSavePicture App.Path & "\tmp.wmf", tmpPB
    'SavePicture tmpPB.Picture, App.Path & "\tmp.wmf"
    Set ChgToWmf = LoadPicture(App.Path & "\tmp.wmf")
    If Dir(App.Path & "\tmp.wmf") <> "" Then
        Kill App.Path & "\tmp.wmf"
    End If
End Function
...
Public Function SaveDIB(DIB As cDIB, _
                        ...
                        Optional ByVal JPEG_Quality As Long = 30, _
                        Optional ByVal TIFF_ColorDepth As Long = 1, _
                        Optional ByVal TIFF_Compression As Long = [EncoderValueCompressionCCITT4] _
                        ) As Boolean
    ...
    With uInfo.bmiHeader
        ...
        .biPlanes = 1
        '.biBitCount = 1  'edit by nickc 2005/11/29 改成黑白  32
        '.biBitCount = 32
        .biBitCount = IIf(lEncoder = [ImageTIFF], 1, 32)
        .biWidth = DIB.Width
        .biHeight = DIB.Height
    End With

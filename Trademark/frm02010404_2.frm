...
Begin VB.Form frm02010404_2 
   ...
   End
End
Attribute VB_Name = "frm02010404_2"
...
Private Function PromptIfTaiwanNoResult() As Boolean
   ...
   If strNation < "010" Then
      ...
      rsTmp.Close
   End If
   'Modified by Morgan 2023/1/13 電子公文除外
   If bPrompt = True And frm02010404_1.m_DocNo = "" Then
      strTit = "資料檢核"
...
'   If frm02010404_1.m_blnTM16Is1 = True Then
   If frm02010404_1.m_blnTM16Is1 = False Then
      MsgBox "本案非核准案件, 不可輸入註冊證資料!!!", vbExclamation + vbOKOnly
      GoTo EXITSUB
   End If
...
Public Sub SetData(ByVal nType As Integer, ByVal strData As String, Optional ByVal bClear As Boolean = False)
   ...
   Select Case nType
      ...
      ' 審定號
      Case 4: m_TM15 = strData
      ' 來函收文日
      Case 5: m_CP05 = strData
      ' 申請案號
      Case 6: m_TM12 = strData
      'add by nick 2004/10/20
      Case 7: m_TM14 = strData
'      Case 8: m_FinalDate = strData 'Add By Sindy 2020/12/14
   End Select
End Sub
...
Private Sub DisplayNextForm()
   frm02010404_3.SetData 0, m_CP01, True
   frm02010404_3.SetData 1, m_CP02, False
   frm02010404_3.SetData 2, m_CP03, False
   frm02010404_3.SetData 3, m_CP04, False
   frm02010404_3.SetData 4, m_CP05, False
   
   'Added by Morgan 2023/1/13 電子公文
   frm02010404_3.m_DocWord = frm02010404_1.m_DocWord
   frm02010404_3.m_DocNo = frm02010404_1.m_DocNo
   'end 2023/1/13
   
   If grdList.Rows > 2 Then
      frm02010404_3.SetData 5, "2", False
   Else
      frm02010404_3.SetData 5, "1", False
   End If
   Me.Hide
   'add by nick 2004/10/20
   frm02010404_3.SetData 6, m_TM14, False
'   frm02010404_3.SetData 7, m_FinalDate, False 'Add By Sindy 2020/12/14
   'Add By Sindy 2019/5/10
   If Not m_PrevForm Is Nothing Then
      Call frm02010404_3.SetParent(m_PrevForm)
   End If
   frm02010404_3.m_strIR01 = m_strIR01
   frm02010404_3.m_strIR02 = m_strIR02
   frm02010404_3.m_strIR03 = m_strIR03
   frm02010404_3.m_strIR04 = m_strIR04
   '2019/5/10 END
   frm02010404_3.Show
   frm02010404_3.QueryData
End Sub

Private Sub ListTradeMarkData(ByRef rsTmp As ADODB.Recordset)
   ...
   Do While rsTmp.EOF = False
      ...
      Else
         If IsNull(rsTmp.Fields("TM06")) = False Then
            grdList.TextMatrix(grdList.row, 2) = rsTmp.Fields("TM06")
         Else
            If IsNull(rsTmp.Fields("TM07")) = False Then
               grdList.TextMatrix(grdList.row, 2) = rsTmp.Fields("TM07")
            End If
         End If
      End If
      ' 申請國家
...
Private Sub ListServicePracticeData(ByRef rsTmp As ADODB.Recordset)
   Dim strNationCode As String
   InitialGrdList
   rsTmp.MoveFirst
   Do While rsTmp.EOF = False
      ...
      Else
         If IsNull(rsTmp.Fields("SP06")) = False Then
            grdList.TextMatrix(grdList.row, 2) = rsTmp.Fields("SP06")
         Else
            If IsNull(rsTmp.Fields("SP07")) = False Then
               grdList.TextMatrix(grdList.row, 2) = rsTmp.Fields("SP07")
            End If
         End If
      End If
      ' 申請國家
...
Private Sub ListData(ByRef rsTmp As ADODB.Recordset)
   ...
   If nType = 1 Or nType = 3 Then
      ListTradeMarkData rsTmp
   Else
      Select Case m_TM01
         Case "T", "TF", "FCT", "CFT":
            ListTradeMarkData rsTmp
         Case Else
            ListServicePracticeData rsTmp
      End Select
   End If
    Me.Label1.Caption = IIf(m_TM12 <> "", "申請案號 :", "審定號數 :")
   textTM15 = IIf(m_TM12 <> "", m_TM12, m_TM15)
   ...
   ' 當只有一筆記錄時, 則直接跳至下一畫面
   If grdList.Rows = 2 Then
      'Add/Modify By Cheng 2002/07/11
      If frm02010404_1.m_blnTM16Is1 = False Then
         MsgBox "本案非核准案件, 不可輸入註冊證資料!!!", vbExclamation + vbOKOnly
         Unload Me
         frm02010404_1.Show
...
'                  "ORDER BY TM01||TM02||TM03||TM04 "
               strSql = "SELECT * FROM TradeMark " & _
                  "WHERE TM01 = '" & strTM01 & "' AND " & _
                        "TM02 = '" & strTM02 & "' AND " & _
                        "TM03 = '" & strTM03 & "' AND " & _
                        "TM04 = '" & strTM04 & "' And TM16 = '1' " & _
                  "ORDER BY TM01||TM02||TM03||TM04 "
            Case "TF":
               ' 設定SQL語法
               '2005/4/13 modify by sonia
               'StrSql = "SELECT * FROM TradeMark " & _
               '   "WHERE TM01 = '" & strTM01 & "' AND " & _
               '         "TM02 = '" & strTM02 & "' "
               strSql = "SELECT * FROM TradeMark " & _
                  "WHERE TM01 = '" & strTM01 & "' AND " & _
                        "substr(TM02,1,5) = '" & Mid(strTM02, 1, 5) & "' "
               '2005/4/13 end
               If IsEmptyText(strTM03) = False Then
                  '2006/5/3 ADD BY SONIA
                  strSql = strSql & "AND SUBSTR(TM02,6,1) = '" & Mid(strTM02, 6, 1) & "' "
                  '2006/5/3 END
                  strSql = strSql & "AND "
...
Dim rsA As New ADODB.Recordset
   ...
   If grdList.row > 0 Then
      ...
      m_CP04 = grdList.Text
        'Add By Cheng 2002/11/21
        StrSQLa = "Select TM16 From TradeMark Where " & ChgTradeMark(m_CP01 & m_CP02 & m_CP03 & m_CP04)
        If rsA.State <> adStateClosed Then rsA.Close
        Set rsA = Nothing
        rsA.CursorLocation = adUseClient
        rsA.Open StrSQLa, cnnConnection, adOpenStatic, adLockReadOnly
        If rsA.RecordCount > 0 Then
            frm02010404_1.m_blnTM16Is1 = IIf("" & rsA(0).Value = "1", True, False)
        Else
            frm02010404_1.m_blnTM16Is1 = False
        End If
        If rsA.State <> adStateClosed Then rsA.Close
        Set rsA = Nothing
        'Add By Cheng 2002/11/28
        '93.4.9 MODIFY BY SONIA
        'If m_CP01 <> "T" And m_CP01 <> "TF" Then
        If m_CP01 <> "T" Then
        '93.4.9 END
            frm02010404_1.m_blnTM16Is1 = True
        End If
   End If

...
Begin VB.Form frm020102_13 
   BorderStyle     =   1  '單線固定
   Caption         =   "發文(設定質權, 撤銷設定質權)"
   ClientHeight    =   5244
   ClientLeft      =   5376
   ClientTop       =   3168
   ClientWidth     =   9144
   ...
   End
End
Attribute VB_Name = "frm020102_13"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'Memo by Amy 2021/12/27 Form2.0已修改 textTM44/textCP13/textCP14/textCP44_2/textTM23(申請人名)/cmbTM05/textCP50/textCP64/lstNameAgent
...
Dim m_Official As String    '銷帳規費   2012/8/3 add by sonia
   ...
   If rsTmp.RecordCount > 0 Then
      ...
      ' 彼所案號
'      strCP45 = Empty
      If IsNull(rsTmp.Fields("CP45")) = False Then: textTM45 = rsTmp.Fields("CP45") ': strCP45 = rsTmp.Fields("CP45")
...
'      SetCPFieldOldData "CP45", strCP45, 0
      ...
      End If
      SetCPFieldOldData "CP53", textCP53, 1
      ' 質權設定期間(迄)
      textCP54 = Empty
      If IsNull(rsTmp.Fields("CP54")) = False Then
         textCP54 = rsTmp.Fields("CP53")
      End If
      SetCPFieldOldData "CP54", textCP54, 1
      ' 進度備註
      textCP64 = Empty
...
Private Sub textAmt_GotFocus()
   InverseTextBox textAmt
End Sub
Private Sub textAmt_Validate(Cancel As Boolean)
   Dim strTit As String
   Dim strMsg As String
   Dim nResponse
   Cancel = False
If IsEmptyText(textAmt) = False Then
    If IsNumeric(textAmt) = False Then
            Cancel = True
            strTit = "資料檢核"
            strMsg = "請輸入數字"
            nResponse = MsgBox(strMsg, vbOKOnly, strTit)
            textAmt_GotFocus
    Else
        textAmt.Text = Trim(Val(textAmt.Text))
    End If
End If
End Sub
...
Private Sub OnUpdateField()
   ...
   strCP64 = textCP64
   'Add By Sindy 2010/02/12
   If CDbl(Val(textAmt)) <> 0 Then
      strCP64 = strCP64 & "；設定質權金額:" & textAmt
   End If
   '2010/02/12 End
'edit by nickc 2006/01/27
...
'         strSql = "UPDATE TradeMark SET TM24 = '" & ChgSQL(Pub_RplCu112(m_TM24, SeekCu112(1))) & "' " & _
'                  "WHERE TM01 = '" & m_TM01 & "' AND " & _
'                        "TM02 = '" & m_TM02 & "' AND " & _
'                        "TM03 = '" & m_TM03 & "' AND " & _
'                        "TM04 = '" & m_TM04 & "' "
         strSql = "UPDATE TradeMark SET TM24 = '" & ChgSQL(Pub_RplCu112(m_TM24, SeekCu112(1), m_TM23)) & "' " & _
                  "WHERE TM01 = '" & m_TM01 & "' AND " & _
                        "TM02 = '" & m_TM02 & "' AND " & _
                        "TM03 = '" & m_TM03 & "' AND " & _
                        "TM04 = '" & m_TM04 & "' "
...
Private Function CheckDataValid() As Boolean
   ...
   End If
   ' 質權設定期間不可為空白
   If IsEmptyText(textCP53) = True Or IsEmptyText(textCP54) = True Then
      strTit = "檢核資料"
      strMsg = "質權設定期間不可為空白"
      nResponse = MsgBox(strMsg, vbOKOnly, strTit)
      textCP53.SetFocus
...
Dim Cancel As Boolean
   ...
   'Add By Sindy 2010/02/12
   If Me.textAmt.Enabled = True Then
      Cancel = False
      textAmt_Validate Cancel
      If Cancel = True Then
         Exit Function
      End If
   End If

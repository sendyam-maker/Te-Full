...
Begin VB.Form frm020102_12 
   ...
   End
End
Attribute VB_Name = "frm020102_12"
...
Dim m_TM12 As String, m_TM15 As String 'Added by Lydia 2025/09/12 申請案號,審定號
...
Private Sub cmdCaseProgress_Click()
   ...
   Set frm020102_06.UpForm = Me  'add by sonia 2018/10/29
   Me.Hide
   frm020102_06.Show
...
Public Sub QueryData()
   ...
   'Modified by Morgan 2025/10/14 不必限制T案 Ex:FCT-053705
...
Private Sub textCP43_Validate(Cancel As Boolean)
   ...
   If IsEmptyText(textCP43) = False Then
      ...
      If (textCP10 = "自請撤回" Or textCP10 = "自請撤銷") And rsTmp.Fields("CP10") = "303" Then
         rsTmp.Close
         Cancel = True
         strTit = "資料檢核"
...
'   OnUpdateCaseProgress
   ...
   If m_TM01 = "T" And m_CP10 = "307" Then
      strTFcase = PUB_GetTFbaseInfo(m_TM01, m_TM02, m_TM03, m_TM04, m_TM15, m_TM10, "2", m_TM12, m_CP09)
   End If
   'end 2025/09/12
   ...
   Select Case m_TM01
      ' 系統類別為T,TF的為更新商標基本檔
      Case "T", "TF":
         If textTM29 = "Y" Then '2011/10/4 MODIFY BY SONIA 閉卷才做,同時更新閉卷日期及原因
            strSql = "UPDATE TradeMark SET TM29='Y',TM31='09',TM30=" & strSrvDate(1) & " " & _
                     "WHERE TM01 = '" & m_TM01 & "' AND " & _
                           "TM02 = '" & m_TM02 & "' AND " & _
                           "TM03 = '" & m_TM03 & "' AND " & _
                           "TM04 = '" & m_TM04 & "' "
            cnnConnection.Execute strSql
         End If                 '2011/10/4 ADD BY SONIA
         'add by nickc 2006/11/17
         If textPrint <> "N" Then
      ...
      Case Else:
         If textTM29 = "Y" Then '2011/10/4 MODIFY BY SONIA 閉卷才做,同時更新閉卷日期及原因
            strSql = "UPDATE ServicePractice SET SP15='Y',SP17='09',SP16=" & strSrvDate(1) & " " & _
                     "WHERE SP01 = '" & m_TM01 & "' AND " & _
                           "SP02 = '" & m_TM02 & "' AND " & _
                           "SP03 = '" & m_TM03 & "' AND " & _
                           "SP04 = '" & m_TM04 & "' "
...
'                        "TM04 = '" & m_TM04 & "' "
   ...
   If Trim(textCP43) <> "" Then
            ...
            rsTmp.Close
   End If
    'add by nickc 2007/10/16 加入  自請拋棄商標權(307)發文時，自動閉卷，不管台灣還是大陸
    If m_TM01 = "T" And m_CP10 = "307" Then
        strSql = "UPDATE TradeMark SET TM29 = 'Y',tm30=to_number(to_char(sysdate,'YYYYMMDD')),tm31='09' " & _
                 "WHERE TM01 = '" & m_TM01 & "' AND " & _
                       "TM02 = '" & m_TM02 & "' AND " & _
...
Private Sub InsExpField()
   ...
   '92.11.10 END
      ...
      If m_TM10 >= "010" Then
         ...
         If textPrint = "1" Then
            ' 清除定稿例外欄位檔原有資料
            EndLetter "01", m_CP09, "15", strUserNum
         ' 申請人國籍非台灣
         'edit by nickc 2006/06/30
         'Else
         ElseIf textPrint = "2" Then
            ' 清除定稿例外欄位檔原有資料
            EndLetter "01", m_CP09, "16", strUserNum
            'Add By Sindy 2016/8/18 有費用
            If Val(m_CP16) > 0 Then
               strSql = "INSERT INTO EXCEPTCONDITION (ET01,ET02,ET03,ET04,ET05,ET06) " & _
                        "VALUES ('" & "01" & "','" & m_CP09 & "','" & "16" & "','" & strUserNum & "'," & _
                        "'" & "有費用" & "','及本所收費通知各乙紙')"
               cnnConnection.Execute strSql
            End If
            '2016/8/18 END
      ...
      Else
         ...
         If textPrint = "1" Then
            EndLetter "01", m_CP09, "40", strUserNum
         End If
      End If
      '92.11.10 END
   '92.11.10 CANCEL BY SONIA
...
'2012/1/12 End
   ...
   '92.11.10 END
      ...
      If m_TM10 < "010" Then
         ...
         If textPrint = "1" Then
            ' 列印定稿
'            NowPrint m_CP09, "01", "15", False, strUserNum, 0
            ET03 = "15" 'Modify By Sindy 2012/1/12
         ' 申請人國籍非台灣
         'edit by nickc 2006/06/30
         'Else
         ElseIf textPrint = "2" Then
...
'            NowPrint m_CP09, "01", "16", False, strUserNum, 0
      ...
      Else
         'add by nickc 2006/06/30
         If textPrint = "1" Then
'            NowPrint m_CP09, "01", "40", False, strUserNum, 0
            ET03 = "40" 'Modify By Sindy 2012/1/12
         End If
      End If
      '92.11.10 END
   '92.11.10 CANCEL BY SONIA
...
Dim Cancel As Boolean
   ...
   If textCP84.Enabled = True And m_TM10 = "000" Then
       If Val(textCP84.Text) <> Val(m_CP84) Then
           If MsgBox("收文規費[" & Trim(m_CP84) & "] 與實際發文規費[" & Trim(Val(textCP84.Text)) & "]不同", vbOKCancel) = vbCancel Then
               textCP84_GotFocus
               Exit Function
           End If
       End If

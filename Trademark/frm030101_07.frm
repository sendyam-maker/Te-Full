...
Begin VB.Form frm030101_07 
   ...
   End
End
Attribute VB_Name = "frm030101_07"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'Memo by Lydia 2021/08/11 改成Form2.0 ; textCP13、textCP14、textTM23、textTM05、textTM06、textTM07、textCP44_2、textCP64
...
Dim m_CP43 As String, m_CP43cp10 As String 'Added by Lydia 2023/09/22 相關收文號和案件性質
...
Private Sub cmdMod_Click()
   ...
   frm030101_05.SetParent "frm030101_07"
   'Me.Hide
   frm030101_05.Show
   ...
   Select Case Index
      Case 0, 1
         If CheckDataValid = True Then
            ...
            ' 90.08.29 檢查變更事項檔
            'Modified by Lydia 2024/02/06 註冊證之更正發文不用輸入變更事項; ex.CFT-022712
            'If IsChangeEventExist(m_CP09) = True Then
            If IsChangeEventExist(m_CP09) = True Or cmdMod.Visible = False Then
               ' 設定滑鼠游標為等待狀態
...
'Private Sub Form_Activate()
    ...
    'Modify By Sindy 2010/5/31 按下變更事項按鈕,不要重新讀取資料(不異動畫面原資料)
...
'      textCP27 =  DBDATE(Date)
      ...
      'end 2021/06/04
      'Added by Lydia 2023/09/22 避免人員誤按，若為註冊證之更正發文時，將畫面的變更事項按鈕隱藏。
      ...
      If m_CP43 <> "" Then
         ...
         End If
      End If
      'Modified by Lydia 2024/04/17 +更正延展證書
      If m_CP10 = "302" And InStr("1701,1713,", m_CP43cp10) > 0 And m_CP43cp10 <> "" Then
...
'計算催審期限
Private Sub CountUrgeDate()
Dim strDay As String
Dim strCP43CP10 As String
   
   'Modify By Sindy 2023/5/3
   'Mark by Lydia 2023/09/22 改變數
   'strSql = "SELECT cp09,cp10 FROM CaseProgress where cp09 in(" & _
            "SELECT cp43 FROM CaseProgress " & _
            "WHERE CP09 = '" & m_CP09 & "' and cp43 is not null)"
   'intI = 1
   'Set RsTemp = ClsLawReadRstMsg(intI, strSql)
   'If intI = 1 Then
   '   strCP43CP10 = RsTemp.Fields("cp10")
   'End If
   strCP43CP10 = m_CP43cp10
   'end 2023/09/22
   If strCP43CP10 = "1701" Or strCP43CP10 = "1713" Then '註冊證,延展證書
      '催審期限:發文日+6個月
      strDay = DBDATE(DateAdd("m", 6, ChangeWStringToWDateString(DBDATE(textCP27))))
   Else
   '2023/5/3 END
      strDay = GetUrgeDate(m_TM01, m_TM10, m_CP10, textCP27)
   End If
...
'         End If
      End If
      Me.textCP27.Tag = Me.textCP27.Text  'Added by Lydia 2019/11/08
   End If
EXITSUB:
End Sub
...
Private Function CheckDataValid() As Boolean
   ...
   If m_CP10 = "301" Then
      ...
      End If
   Else
      If m_CP10 <> "302" Then 'Added by Lydia 2023/09/12 更正：不必再檢查一定要按變更事項的按鈕。

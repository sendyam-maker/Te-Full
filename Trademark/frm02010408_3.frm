...
Begin VB.Form frm02010408_3 
   ...
   End
End
Attribute VB_Name = "frm02010408_3"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'Memo by Lydia 2023/10/18 Form2.0已修改 ; grdList從MSFlexGrid改為MSHFlexGrid
'Memo by Amy 2021/12/29 Form2.0已修改 textTM05/textTM07/textTM23/grdList
...
' 取得催審期間的資料
Private Function GetUrgeData(ByVal strKey As String) As String
   Dim rsTmp As New ADODB.Recordset
   Dim strSql As String
   ...
   If rsTmp.RecordCount > 0 Then
      ...
      Do While rsTmp.EOF = False
         If IsNull(rsTmp.Fields("NP06")) = True Then
            If IsNull(rsTmp.Fields("NP09")) = False Then
               GetUrgeData = rsTmp.Fields("NP09")
            End If
            Exit Do
         End If
         If IsEmptyText(rsTmp.Fields("NP06")) = True Then
            If IsNull(rsTmp.Fields("NP09")) = False Then
               GetUrgeData = rsTmp.Fields("NP09")
            End If
            Exit Do
...
Private Sub ListData()
   ...
   If rsTmp.RecordCount > 0 Then
      ...
      Do While rsTmp.EOF = False
         bSpecial = False
         strCP09 = Empty
         'add by nickc 2006/09/08 204、205、407 不管有沒發文
         If CheckStr(rsTmp.Fields("cp10")) <> "204" And CheckStr(rsTmp.Fields("cp10")) <> "205" And CheckStr(rsTmp.Fields("cp10")) <> "407" Then
            ' 無發文日不予列入
         ...
         Else
            ' 取消收文日不予列入
            If Not IsNull(rsTmp.Fields("CP57")) = True Then: GoTo NextRecord
            ' 取消收文日是空白不予列入
            If Not IsEmptyText("" & rsTmp.Fields("CP57")) = True Then: GoTo NextRecord
         End If
         ' 無收文號不予列入
         If IsNull(rsTmp.Fields("CP09")) = True Then: GoTo NextRecord
         ' 收文號不為A,B類的不予計入
         Select Case Mid(rsTmp.Fields("CP09"), 1, 1)
            '92.10.31 modify by sonia 加入C類部分案件性質
            'Case "A", "B":
            Case "A", "B", "C":
            '92.10.31 END
...
Private Sub DisplayNextForm()
   frm02010408_4.SetData 0, m_TM01, True
   frm02010408_4.SetData 1, m_TM02, False
   frm02010408_4.SetData 2, m_TM03, False
   frm02010408_4.SetData 3, m_TM04, False
   frm02010408_4.SetData 4, m_CP05, False
   frm02010408_4.SetData 5, m_CP09, False
   'Added by Morgan 2017/4/24 電子公文
   frm02010408_4.m_DocWord = frm02010408_1.m_DocWord
   frm02010408_4.m_DocNo = frm02010408_1.m_DocNo
   frm02010408_4.m_AppNo = frm02010408_1.m_AppNo
   frm02010408_4.m_DeadLine = frm02010408_1.m_DeadLine
   frm02010408_4.m_NewCP10 = frm02010408_1.m_NewCP10
   'end 2017/4/24
   'Add By Sindy 2019/5/10
   If Not m_PrevForm Is Nothing Then
      Call frm02010408_4.SetParent(m_PrevForm)
   End If
   frm02010408_4.m_strIR01 = m_strIR01
   frm02010408_4.m_strIR02 = m_strIR02
   frm02010408_4.m_strIR03 = m_strIR03
   frm02010408_4.m_strIR04 = m_strIR04
   '2019/5/10 END
   Me.Hide
   frm02010408_4.Show
   frm02010408_4.QueryData
End Sub

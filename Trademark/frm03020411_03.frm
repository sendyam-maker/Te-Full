...
Begin VB.Form frm03020411_03 
   ...
   End
End
Attribute VB_Name = "frm03020411_03"
...
'Create by Lydia 2018/01/10 FCT-修改承辦人(案件進度檔)
Option Explicit
...
Private Sub ListData()
   ...
   'Modifed by Lydia 2018/02/05 增加一個月內C類來函1701註冊證和1001核准並且承辦人為「程序人員」之進度
   'strSql = "SELECT a.*,b.st52||','||st53||','||st54||','||st55 as SA FROM CaseProgress a ,Staff  b " & _
            "WHERE CP01 = '" & m_TM01 & "' AND " & _
                  "CP02 = '" & m_TM02 & "' AND " & _
                  "CP03 = '" & m_TM03 & "' AND " & _
                  "CP04 = '" & m_TM04 & "' AND CP14=ST01(+) " & _
                  "AND CP158=0 AND CP159=0 "
   strSql = "SELECT a.*,b.st52||','||b.st53||','||b.st54||','||b.st55 as SA,b.st03 as cp14d,c.st52||','||c.st53||','||c.st54||','||c.st55 as SA2,c.st03 as cp13d " & _
               "FROM CaseProgress a ,Staff  b, Staff c " & _
               "WHERE CP01 = '" & m_TM01 & "' AND " & _
                  "CP02 = '" & m_TM02 & "' AND " & _
                  "CP03 = '" & m_TM03 & "' AND " & _
                  "CP04 = '" & m_TM04 & "' AND CP14=b.ST01(+) AND CP13=c.ST01(+) "
   strSql = strSql & "AND CP159=0 AND (CP158=0 OR CP158>=" & CompDate(1, -1, strSrvDate(1)) & ") "
   ...
   If rsTmp.RecordCount > 0 Then
      ...
      Do While rsTmp.EOF = False
         ...
         Select Case Mid(rsTmp.Fields("CP09"), 1, 1)
            Case "A": GoTo NextRecord
            Case "B"
                    If "" & rsTmp.Fields("CP10") <> "722" Then
                           GoTo NextRecord
                    End If
            Case Else:
         End Select
         ...
         If InStr("1001,1701", "" & rsTmp.Fields("CP10")) > 0 And "" & rsTmp.Fields("CP14D") = "F12" And "" & rsTmp.Fields("CP13D") = "F11" Then
                If InStr(rsTmp.Fields("CP13") & "," & rsTmp.Fields("SA2"), strUserNum) = 0 Then
                        GoTo NextRecord
                End If
         Else
                '因為一開始就CP14預設到主管,所以承辦人只能是本人或下屬
                If InStr(rsTmp.Fields("CP14") & "," & rsTmp.Fields("SA"), strUserNum) = 0 Then
                    'Remark by Lydia 2018/02/05 不彈訊息
                    'If Pub_StrUserSt03 = "M51" Then
                    '    If MsgBox("電腦中心人員是否繼續作業?", vbYesNo + vbDefaultButton2) = vbNo Then
                    '        GoTo NextRecord
                   '     End If
                   ' Else
                        GoTo NextRecord
...
Private Sub DisplayNextForm()
   frm03020411_04.SetData 0, m_TM01, True
   frm03020411_04.SetData 1, m_TM02, False
   frm03020411_04.SetData 2, m_TM03, False
   frm03020411_04.SetData 3, m_TM04, False
   frm03020411_04.SetData 4, m_CP05, False
   frm03020411_04.SetData 5, m_CP09, False
   Me.Hide
   frm03020411_04.Show
   frm03020411_04.QueryData
End Sub

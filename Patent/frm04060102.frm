...
Private Function GetCompany01() As String
   strSql = "select ta04 from tagent where ta01='P' and ta02='01'"
   intI = 1
   Set RsTemp = ClsLawReadRstMsg(intI, strSql)
   If intI = 1 Then
      GetCompany01 = "" & RsTemp(0)
   End If
End Function
' 從資料庫中取得所有的資料
Private Function GetDBData_RP(ByVal nReport As Integer) As Boolean
   ...
   Dim strCompany01 As String
   
   strCompany01 = GetCompany01 'Add by Morgan 2007/10/18
   ...
   Select Case nReport
      ...
      'add by nickc 2006/04/27
      Case 3
         strSql = "SELECT  TPB01,TPB02,TPB03,TPB04,TPB05,decode(tpb06,'015','C4','016','C4','039','C4'" & IIf(Check1.Value = 1, ",'020','C0'", "") & ",substr(na02,1,2))||TPB06 TPB06,TPB07,TPB08,'' TPB09,PA01,PA02,PA03,PA04 FROM TPBulletin,nation,PATENT where na01(+)=tpb06 and pa11(+)=tpb01 and pa23(+)='1'"
         StrSQL3 = "SELECT  TPB01,TPB02,TPB03,TPB04,TPB05,decode(tpb06,'015','C4ZZZ','016','C4ZZZ','039','C4ZZZ'" & IIf(Check1.Value = 1, ",'020','C0ZZZ'", "") & ",substr(na02,1,2)||decode(substr(na02,1,1),'C','ZZZ','ZZB')) TPB06,TPB07,TPB08,Tpb09,PA01,PA02,PA03,PA04 FROM TPBulletin,nation,PATENT where na01(+)=tpb06 and pa11(+)=tpb01 and pa23(+)='1'" & IIf(Check1.Value = 1, " and na02>'B' ", " and na02>'C' ")
   End Select
   
   
   If IsEmpty(text01_01) = False Then
      strSubSQL = strSubSQL & " and TPB03 >= " & ChangeTStringToWString(text01_01) & " "
   End If
   
   If IsEmpty(text01_02) = False Then
      strSubSQL = strSubSQL & " and TPB03 <= " & ChangeTStringToWString(text01_02) & " "
   End If
   
   If nReport = 3 Or nReport = 4 Then
      If IsEmpty(text04_01) = False Then
         strSubSQL = strSubSQL & " and TPB06 >= '" & text04_01 & "' "
      End If
      If IsEmpty(text04_02) = False Then
         strSubSQL = strSubSQL & " and TPB06 <= '" & text04_02 & "' "
      End If
   End If
      
   If strSubSQL <> Empty Then
      strSql = strSql & strSubSQL
      StrSQL3 = StrSQL3 & strSubSQL
   End If
   
   If nReport = "3" Then
      strSql = strSql & " union " & StrSQL3
   End If
   
   strSql = strSql & " order by Tpb06 "
   ...
   Do While Not rsMain.EOF
      strZone = Empty
      If IsNull(rsMain.Fields("TPB06")) = False Then
         strZone = rsMain.Fields("TPB06")
      End If
      
      'Add by Morgan 2007/10/17 檢查無代理人且為本所案件
      If IsNull(rsMain.Fields("TPB07")) And Not IsNull(rsMain.Fields("PA01")) Then
         strAgent = Empty
         strCompany = Empty
         strSql = "select substr(max(cp27||cp22),9) from caseprogress where cp01='" & rsMain("pa01") & "' and cp02='" & rsMain("pa02") & "' and cp03='" & rsMain("pa03") & "' and cp04='" & rsMain("pa04") & "' and cp09<'C' and cp27<" & rsMain("tpb03")
         intI = 1
         Set RsTemp = ClsLawReadRstMsg(intI, strSql)
         If intI = 1 Then
            ...
            If "" & RsTemp(0) = "N" Then
               strAgent = "01"
               strCompany = strCompany01
            End If
         End If
      Else
      'end 2007/10/17
         ' 代理人代碼
         If IsNull(rsMain.Fields("TPB07")) = False Then
            strAgent = rsMain.Fields("TPB07")
         Else
            strAgent = Empty
         End If
         
         ' 事務所名稱
         If IsNull(rsMain.Fields("TPB08")) = False Then
            strCompany = rsMain.Fields("TPB08")
         Else
            strCompany = Empty
         End If
      End If
      ...
      'Select Case Mid(rsMain.Fields("TPB01"), 3, 1)
      Select Case Mid(rsMain.Fields("TPB01"), 4, 1)
         Case "1": nType = 1
         Case "2": nType = 2
         Case "3": nType = 3
      End Select

...
Begin VB.Form frm06010604_2 
   ...
   Begin VB.ComboBox Combo1 
      Height          =   300
      ItemData        =   "frm06010604_2.frx":0000
      Left            =   1080
      List            =   "frm06010604_2.frx":000D
      Style           =   2  '單純下拉式
      TabIndex        =   5
      Top             =   960
      Width           =   615
   ...
   End
End
Attribute VB_Name = "frm06010604_2"
...
Private Sub FormConfirm()
 Dim bolChk As Boolean, i As Integer, j As Integer, strTmp(1 To 2) As String
   ...
   frm06010604_3.m_DocWord = frm06010604_1.m_DocWord
   frm06010604_3.m_DocNo = frm06010604_1.m_DocNo
   frm06010604_3.m_DocDate = frm06010604_1.m_DocDate
   frm06010604_3.m_AppNo = frm06010604_1.m_AppNo
   frm06010604_3.m_DeadLine = frm06010604_1.m_DeadLine
   'end 2017/5/10
   'Added by Lydia 2023/09/25
   frm06010604_3.m_strIR01 = m_strIR01
   frm06010604_3.m_strIR02 = m_strIR02
   frm06010604_3.m_strIR03 = m_strIR03
   frm06010604_3.m_strIR04 = m_strIR04
   'end 2023/09/25
   frm06010604_3.Show
   Me.Hide
End Sub
...
Private Sub Form_Load()
   ...
   'Added by Lydia 2023/09/25
   m_strIR01 = frm06010604_1.m_strIR01
   m_strIR02 = frm06010604_1.m_strIR02
   m_strIR03 = frm06010604_1.m_strIR03
   m_strIR04 = frm06010604_1.m_strIR04
   If m_strIR01 <> "" Then
...
Dim Lbl As LABEL, txt As TextBox, i As Integer, j As Integer
Dim strTmp(0 To 5) As String, varTmp As Variant
   ...
   strExc(0) = "SELECT '',CP09,CPM03," & _
      ...
      "CP10,DECODE(CP27,19221111,99999999,CP27) AS SORTFIELD,CP05 from caseprogress,casepropertymap,CUSTOMER where " & _
   ...
   'add by sonia 2017/7/25
   strExc(0) = strExc(0) & " union " & _
   "select '',CP09,CPM03," & _
      "DECODE(CP10,'704',CP50,'705',CP50,'706',CP50,'701',NVL(CU04,NVL(CU05,CU06)),CP40)," & _
      SQLDate("CP06") & "," & SQLDate("CP07") & "," & SQLDate("CP27") & ",decode(CP24,'1','准,勝','2','駁,敗',''),CP64," & _
      "CP10,DECODE(CP27,19221111,99999999,CP27) AS SORTFIELD,CP05 from caseprogress,casepropertymap,CUSTOMER,nextprogress where " & _
      ChgCaseprogress(pa(1) & pa(2) & pa(3) & pa(4)) & _
   ...
   'end 2017/7/25
   'Modify by Morgan 2010/8/24 排序改依收文日,收文號由小至大--靜芳
   ...
   'add by sonia 2016/3/24 預設新申請案那一道
   'Remove by Lydia 2023/03/16 請取消"一般來函預設勾選新申請案"，由輸入人員自行判斷勾選 ---Sharon

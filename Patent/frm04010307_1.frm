...
Begin VB.Form frm04010307_1 
   ...
   Begin VB.ComboBox Combo1 
      Height          =   300
      ItemData        =   "frm04010307_1.frx":0000
      Left            =   1170
      List            =   "frm04010307_1.frx":000D
      Style           =   2  '單純下拉式
      TabIndex        =   19
      Top             =   720
      Width           =   615
   ...
   End
End
Attribute VB_Name = "frm04010307_1"
...
'            End If
            ...
            '2019/10/9 END
               ...
               'strFileName = strFolder & "\" & m_CaseNo & "專利申請延展指定期間申請書"
               'Modify By Sindy 2020/1/6
               strFileName = strFolder & "\" & m_CaseNo & ".data"
               'Call PUB_MakeDoc(strExc(9), strFileName)
            End If
            '1.基本資料
            If Check2.Value = 1 Then 'Modify By Sindy 2018/8/9 韻丞:沒勾選基本資料表，可以不用出CONTACT
...
'            Text12 = Mid(strExc(0), 1, InStr(RsTemp("cp08"), "號") - 1)
'         End If
'      'End If
'   End If
'   If Text10 = "" Then
   ...
   If Me.Text9.Enabled = True Then
      ...
      For ii = 1 To Me.MSHFlexGrid1.Rows - 1
         If Me.MSHFlexGrid1.TextMatrix(ii, 0) <> "" Then
            ...
            End If
            '2018/7/31 END
            Call GetCP08(Me.MSHFlexGrid1.TextMatrix(ii, 7))
            Exit For
         End If
      Next ii
   End If
...
Private Sub GetCP08(strCP09 As String)
   'Add By Sindy 2018/7/4
   '來函文號:
   'CP05 Desc -> NVL(ED08,CP05) Desc 歸卷公文會有一筆以上，考慮可能有紙本公文保留CP05的判斷
   strExc(0) = "select cp08,ed08 from caseprogress,edocument where CP01='" & pa(1) & "' AND CP02='" & pa(2) & "' AND CP03='" & pa(3) & "' AND CP04='" & pa(4) & "' AND CP09='" & strCP09 & "' AND ed11(+)=cp09 ORDER BY NVL(ED08,CP05) Desc"
   intI = 1
   Set RsTemp = ClsLawReadRstMsg(intI, strExc(0))
   If intI = 1 Then
      If Not IsNull(RsTemp("ED08")) Then
   ...
   If Text10 = "" Then
      ...
      strExc(0) = "select cp08,ed08 from caseprogress,edocument" & _
                  ...
                  " where CP01='" & pa(1) & "' AND CP02='" & pa(2) & "' AND CP03='" & pa(3) & "' AND CP04='" & pa(4) & "'" & _
                  " AND CP09='" & strCP09 & "' and cp43 is not null)" & _
...
On Error GoTo ErrorHandler
   ...
   cp(110) = m_CP110 'Move By Sindy 2019/10/15
...
'   End If
   ...
   If Text9 = 申復 Then
      ii = ii + 1
      strTxt(ii) = "INSERT INTO EXCEPTCONDITION (ET01,ET02,ET03,ET04,ET05,ET06) " & _
         " VALUES ('" & ET01 & "','" & strReceiveNo & "','" & ET03 & "','" & strUserNum & "','延期前期限','文到次日起3個月內。')"
   ElseIf Val(txtCP07) > 0 Then
      strExc(1) = Left(txtCP07, Len(txtCP07) - 4) & "年" & Mid(txtCP07, Len(txtCP07) - 4 + 1, 2) & "月" & Right(txtCP07, 2) & "日"
      '法定期限
      ii = ii + 1
      strTxt(ii) = "INSERT INTO EXCEPTCONDITION (ET01,ET02,ET03,ET04,ET05,ET06) " & _
         " VALUES ('" & ET01 & "','" & strReceiveNo & "','" & ET03 & "','" & strUserNum & "','延期前期限','" & strExc(1) & "前。')"
      '法定期限+2個月
   ...
   Select Case Text9
      Case 翻譯, 檢視中說, 核對中說格式
         strTmp = "因「尚在準備中，不克於期限內補呈」之原因，「摘要、專利說明書、申請專利範圍及圖式」文件申請延期「至" & strCP07Add2M & "」補正。"
      Case 製作中說
         strTmp = "因「尚在準備中，不克於期限內補呈」之原因，「設計專利說明書及圖式」文件申請延期「至" & strCP07Add2M & "」補正。"
      Case 補文件
         '請抓延期進度檔的備註
         strTmp = "因「文件尚在準備中，不克於期限內補呈」之原因，「" & cp(64) & "」文件申請延期「至" & strCP07Add2M & "」補正。"
      Case 申復
         '原審查意見函的法定期限+3個月
         strExc(1) = "　年　月　日"
         ...
         End If
         strTmp = "因「申請人正積極準備相關資料，由於本案有其複雜性且事關權益，申請人正與公司內部人員研討，實難於　鈞局指定期限內提呈相關資料及修正。故依專利法施行細則第六條規定申請延期3個月，並將於近期內補呈相關資料或修正，務使本專利申請案所請內容儘量符合專利法之規定」之原因，「相關資料或修正」文件申請延期「至" & strExc(1) & "」補正。"
      Case "107" '再審查
         '第1次延期出再審申請書
         If Val(Text6) = 1 Then
            ...
            ii = ii + 1
            strTxt(ii) = "INSERT INTO EXCEPTCONDITION (ET01,ET02,ET03,ET04,ET05,ET06) " & _
               " VALUES ('" & ET01 & "','" & strReceiveNo & "','" & ET03 & "','" & strUserNum & "','備註','再審查申請規費及再審查理由書請准予後補。')"
            
            '1002.核駁-新申請案
            strExc(0) = "select cp05,cp08,ed08 from caseprogress,edocument where CP01='" & pa(1) & "' AND CP02='" & pa(2) & "' AND CP03='" & pa(3) & "' AND CP04='" & pa(4) & "' AND ed11(+)=cp09 AND cp10='1002' AND cp43 in(select cp09 from caseprogress where CP01='" & pa(1) & "' AND CP02='" & pa(2) & "' AND CP03='" & pa(3) & "' AND CP04='" & pa(4) & "' and cp10 in(" & NewCasePtyList & ")) ORDER BY CP05 DESC"
            intI = 1
            Set RsTemp = ClsLawReadRstMsg(intI, strExc(0))
            If intI = 1 Then
               If Not IsNull(RsTemp("ED08")) Then
         ...
         Else
            ...
            ii = ii + 1
            strTxt(ii) = "INSERT INTO EXCEPTCONDITION (ET01,ET02,ET03,ET04,ET05,ET06) " & _
               " VALUES ('" & ET01 & "','" & strReceiveNo & "','" & ET03 & "','" & strUserNum & "','一併申請修正','是/否')"
            ii = ii + 1
            strTxt(ii) = "INSERT INTO EXCEPTCONDITION (ET01,ET02,ET03,ET04,ET05,ET06) " & _
               " VALUES ('" & ET01 & "','" & strReceiveNo & "','" & ET03 & "','" & strUserNum & "','一併申請誤譯訂正','是/否')"
            '2019/10/15 END
            
            strTmp = "因「相關資料或修正，不克於期限內補呈」之原因，「再審查申請書、理由書及再審查申請規費」文件申請延期「至" & strCP07Add2M & "」補正。"
         End If
   End Select
   If strTmp <> "" Then
      ii = ii + 1

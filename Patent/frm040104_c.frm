VERSION 5.00
Object = "{0D452EE1-E08F-101A-852E-02608C4D0BB4}#2.0#0"; "FM20.DLL"
Begin VB.Form frm040104_c 
   BorderStyle     =   1  '單線固定
   Caption         =   "內專發文-讓與, 專利權讓與, 合併, 繼承"
   ClientHeight    =   5760
   ClientLeft      =   300
   ClientTop       =   1356
   ClientWidth     =   8532
   ControlBox      =   0   'False
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ScaleHeight     =   5760
   ScaleWidth      =   8532
   Begin VB.TextBox txtCP113 
      Height          =   270
      Left            =   5220
      MaxLength       =   4
      TabIndex        =   5
      Top             =   2655
      Width           =   540
   End
   Begin VB.TextBox txtPayToday 
      Height          =   270
      Left            =   7650
      MaxLength       =   1
      TabIndex        =   19
      Top             =   4470
      Width           =   255
   End
   Begin VB.TextBox txtCP118 
      Height          =   270
      Left            =   7650
      MaxLength       =   1
      TabIndex        =   16
      Top             =   4140
      Width           =   255
   End
   Begin VB.TextBox txtChkRltDate 
      Height          =   270
      Left            =   915
      MaxLength       =   8
      TabIndex        =   21
      Top             =   5460
      Width           =   975
   End
   Begin VB.TextBox Text9 
      Height          =   270
      Left            =   915
      MaxLength       =   8
      TabIndex        =   3
      Top             =   2655
      Width           =   1212
   End
   Begin VB.TextBox txtCP84 
      Height          =   270
      Left            =   3120
      TabIndex        =   4
      Top             =   2655
      Width           =   1092
   End
   Begin VB.TextBox txtAD 
      Enabled         =   0   'False
      Height          =   270
      Index           =   5
      Left            =   2160
      MaxLength       =   1
      TabIndex        =   18
      Top             =   4500
      Width           =   240
   End
   Begin VB.TextBox txtAppNew 
      Height          =   270
      Index           =   5
      Left            =   915
      MaxLength       =   9
      TabIndex        =   17
      Top             =   4500
      Width           =   1212
   End
   Begin VB.TextBox txtAD 
      Enabled         =   0   'False
      Height          =   270
      Index           =   4
      Left            =   2160
      MaxLength       =   1
      TabIndex        =   15
      Top             =   4200
      Width           =   240
   End
   Begin VB.TextBox txtAppNew 
      Height          =   270
      Index           =   4
      Left            =   915
      MaxLength       =   9
      TabIndex        =   14
      Top             =   4200
      Width           =   1212
   End
   Begin VB.TextBox txtAD 
      Enabled         =   0   'False
      Height          =   270
      Index           =   3
      Left            =   2160
      MaxLength       =   1
      TabIndex        =   13
      Top             =   3900
      Width           =   240
   End
   Begin VB.TextBox txtAppNew 
      Height          =   270
      Index           =   3
      Left            =   915
      MaxLength       =   9
      TabIndex        =   12
      Top             =   3900
      Width           =   1212
   End
   Begin VB.TextBox txtAD 
      Enabled         =   0   'False
      Height          =   270
      Index           =   2
      Left            =   2160
      MaxLength       =   1
      TabIndex        =   11
      Top             =   3600
      Width           =   240
   End
   Begin VB.TextBox txtAppNew 
      Height          =   270
      Index           =   2
      Left            =   915
      MaxLength       =   9
      TabIndex        =   10
      Top             =   3600
      Width           =   1212
   End
   Begin VB.TextBox txtAD 
      Enabled         =   0   'False
      Height          =   270
      Index           =   1
      Left            =   2160
      MaxLength       =   1
      TabIndex        =   9
      Top             =   3300
      Width           =   240
   End
   Begin VB.TextBox txtAppNew 
      Height          =   270
      Index           =   1
      Left            =   915
      MaxLength       =   9
      TabIndex        =   8
      Top             =   3300
      Width           =   1212
   End
   Begin VB.ComboBox Combo2 
      Height          =   300
      Left            =   915
      TabIndex        =   7
      Top             =   2970
      Width           =   1500
   End
   Begin VB.TextBox Text10 
      Height          =   270
      Left            =   7080
      MaxLength       =   9
      TabIndex        =   23
      Top             =   1530
      Visible         =   0   'False
      Width           =   1335
   End
   Begin VB.CommandButton cmdOK 
      BackColor       =   &H008080FF&
      Cancel          =   -1  'True
      Caption         =   "變更事項(&R)"
      CausesValidation=   0   'False
      Height          =   400
      Index           =   2
      Left            =   5160
      Style           =   1  '圖片外觀
      TabIndex        =   25
      Top             =   70
      Width           =   1200
   End
   Begin VB.CommandButton cmdOK 
      Caption         =   "確定(&O)"
      Default         =   -1  'True
      Height          =   400
      Index           =   0
      Left            =   6384
      TabIndex        =   26
      Top             =   70
      Width           =   800
   End
   Begin VB.CommandButton cmdOK 
      Caption         =   "回前畫面(&U)"
      CausesValidation=   0   'False
      Height          =   400
      Index           =   1
      Left            =   7212
      TabIndex        =   27
      Top             =   70
      Width           =   1200
   End
   Begin VB.CommandButton cmdOK 
      Caption         =   "同時發文(&N)"
      Height          =   400
      Index           =   3
      Left            =   3936
      TabIndex        =   24
      Top             =   70
      Width           =   1200
   End
   Begin VB.TextBox Text1 
      BackColor       =   &H00E0E0E0&
      Enabled         =   0   'False
      Height          =   270
      Left            =   960
      MaxLength       =   3
      TabIndex        =   31
      Top             =   768
      Width           =   495
   End
   Begin VB.TextBox Text2 
      BackColor       =   &H00E0E0E0&
      Enabled         =   0   'False
      Height          =   270
      Left            =   1440
      MaxLength       =   6
      TabIndex        =   30
      Top             =   768
      Width           =   855
   End
   Begin VB.TextBox Text3 
      BackColor       =   &H00E0E0E0&
      Enabled         =   0   'False
      Height          =   270
      Left            =   2280
      MaxLength       =   1
      TabIndex        =   29
      Top             =   768
      Width           =   255
   End
   Begin VB.TextBox Text4 
      BackColor       =   &H00E0E0E0&
      Enabled         =   0   'False
      Height          =   270
      Left            =   2520
      MaxLength       =   2
      TabIndex        =   28
      Top             =   768
      Width           =   375
   End
   Begin MSForms.ListBox lstNameAgent 
      Height          =   1392
      Left            =   6888
      TabIndex        =   6
      Top             =   2700
      Width           =   1500
      ScrollBars      =   2
      DisplayStyle    =   2
      Size            =   "2646;2455"
      MatchEntry      =   0
      ListStyle       =   1
      MultiSelect     =   1
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin MSForms.TextBox Text15 
      Height          =   300
      Index           =   0
      Left            =   1245
      TabIndex        =   0
      Top             =   1875
      Width           =   7170
      VariousPropertyBits=   671107099
      MaxLength       =   160
      Size            =   "12647;529"
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin MSForms.TextBox Text14 
      Height          =   615
      Left            =   915
      TabIndex        =   20
      Top             =   4800
      Width           =   7440
      VariousPropertyBits=   -1467987941
      MaxLength       =   2000
      ScrollBars      =   2
      Size            =   "13123;1085"
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin MSForms.TextBox Text15 
      Height          =   300
      Index           =   1
      Left            =   1245
      TabIndex        =   1
      Top             =   2115
      Width           =   7170
      VariousPropertyBits=   671107099
      MaxLength       =   250
      Size            =   "12647;529"
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin MSForms.TextBox Text15 
      Height          =   300
      Index           =   2
      Left            =   1245
      TabIndex        =   2
      Top             =   2355
      Width           =   7170
      VariousPropertyBits=   671107099
      MaxLength       =   160
      Size            =   "12647;529"
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin VB.Label lblCP113 
      AutoSize        =   -1  'True
      Caption         =   "工作時數:"
      Height          =   180
      Index           =   18
      Left            =   4380
      TabIndex        =   74
      Top             =   2700
      Width           =   765
   End
   Begin VB.Label lblPayToday 
      AutoSize        =   -1  'True
      Caption         =   "電子送件是否當日扣款:         (Y/N)"
      Height          =   180
      Left            =   5715
      TabIndex        =   73
      Top             =   4500
      Width           =   2655
   End
   Begin VB.Label Label20 
      AutoSize        =   -1  'True
      Caption         =   "是否電子送件:         (Y:是)"
      Height          =   180
      Index           =   2
      Left            =   6435
      TabIndex        =   72
      Top             =   4170
      Width           =   1995
   End
   Begin VB.Label Label11 
      AutoSize        =   -1  'True
      Caption         =   "催審期限:"
      Height          =   180
      Left            =   120
      TabIndex        =   70
      Top             =   5475
      Width           =   765
   End
   Begin VB.Label lblCaseFee 
      Appearance      =   0  '平面
      AutoSize        =   -1  'True
      Caption         =   "@"
      BeginProperty Font 
         Name            =   "Webdings"
         Size            =   12
         Charset         =   2
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H000000C0&
      Height          =   285
      Left            =   1905
      TabIndex        =   22
      Tag             =   "Y"
      Top             =   5400
      Width           =   255
   End
   Begin VB.Label Label20 
      AutoSize        =   -1  'True
      Caption         =   "發文日:"
      Height          =   180
      Index           =   0
      Left            =   120
      TabIndex        =   69
      Top             =   2700
      Width           =   585
   End
   Begin VB.Label lblCP84 
      AutoSize        =   -1  'True
      Caption         =   "發文規費:"
      Height          =   180
      Left            =   2250
      TabIndex        =   68
      Top             =   2700
      Width           =   765
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      Caption         =   "受讓人5:"
      Height          =   180
      Index           =   5
      Left            =   120
      TabIndex        =   67
      Top             =   4545
      Width           =   675
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      Caption         =   "受讓人4:"
      Height          =   180
      Index           =   4
      Left            =   120
      TabIndex        =   66
      Top             =   4245
      Width           =   675
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      Caption         =   "受讓人3:"
      Height          =   180
      Index           =   3
      Left            =   120
      TabIndex        =   65
      Top             =   3945
      Width           =   675
   End
   Begin MSForms.Label Label3 
      Height          =   180
      Index           =   5
      Left            =   2475
      TabIndex        =   64
      Top             =   4545
      Width           =   3120
      VariousPropertyBits=   27
      Caption         =   "Label3"
      Size            =   "14499;317"
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin MSForms.Label Label3 
      Height          =   180
      Index           =   4
      Left            =   2475
      TabIndex        =   63
      Top             =   4245
      Width           =   3120
      VariousPropertyBits=   27
      Caption         =   "Label3"
      Size            =   "14499;317"
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin MSForms.Label Label3 
      Height          =   180
      Index           =   3
      Left            =   2475
      TabIndex        =   62
      Top             =   3945
      Width           =   3120
      VariousPropertyBits=   27
      Caption         =   "Label3"
      Size            =   "14499;317"
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin MSForms.Label Label3 
      Height          =   180
      Index           =   2
      Left            =   2475
      TabIndex        =   61
      Top             =   3645
      Width           =   3120
      VariousPropertyBits=   27
      Caption         =   "Label3"
      Size            =   "14499;317"
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin MSForms.Label Label3 
      Height          =   180
      Index           =   1
      Left            =   2475
      TabIndex        =   60
      Top             =   3345
      Width           =   3120
      VariousPropertyBits=   27
      Caption         =   "Label3"
      Size            =   "14499;317"
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      Caption         =   "受讓人2:"
      Height          =   180
      Index           =   1
      Left            =   120
      TabIndex        =   59
      Top             =   3645
      Width           =   675
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      Caption         =   "受讓人1:"
      Height          =   180
      Index           =   2
      Left            =   120
      TabIndex        =   58
      Top             =   3345
      Width           =   675
   End
   Begin VB.Label lblNameAgent 
      AutoSize        =   -1  'True
      Caption         =   "出名代理人:"
      Height          =   180
      Left            =   5892
      TabIndex        =   57
      Top             =   2736
      Width           =   948
   End
   Begin VB.Label Label21 
      AutoSize        =   -1  'True
      Caption         =   "代理人:"
      Height          =   180
      Index           =   1
      Left            =   120
      TabIndex        =   56
      Top             =   3030
      Width           =   585
   End
   Begin MSForms.Label Label2 
      Height          =   180
      Index           =   14
      Left            =   2490
      TabIndex        =   55
      Top             =   3030
      Width           =   3090
      VariousPropertyBits=   27
      Caption         =   "Label2"
      Size            =   "14499;317"
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin VB.Label Label8 
      AutoSize        =   -1  'True
      Caption         =   "案件名稱(日):"
      Height          =   180
      Left            =   120
      TabIndex        =   54
      Top             =   2370
      Width           =   1065
   End
   Begin VB.Label Label31 
      AutoSize        =   -1  'True
      Caption         =   "案件名稱(英):"
      Height          =   180
      Index           =   1
      Left            =   120
      TabIndex        =   53
      Top             =   2130
      Width           =   1065
   End
   Begin VB.Label Label4 
      AutoSize        =   -1  'True
      Caption         =   "案件名稱(中):"
      Height          =   180
      Left            =   120
      TabIndex        =   52
      Top             =   1890
      Width           =   1065
   End
   Begin VB.Line Line1 
      BorderColor     =   &H00FFFFFF&
      BorderWidth     =   2
      Index           =   0
      X1              =   120
      X2              =   8460
      Y1              =   1812
      Y2              =   1812
   End
   Begin VB.Line Line1 
      BorderColor     =   &H00808080&
      BorderStyle     =   6  '內實線
      Index           =   1
      X1              =   120
      X2              =   8460
      Y1              =   1776
      Y2              =   1776
   End
   Begin VB.Label Label21 
      AutoSize        =   -1  'True
      Caption         =   "承辦人:"
      Height          =   180
      Index           =   0
      Left            =   3915
      TabIndex        =   51
      Top             =   1560
      Width           =   585
   End
   Begin VB.Label Label31 
      AutoSize        =   -1  'True
      Caption         =   "進度備註:"
      Height          =   180
      Index           =   0
      Left            =   120
      TabIndex        =   50
      Top             =   4830
      Width           =   765
   End
   Begin MSForms.Label Label2 
      Height          =   180
      Index           =   7
      Left            =   960
      TabIndex        =   49
      Top             =   1560
      Width           =   2910
      VariousPropertyBits=   27
      Caption         =   "Label2"
      Size            =   "14499;317"
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin MSForms.Label Label2 
      Height          =   180
      Index           =   6
      Left            =   4755
      TabIndex        =   48
      Top             =   1335
      Width           =   3690
      VariousPropertyBits=   27
      Caption         =   "Label2"
      Size            =   "14499;317"
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin MSForms.Label Label2 
      Height          =   180
      Index           =   5
      Left            =   960
      TabIndex        =   47
      Top             =   1335
      Width           =   2910
      VariousPropertyBits=   27
      Caption         =   "Label2"
      Size            =   "14499;317"
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin MSForms.Label Label2 
      Height          =   180
      Index           =   4
      Left            =   4755
      TabIndex        =   46
      Top             =   1110
      Width           =   3690
      VariousPropertyBits=   27
      Caption         =   "Label2"
      Size            =   "14499;317"
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin MSForms.Label Label2 
      Height          =   180
      Index           =   3
      Left            =   960
      TabIndex        =   45
      Top             =   1110
      Width           =   2910
      VariousPropertyBits=   27
      Caption         =   "Label2"
      Size            =   "14499;317"
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin MSForms.Label Label2 
      Height          =   180
      Index           =   2
      Left            =   4755
      TabIndex        =   44
      Top             =   765
      Width           =   1140
      VariousPropertyBits=   27
      Caption         =   "Label2"
      Size            =   "14499;317"
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin MSForms.Label Label2 
      Height          =   180
      Index           =   1
      Left            =   4755
      TabIndex        =   43
      Top             =   540
      Width           =   1140
      VariousPropertyBits=   27
      Caption         =   "Label2"
      Size            =   "14499;317"
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin MSForms.Label Label2 
      Height          =   180
      Index           =   0
      Left            =   960
      TabIndex        =   42
      Top             =   540
      Width           =   1950
      VariousPropertyBits=   27
      Caption         =   "Label2"
      Size            =   "14499;317"
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin VB.Label Label7 
      AutoSize        =   -1  'True
      Caption         =   "收文號:"
      Height          =   180
      Left            =   120
      TabIndex        =   41
      Top             =   540
      Width           =   585
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      Caption         =   "智權人員:"
      Height          =   180
      Index           =   0
      Left            =   3915
      TabIndex        =   40
      Top             =   540
      Width           =   765
   End
   Begin VB.Label Label6 
      AutoSize        =   -1  'True
      Caption         =   "本所案號:"
      Height          =   180
      Left            =   120
      TabIndex        =   39
      Top             =   765
      Width           =   765
   End
   Begin VB.Label Label9 
      AutoSize        =   -1  'True
      Caption         =   "本所期限:"
      Height          =   180
      Left            =   3915
      TabIndex        =   38
      Top             =   765
      Width           =   765
   End
   Begin VB.Label Label5 
      AutoSize        =   -1  'True
      Caption         =   "申請人1:"
      Height          =   180
      Left            =   120
      TabIndex        =   37
      Top             =   1110
      Width           =   675
   End
   Begin VB.Label Label10 
      AutoSize        =   -1  'True
      Caption         =   "申請人2:"
      Height          =   180
      Left            =   3915
      TabIndex        =   36
      Top             =   1110
      Width           =   675
   End
   Begin VB.Label Label12 
      AutoSize        =   -1  'True
      Caption         =   "申請人3:"
      Height          =   180
      Left            =   120
      TabIndex        =   35
      Top             =   1335
      Width           =   675
   End
   Begin VB.Label Label14 
      AutoSize        =   -1  'True
      Caption         =   "申請人4:"
      Height          =   180
      Left            =   3915
      TabIndex        =   34
      Top             =   1335
      Width           =   675
   End
   Begin VB.Label Label16 
      AutoSize        =   -1  'True
      Caption         =   "申請人5:"
      Height          =   180
      Left            =   120
      TabIndex        =   33
      Top             =   1560
      Width           =   675
   End
   Begin MSForms.Label Label2 
      Height          =   180
      Index           =   9
      Left            =   4755
      TabIndex        =   32
      Top             =   1560
      Width           =   1140
      VariousPropertyBits=   27
      Caption         =   "Label2"
      Size            =   "14499;317"
      FontName        =   "新細明體-ExtB"
      FontHeight      =   180
      FontCharSet     =   136
      FontPitchAndFamily=   34
   End
   Begin VB.Label lblCaseFees 
      BackColor       =   &H80000010&
      Height          =   255
      Left            =   1950
      TabIndex        =   71
      Top             =   5460
      Width           =   255
   End
End
Attribute VB_Name = "frm040104_c"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'Memo by Morgan 2022/12/28 改成Form2.0 (Text15,Text14,lstNameAgent,Label2,Label3)
'Memo By Morgan 2012/12/11 智權人員欄已修改
'2010/12/3 memo by sonia 員工編號欄已修改
'Memo by Morgan2010/8/10 日期欄已修改
'整理 by Morgan 2005/7/15
Option Explicit

Dim strReceiveNo As String

'Modify by Morgan 2005/7/15 改用動態陣列
'Dim pa(1 To T_PA) As String, cp(T_CP) As String
Dim pa() As String, cp() As String

Dim intWhere As Integer
'Add By Cheng 2003/10/06
Dim m_blnClkChgEvnBtn As Boolean '是否按下變更事項按鈕
Dim m_DiscType As String   '減免身分
Dim m_bolAdd919 As Boolean '自動收文減免退費並發文
Dim m_bolAddNP As Boolean '是否新增下一程序
Dim m_strNP07 As String, m_strNP08 As String, m_strNP09 As String '年費案件性質,所限,法限
Dim m_CP09s As String, m_CP123s As String 'Add by Morgan 2009/3/23 收文號,是否算發文室案件
Dim m_CP130 As String 'Add by Morgan 2009/4/28 發文-主管機關
Dim bolSetPA165 As Boolean 'Added by Morgan 2021/2/5
Dim m_bolFMP As Boolean 'Added by Lydia 2023/06/20 是否為FMP案
Dim m_bolFMP2 As Boolean 'Added by Lydia 2023/06/20 是否為寰華

Private Function Process(Index As Integer) As Boolean
Dim strTmp As String
Dim strAnoPA() As String, strCaseNo As String, bolIsExistsCP10 As Boolean, bolIsDualApply As Boolean 'Add By Sindy 2014/7/8
Dim bolAnoDated As Boolean 'Added by Morgan 2015/6/11
'Added by Lydia 2019/12/05
Dim strFilePath As String '記錄智慧局收文文號
Dim strNewCP64 As String '保留進度備註
Dim bolUp As Boolean 'Added by Lydia 2020/03/23 是否需要上傳檔案到卷宗區

   'Add By Cheng 2002/03/08
   '檢查輸入資料的完整性
   If CheckDataIntegrity = False Then Exit Function
   'Add By Cheng 2002/05/22
   '重新檢查欄位有效性
   If TxtValidate = False Then Exit Function
   
   'Add by Morgan 2007/10/26 專利權讓與須檢查是否有年費期限(聯合案不必)
   m_bolAddNP = False
   If cp(10) = 專利權讓與 And Not (pa(9) = "000" And Len(pa(11)) > 9) Then
      If PUB_708FeeCheck(pa, m_strNP07, m_strNP08, m_strNP09) = False Then
          If Val(m_strNP09) < Val(strSrvDate(1)) Then
            If MsgBox("本案計算出之下次繳費日為【" & ChangeWStringToTDateString(m_strNP09) & "】已過期，是否要繼續?", vbYesNo) = vbNo Then
               Exit Function
            End If
         ElseIf Val(m_strNP09) < Val(DBDATE(pa(25))) Then
            If MsgBox("本案下次繳費日為【" & ChangeWStringToTDateString(m_strNP09) & "】但目前無年費期限，存檔時是否要一併新增？", vbYesNo) = vbYes Then
               m_bolAddNP = True
            End If
         End If
      End If
   End If
   'end 2007/10/26
   
    strNewCP64 = Text14.Text 'Added by Lydia 2019/12/05 保留進度備註
    
   'Add by Morgan 2009/3/23 設定是否算發文室案件
   If pa(9) = 台灣國家代號 Then
   
      'Add by Morgan 2015/7/7 電子送件
      If txtCP118 = "Y" Then
         m_CP123s = ""
         'Added by Morgan 2016/5/16 電子送件也要記錄主管機關
         If ModifyDispatchCp130(cp(9), m_CP09s, m_CP123s, m_CP130, Text9, , True) = False Then
            Exit Function
         End If
         'end 2016/5/16
         strExc(0) = InputBox("請輸入智慧局收文文號!!")
         If strExc(0) = "" Then
            Exit Function
         Else
            'Text14 = "智慧局收文文號:" & strExc(0) & ";" & Text14 'Remove by Lydia 2020/04/08 重複
            'Modified by Lydia 2019/12/05
            'Text14 = "智慧局收文文號:" & strExc(0) & ";" & Text14
            strFilePath = strExc(0)  '記錄智慧局收文文號
            strNewCP64 = "智慧局收文文號:" & strExc(0) & ";" & Text14 '保留進度備註
            'end 2019/12/05
         End If
         'end 2012/12/26
         
      Else
      'end 2015/7/7
      
         'Add by Morgan 2009/4/28
         If ModifyDispatchCp130(cp(9), m_CP09s, m_CP123s, m_CP130, Text9) = False Then
            Exit Function
         End If
         If m_CP123s = "Y" Then
         'end 2009/4/28
            'modify by sonia 2014/6/23 加傳發文規費, P-108903
            If ModifyDispatch(cp(9), m_CP09s, m_CP123s, txtCP84, Text9) = False Then
                Exit Function
            End If
         End If
         
      End If 'Add by Morgan 2015/7/7
      
      'Add by Amy 2014/10/14 P台灣案發文控制
      If P台灣案電子化啟用日 <= Val(strSrvDate(1)) Then
        If pa(1) = "P" And cp(9) < "C" Then
            If cp(9) < "B" Then
                'A類一定要有接洽單才可發文
                'Modify by Amy 2014/11/27 取消ChkOneDayHasCP27判斷,接洽單改檢查,因考慮可能同時發文其他案件性質情形
                'If PUB_CheckPDF2(cp(9), 0, True, strExc(0)) = False And ChkOneDayHasCP27(pa(1), pa(2), pa(3), pa(4), cp(5) + 19110000) = False Then
                If PUB_CheckPDF3(Text1, Text2, Text3, Text4) = False Then
                    Exit Function
                End If
            End If
            'AB類申請書確認檢查,符合條件才可發文
            'Modified by Morgan 2015/3/17
            'If PUB_GetST03(cp(14)) = "P12" And Left(m_CP123s, 1) = "Y" And PUB_CheckPDF2(cp(9), 1, True, strExc(0)) = False Then
            If PUB_GetST03(cp(14)) = "P12" And Left(m_CP123s, 1) = "Y" Then
               If PUB_CheckPDF2(cp(9), 1, True, strExc(0)) = False Then
            'end 2015/3/17
                  MsgBox "無申請書PDF檔 ,不可發文!", vbInformation
                  Exit Function
               End If 'Added by Morgan 2015/3/17
            End If
        End If
      End If
      'end 2014/10/14
   Else
      'Added by Morgan 2016/6/29 非臺灣案電子化
      If 內專全面電子化啟用日 <= Val(strSrvDate(1)) And Left(Pub_StrUserSt03, 1) <> "F" Then
         If cp(9) < "B" And Left(cp(12), 1) <> "F" Then
             If PUB_CheckPDF3(Text1, Text2, Text3, Text4) = False Then
                 Exit Function
             End If
         End If
      End If
      'end 2016/6/29
   
      'Add by Morgan 2009/6/1
      If PUB_ChkFileNP(cp(9), "997,998") Then
         MsgBox "下一程序已有提申或收達期限，不可發文！"
         Exit Function
      End If
   End If
   
   'Add By Sindy 2014/7/8
   ReDim Preserve strAnoPA(1 To TF_PA) As String
   'Modified by Morgan 2015/6/11
   'bolIsDualApply = PUB_IsDualApplyCom(pa, strAnoPA, strCaseNo, , , , "701,708", bolIsExistsCP10)
   bolIsDualApply = PUB_IsDualApplyCom(pa, strAnoPA, strCaseNo, , , , "701,708", bolIsExistsCP10, DBDATE(Text9), bolAnoDated)
   If bolIsDualApply = True Then
      If bolIsExistsCP10 = False Then
         If MsgBox("本案為一案兩請案件，欲辦理讓與須發明及新型兩案同時辦理！不可發文！" & vbCrLf & _
                   "是否發E-Mail通知智權同仁？", vbYesNo + vbDefaultButton2) = vbYes Then
            '發Mail
            PUB_SendMail strUserNum, cp(13), "", _
                         pa(1) & "-" & pa(2) & IIf(pa(3) & pa(4) = "000", "", "-" & pa(3) & "-" & pa(4)) & "為一案兩請案件，欲辦理讓與須發明及新型兩案同時辦理", _
                         "本所案號：" & pa(1) & "-" & pa(2) & IIf(pa(3) & pa(4) = "000", "", "-" & pa(3) & "-" & pa(4)) & vbCrLf & _
                         "案件名稱：" & Trim(Text15(0)) & Trim(Text15(1)) & Trim(Text15(2)) & vbCrLf & _
                         "申 請 人：" & Label2(3) & vbCrLf & _
                         "一案兩請另一案：" & strCaseNo & vbCrLf & vbCrLf & vbCrLf & _
                         "請再補收文！"
         End If
         Exit Function
      End If
   End If
   '2014/7/8 END
   
    'Added by Lydia 2019/12/05 檢查是否有電子送件的檔案
    bolUp = False 'Added by Lydia 2020/03/23
    If txtCP118.Text = "Y" And strFilePath <> "" And pa(9) = "000" Then
        strExc(1) = cp(82)
        If Val(cp(82)) > 0 Then
            If MsgBox("重新發文是否上傳檔案到卷宗區？", vbInformation + vbYesNo + vbDefaultButton1) = vbYes Then
                 strExc(1) = ""
            End If
        End If
        If Val(strExc(1)) = 0 Then
           'Modified by Lydia 2020/03/23 改成先判斷是否上傳檔案; ex.P-124220發明申請 因為上傳檔案在FormSave前,所以沒抓到出名代理,造成無POA檔仍設電子檔案齊備CP121=Y
           'If Pub_AutoEsetToCppByP(True, pa(1), pa(2), pa(3), pa(4), pa(8), Label2(0).Caption, cp(10), strFilePath, Text9.Text) = False Then
           If Pub_AutoEsetToCppByP(True, pa(1), pa(2), pa(3), pa(4), pa(8), "", cp(10), strFilePath, Text9.Text) = False Then
                 Exit Function
           End If
           bolUp = True 'Added by Lydia 2020/04/08
        End If
    End If
   
   If pa(9) = 台灣國家代號 Then 'Added by Lydia 2020/01/17 限台灣案
       Text14.Text = strNewCP64 '檢查完畢，更新備註欄位
   End If
   'end 2019/12/05
   
   If FormSave = False Then MsgBox "存檔失敗，請洽系統管理員 !", vbCritical:  Exit Function
   
   'Move by Lydia 2020/04/07 移到最下方
   
   Process = True
   
   'Add by Morgan 2005/11/21
   If m_bolAdd919 = True Then
      If MsgBox("原申請人有預繳年費且新申請人符合減免資格，是否自動收文【減免退費】並發文？", vbYesNo + vbDefaultButton2) = vbYes Then
         AddNewCP
      End If
   End If
   
   'Add by Morgan 2008/2/20 檢查代理人Email(需考慮可能為FF案件)
   PUB_CheckEMail Combo2
   PUB_CheckEMail pa(75), pa(144)
   If pa(145) <> "" Then
      PUB_CheckEMail pa(75), pa(145)
   End If
   'end 2008/2/20
   
   'Add by Morgan 2007/6/14
   If pa(9) = "000" Then
      PUB_ReAsignInform pa(1), pa(2), pa(3), pa(4), strReceiveNo
   End If
   
   '2012/7/23 add by sonia
   '台灣案發文規費與收文規費不符時,mail給智權人員
   If txtCP84.Enabled = True And pa(9) = "000" And Val(Me.txtCP84.Text) <> Val(cp(17)) Then
      '2013/7/2 modify by sonia 改用共用module
      'Modified by Morgan 2015/7/7 +傳strCP118參數
      PUB_ChkOfficialFee cp(9), Me.txtCP84.Text, IIf(txtCP118 = "Y", "A", "")
   End If
   '2012/7/23 end
   
   If pa(9) = 台灣國家代號 Then '通知函
      'modify by sonia 2019/6/3 +大->台定稿P-122734
      'strTmp = "00"
      If PUB_CheckCuNation(pa(26), Text1, Text2, Text3, Text4) = "1" Then
         'Modified by Morgan 2023/2/20 內容相同，改用一般大->台定稿 09
         'strTmp = "02"
         strTmp = "09"
         'end 2023/2/20
      Else
         'Modified by Morgan 2023/2/20 共用定稿:讓與,專利權讓與,合併,繼承
         'strTmp = "00"
         strTmp = "10"
         'end 2023/2/20
      End If
      'end 2019/6/3
   ElseIf pa(9) <> 台灣國家代號 Then
      strTmp = "01"
   End If
   EndLetter "02", strReceiveNo, strTmp, strUserNum
   'Modify by Amy 2014/08/25 +傳strLetterRecNo
   NowPrint strReceiveNo, "02", strTmp, False, strUserNum, 0, , , , , , , , , , , , strReceiveNo
   
   'Add By Sindy 2014/7/8
   'Modified by Morgan 2015/6/11
   'If bolIsDualApply = True And bolIsExistsCP10 = True Then
   If bolIsDualApply = True And bolIsExistsCP10 = True And bolAnoDated = False Then
      MsgBox "尚有一案兩請" & strCaseNo & "尚未發文！", vbExclamation + vbOKOnly
   End If
   '2014/7/8 END
   
   'Added by Lydia 2020/03/23 是否可以上傳檔案,前面已判斷
   'Move by Lydia 2020/04/07 從中間移下來
   If bolUp = True Then
      If Pub_AutoEsetToCppByP(False, pa(1), pa(2), pa(3), pa(4), pa(8), Label2(0).Caption, cp(10), strFilePath, Text9.Text) = False Then
           Exit Function
      End If
   End If
   'end 2020/03/23
   
End Function

Private Sub cmdok_Click(Index As Integer)
   ' 設定滑鼠游標為等待狀態
   Screen.MousePointer = vbHourglass
   Select Case Index
      Case 0, 3
         'Modify by Morgan 2010/2/10 改呼叫函數方式以便鎖定按鍵
         cmdOK(Index).Enabled = False
         If Not Process(Index) Then
            cmdOK(Index).Enabled = True
         Else
            If Index = 0 Then
'               '若有未發文資料顯示警告
'               PUB_GetCPunIssueDatas "" & Me.Text1.Text & "-" & Me.Text2.Text & "-" & IIf(Len("" & Me.Text3.Text) <= 0, "0", Me.Text3.Text) & "-" & IIf(Len("" & Me.Text4.Text) <= 0, "00", Me.Text4.Text)
'               ' 90.07.11 modify by louis (回第一個畫面清除)
'               frm040104_1.Show
'               frm040104_1.Clear
               '若有未發文資料顯示警告
               If PUB_GetCPunIssueDatas("" & Me.Text1.Text & "-" & Me.Text2.Text & "-" & IIf(Len("" & Me.Text3.Text) <= 0, "0", Me.Text3.Text) & "-" & IIf(Len("" & Me.Text4.Text) <= 0, "00", Me.Text4.Text)) Then
                  'Add By Sindy 2013/5/20
                  If frm040104_1.bolIsEMPFlow = True Then
                     frm090202_4.QueryData
                  End If
                  '2013/5/20 End
                  frm040104_1.Show
                  frm040104_1.Command1_Click
               Else
                  'Add By Sindy 2013/5/20
                  If frm040104_1.bolIsEMPFlow = True Then
                     Unload frm040104_1
                     frm090202_4.Show
                     frm090202_4.QueryData
                  Else
                  '2013/5/20 End
                     frm040104_1.Show
                     ' 90.07.11 modify by louis (回第一個畫面清除)
                     frm040104_1.Clear
                  End If
               End If
            Else
               ' 90.07.11 modify by louis (回第一個畫面重新查詢)
               'Add By Sindy 2013/5/20
               If frm040104_1.bolIsEMPFlow = True Then
                  frm090202_4.QueryData
               End If
               '2013/5/20 End
               frm040104_1.Show
               frm040104_1.ReQuery
            End If
            Unload Me
         End If
      Case 1
         'Add By Sindy 2013/5/20
         If frm040104_1.bolIsEMPFlow = True Then
            Unload frm040104_1
            frm090202_4.Show
            frm090202_4.QueryData
         Else
         '2013/5/20 End
            frm040104_1.Show
         End If
         Unload Me
      Case 2
         Me.Hide
         Set frm06010303_1.oParent = Me 'Add by Morgan 2011/10/5
         frm06010303_1.LoadMe strReceiveNo, pa(1), pa(2), pa(3), pa(4), 46
         frm06010303_1.Text41 = "N"
         frm06010303_1.Caption = "內專發文-變更事項"
        m_blnClkChgEvnBtn = True
   End Select
   ' 設定滑鼠游標為預設
   Screen.MousePointer = vbDefault
End Sub

Private Function FormSave() As Boolean
   Dim i As Integer, strTxt(1 To 20) As String, ii As Integer
   'Add By Cheng 2003/03/04
   Dim StrSQLa As String
   'Add by Morgan 2005/11/17
   Dim iIdx As Integer
   Dim stUpdateSQL1 As String '
   Dim stUpdateSQL2 As String
   Dim lngNP22 As Long
   Dim stCP118 As String, stCP152 As String 'Added by Morgan 2015/7/7
   Dim stNP23 As String 'Added by Lydia 2025/10/29
   
'Add By Cheng 2002/11/06
On Error GoTo ErrorHandler
FormSave = True
cnnConnection.BeginTrans

   'Added by Morgan 2013/6/7 自 lstNameAgent_Validate 移來,否則若觸發 Form_Activate 事件會跑 ReadPatent 導致 cp(110) 被清除
   cp(110) = ""
   If lstNameAgent.Visible = True Then
      For ii = 0 To lstNameAgent.ListCount - 1
         If lstNameAgent.Selected(ii) = True Then
            'modify by sonia 2016/10/7 員工編號已可非數字需做轉換
            'cp(110) = cp(110) & "," & lstNameAgent.ItemData(ii)
            'Modified by Morgan 2021/12/15f Forms2.0 改用模組
            'cp(110) = cp(110) & "," & PUB_Num2Id(lstNameAgent.ItemData(ii))
            'Modified by Morgan 2021/12/15f Forms2.0 改用模組
            'cp(110) = cp(110) & "," & PUB_Num2Id(lstNameAgent.ItemData(ii))
            cp(110) = cp(110) & "," & PUB_GetItemData(lstNameAgent.Tag, ii)
         End If
      Next
      If Left(cp(110), 1) = "," Then cp(110) = Mid(cp(110), 2)
   End If
   'end 2013/6/7

   'Add by Morgan 2005/11/21
   '設定客戶減免身分
   If pa(9) = "000" Then
      For iIdx = 1 To 5
         If txtAD(iIdx).Enabled = True Then
            '身分有變更才要做
            If txtAD(iIdx).Tag <> txtAD(iIdx).Text Then
               '不可減免
               If txtAD(iIdx).Text = "N" Then
                  strSql = PUB_GetADSQL(txtAppNew(iIdx), pa(9), "N")
               '自然人,學校
               ElseIf (txtAD(iIdx).Text = "1" Or txtAD(iIdx).Text = "2") Then
                  strSql = PUB_GetADSQL(txtAppNew(iIdx), pa(9), "Y", txtAD(iIdx).Text)
               '公司
               Else
                  '原來沒有減免資料或不可減免
                  If txtAD(iIdx).Tag = "" Or txtAD(iIdx).Tag = "N" Then
                     strSql = PUB_GetADSQL(txtAppNew(iIdx), pa(9), "Y", txtAD(iIdx).Text, pa(1), pa(2), pa(3), pa(4))
                  '修改減免身分別
                  Else
                     strSql = PUB_GetADSQL(txtAppNew(iIdx), pa(9), "Y", txtAD(iIdx).Text)
                  End If
               End If
               cnnConnection.Execute strSql
            End If
         End If
      Next
   End If
   

   'Add By Cheng 2002/07/05
   If Combo2 <> "" Then
      'Modify by Morgan 2008/2/22
      'cp(44) = ChangeCustomerL(Combo2)
      intI = InStr(Combo2, "-")
      If intI > 0 Then
         cp(44) = Left(Combo2, intI - 1)
         cp(116) = Mid(Combo2, intI + 1)
      Else
         cp(44) = Combo2
         cp(116) = ""
      End If
      cp(44) = ChangeCustomerL(cp(44))
      'end 2008/2/22
      
      'edit by nickc 2007/02/02 不用 dll 了
      'If Not objPublicData.GetCaseThatCode(cp) Then cp(45) = ""
      If Not ClsPDGetCaseThatCode(cp) Then cp(45) = ""
   Else
      cp(44) = ""
      cp(116) = ""
      cp(45) = ""
   End If
 
   '受讓人補滿九碼
   'Modify by Morgan 2005/11/18
'   If IsEmptyText(Text6) = False Then
'      Text6 = ChangeCustomerL(Me.Text6.Text)
'   End If
   '若原申請人與讓與申請人不同時, 才更新讓與人(避免讓與人欄位被蓋掉)
   '受讓人1
   cp(56) = ChangeCustomerL(txtAppNew(1))
   '讓與申請人1
   stUpdateSQL1 = ",CP56=" & CNULL(cp(56))
   If ChangeCustomerL(pa(26)) <> cp(56) Then
      '讓與人1
      stUpdateSQL1 = stUpdateSQL1 & ",CP55=" & CNULL(ChangeCustomerL(pa(26)))
      '新申請人1
      stUpdateSQL2 = ",PA26=" & CNULL(cp(56))
      '新申請人1地址
      'edit by nickc 2007/02/02 不用 dll 了
      'Call objPublicData.GetCustomerNameAndAddress(cp(56), strExc(0), strExc(1), strExc(2), strExc(3))
      Call ClsPDGetCustomerNameAndAddress(cp(56), strExc(0), strExc(1), strExc(2), strExc(3))
      stUpdateSQL2 = stUpdateSQL2 & ",PA" & Format(31) & "=" & CNULL(ChgSQL(Trim(strExc(1))))
      stUpdateSQL2 = stUpdateSQL2 & ",PA" & Format(36) & "=" & CNULL(ChgSQL(Trim(strExc(2))))
      stUpdateSQL2 = stUpdateSQL2 & ",PA" & Format(41) & "=" & CNULL(ChgSQL(Trim(strExc(3))))
   'Add by Morgan 2006/3/23
   '讓與人與受讓人相同時判斷CP有資料時才不更新
   Else
      '讓與人1
      stUpdateSQL1 = stUpdateSQL1 & ",CP55=NVL(CP55," & CNULL(ChangeCustomerL(pa(26))) & ")"
   '2006/3/23 end
   End If
      
   '受讓人2~5
   For iIdx = 2 To 5
      cp(87 + iIdx) = ChangeCustomerL(txtAppNew(iIdx))
      '讓與申請人
      stUpdateSQL1 = stUpdateSQL1 & ",CP" & Format(87 + iIdx) & "=" & CNULL(cp(87 + iIdx))
      '(原申請人與讓與申請人不同時, 才更新)
      If ChangeCustomerL(pa(25 + iIdx)) <> cp(87 + iIdx) Then
         '讓與人
         stUpdateSQL1 = stUpdateSQL1 & ",CP" & Format(91 + iIdx) & "=" & CNULL(ChangeCustomerL(pa(25 + iIdx)))
         '新申請人
         stUpdateSQL2 = stUpdateSQL2 & ",PA" & Format(25 + iIdx) & "=" & CNULL(cp(87 + iIdx))
         '新申請人地址
         If cp(87 + iIdx) <> "" Then
            'edit by nickc 2007/02/02 不用 dll 了
            'Call objPublicData.GetCustomerNameAndAddress(cp(87 + iIdx), strExc(0), strExc(1), strExc(2), strExc(3))
            Call ClsPDGetCustomerNameAndAddress(cp(87 + iIdx), strExc(0), strExc(1), strExc(2), strExc(3))
            stUpdateSQL2 = stUpdateSQL2 & ",PA" & Format(30 + iIdx) & "=" & CNULL(ChgSQL(Trim(strExc(1))))
            stUpdateSQL2 = stUpdateSQL2 & ",PA" & Format(35 + iIdx) & "=" & CNULL(ChgSQL(Trim(strExc(2))))
            stUpdateSQL2 = stUpdateSQL2 & ",PA" & Format(40 + iIdx) & "=" & CNULL(ChgSQL(Trim(strExc(3))))
         Else
            stUpdateSQL2 = stUpdateSQL2 & ",PA" & Format(30 + iIdx) & "=NULL"
            stUpdateSQL2 = stUpdateSQL2 & ",PA" & Format(35 + iIdx) & "=NULL"
            stUpdateSQL2 = stUpdateSQL2 & ",PA" & Format(40 + iIdx) & "=NULL"
         End If
      'Add by Morgan 2006/3/23
      '讓與人與受讓人相同時判斷CP有資料時才不更新
      Else
         '讓與人
         stUpdateSQL1 = stUpdateSQL1 & ",CP" & Format(91 + iIdx) & "=NVL(CP" & Format(91 + iIdx) & "," & CNULL(ChangeCustomerL(pa(25 + iIdx))) & ")"
      '2006/3/23 end
      End If
   Next
      
   'Added by Morgan 2015/7/7
   stCP118 = txtCP118
   stCP152 = ""
   If txtCP118 = "Y" And Val(txtCP84) > 0 And pa(9) = "000" Then
      If txtPayToday <> "" Then
         stCP118 = "A"
         If txtPayToday = "Y" Then
            stCP152 = CompWorkDay(2, DBDATE(Text9))
         Else
            stCP152 = CompWorkDay(3, DBDATE(Text9))
         End If
      End If
   End If
   'end 2015/7/7
   
   'Modify by Morgan 2004/8/11 加 cp84
   'Modify by Morgan 2005/7/15 加 cp22,cp110
   'Modify by Morgan 2005/11/18 加更新讓與人2~5
'   strTxt(1) = "UPDATE CASEPROGRESS SET CP27=" & TransDate(Text9, 2) & ",cp14=" & CNULL(Text10) & ",CP44=" & CNULL(ChangeCustomerL(Combo2)) & ",CP45=" & CNULL(ChgSQL(cp(45))) & "," & _
'      "cp64=" & CNULL(ChgSQL(Text14)) & ",cp56=" & CNULL(ChgSQL(ChangeCustomerL(Text6.Text))) & ", cp84=" & Format(Val(txtCP84.Text)) & _
'      ",cp22=" & CNULL(cp(22)) & ",cp110=" & CNULL(cp(110)) & stUpdateSQL1 & _
'      " where cp09='" & strReceiveNo & "'"
   'Modified by Morgan 2015/7/7 +CP118,CP152
   'Modified by Morgan 2016/5/19 +CP120 電子送件都要說明書電子檔--韻丞
   'Modified by Lydia 2021/05/25 +CP113工作時數
   'Modified by Morgan 2024/1/24 +判斷台灣案才要設定CP120
   strTxt(1) = "UPDATE CASEPROGRESS SET CP27=" & TransDate(Text9, 2) & ",cp14=" & CNULL(Text10) & ",CP44=" & CNULL(cp(44)) & ",CP116=" & CNULL(cp(116)) & ",CP45=" & CNULL(ChgSQL(cp(45))) & "," & _
      "cp64=" & CNULL(ChgSQL(Text14)) & ", cp84=" & Format(Val(txtCP84.Text)) & _
      ",cp22=" & CNULL(cp(22)) & ",cp110=" & CNULL(cp(110)) & stUpdateSQL1 & _
      ",cp118='" & stCP118 & "',cp120='" & IIf(pa(9) = "000", txtCP118, "") & "',cp152=" & CNULL(stCP152, True) & _
      ", cp113=" & CNULL(txtCP113, True) & _
      " where cp09='" & strReceiveNo & "'"
      
      
    'Add By Cheng 2002/11/06
    cnnConnection.Execute strTxt(1)
          
   'Modify by Morgan 2005/11/18 加更新申請人
'   strTxt(2) = "UPDATE PATENT SET PA05=" & CNULL(Text15(0)) & ",PA06=" & CNULL(ChgSQL(Text15(1))) & _
'      ",PA07=" & CNULL(Text15(2)) & " WHERE " & ChgPatent(pa(1) & pa(2) & pa(3) & pa(4))
   strTxt(2) = "UPDATE PATENT SET PA05=" & CNULL(Text15(0)) & ",PA06=" & CNULL(ChgSQL(Text15(1))) & _
      ",PA07=" & CNULL(Text15(2)) & stUpdateSQL2 & " WHERE " & ChgPatent(pa(1) & pa(2) & pa(3) & pa(4))
      
      
    'Add By Cheng 2002/11/06
    cnnConnection.Execute strTxt(2)
    
   'Added by Morgan 2021/2/5 設定為總委任書案件
   If bolSetPA165 = True Then
      strSql = "UPDATE PATENT SET PA165='Y' WHERE " & ChgPatent(pa(1) & pa(2) & pa(3) & pa(4))
      cnnConnection.Execute strSql, intI
   End If
   'end 2021/2/5
   
'Remove by Morgan 2005/11/18 併入上方程式
'    'Add By Cheng 2003/03/04
'    '若案件性質為讓與(701), 專利權讓與(708)
'    If cp(10) = 讓與 Or cp(10) = 專利權讓與 Then
'        '若有輸入受讓人
'        If Me.Text6.Text <> "" Then
'            '更新進度檔讓與人
'            '若原申請人與讓與申請人不同時, 才更新讓與人(避免讓與人欄位被蓋掉)
'            If ChangeCustomerL(pa(26)) <> ChangeCustomerL(Me.Text6.Text) Then
'                StrSQLa = "Update CaseProgress Set CP55='" & ChangeCustomerL(pa(26)) & "' Where CP09='" & strReceiveNo & "' "
'                cnnConnection.Execute StrSQLa
'            End If
'            '更新基本檔申請人及相關資料
'            StrSQLa = "Update Patent Set PA26='" & ChangeCustomerL(Me.Text6.Text) & "' " & _
'                            " ,PA31='" & ChgSQL(PUB_GetCustEachAdd(ChangeCustomerL(Me.Text6.Text), "1")) & "' " & _
'                            " ,PA36='" & ChgSQL(PUB_GetCustEachAdd(ChangeCustomerL(Me.Text6.Text), "2")) & "' " & _
'                            " ,PA41='" & ChgSQL(PUB_GetCustEachAdd(ChangeCustomerL(Me.Text6.Text), "3")) & "' " & _
'                            " ,PA27=NULL,PA28=NULL,PA29=NULL,PA30=NULL,PA32=NULL,PA33=NULL,PA34=NULL,PA35=NULL,PA37=NULL" & _
'                            " ,PA38=NULL,PA39=NULL,PA40=NULL,PA42=NULL,PA43=NULL,PA44=NULL,PA45=NULL" & _
'                            " Where " & ChgPatent(pa(1) & pa(2) & pa(3) & pa(4))
'            '92.8.13 END
'            cnnConnection.Execute StrSQLa
'        End If
'    End If
    
   i = 2
   'Modify by Morgan 2009/5/14 +提申天數 CF11
   strExc(0) = "SELECT CF23,CF11 FROM CASEFEE WHERE CF01='" & pa(1) & "' AND CF02='" & pa(9) & "' AND CF03='" & cp(10) & "'"
   intI = 1
   Set RsTemp = ClsLawReadRstMsg(intI, strExc(0)) 'edit by nickc 2007/02/05 不用 dll 了 objLawDll.ReadRstMsg(intI, strExc(0))
   If intI = 1 Then
   
'Modify by Morgan 2009/11/11 收達期限管控改呼叫公用函式
'      If Not IsNull(RsTemp.Fields(0)) Or RsTemp.Fields(0) <> 0 Then
'         i = 3
'            '若本所期限非工作天則抓最近的工作天
'         'edit by nickc 2007/02/02 不用 dll 了
'         'strTxt(i) = "INSERT INTO NEXTPROGRESS (NP01,NP02,NP03,NP04,NP05,NP07,NP08," & _
'            "NP09,NP10,NP22) VALUES ('" & strReceiveNo & "','" & pA(1) & "','" & pA(2) & _
'            "','" & pA(3) & "','" & pA(4) & "'," & 收達 & "," & _
'            PUB_GetWorkDay1(CompDate(2, rsTemp.Fields(0), TransDate(Text9, 2)), True) & "," & _
'            CompDate(2, rsTemp.Fields(0), TransDate(Text9, 2)) & ",'" & _
'            strUserNum & "'," & objPublicData.GetNextProgressNo & ")"
'         strTxt(i) = "INSERT INTO NEXTPROGRESS (NP01,NP02,NP03,NP04,NP05,NP07,NP08," & _
'            "NP09,NP10,NP22) VALUES ('" & strReceiveNo & "','" & pa(1) & "','" & pa(2) & _
'            "','" & pa(3) & "','" & pa(4) & "'," & 收達 & "," & _
'            PUB_GetWorkDay1(CompDate(2, RsTemp.Fields(0), TransDate(Text9, 2)), True) & "," & _
'            CompDate(2, RsTemp.Fields(0), TransDate(Text9, 2)) & ",'" & _
'            strUserNum & "'," & GetNextProgressNo & ")"
'        cnnConnection.Execute strTxt(i)
'      End If
   If pa(9) <> "000" Then PUB_SetArriveDate cp(9)
'end 2009/11/11

      'Add by Morgan 2009/6/1
      If Val("" & RsTemp.Fields(1)) > 0 Then
         If PUB_ChkFileNP(cp(9), "'995'") = False Then
            '一般提申
            strExc(1) = CompDate(2, Val("" & RsTemp.Fields(1)), DBDATE(Text9))
            strExc(2) = PUB_GetWorkDay1(strExc(1), True)
            strSql = " insert into nextprogress a (np01,np02,np03,np04,np05,np07,np08,np09,np10,np22)" & _
               " values('" & cp(9) & "','" & cp(1) & "','" & cp(2) & "','" & cp(3) & "','" & cp(4) & "','998'" & _
               "," & strExc(2) & "," & strExc(1) & ",'" & strUserNum & "',GETNP22)"
            cnnConnection.Execute strSql, intI
            
            '最終提申
            If cp(7) <> "" Then
               If PUB_ChkFileNP(cp(9), "'996'") = False Then
                  strExc(1) = DBDATE(cp(7))
                  strExc(2) = PUB_GetWorkDay1(strExc(1), True)
                  strSql = " insert into nextprogress a (np01,np02,np03,np04,np05,np07,np08,np09,np10,np22)" & _
                     " values('" & cp(9) & "','" & cp(1) & "','" & cp(2) & "','" & cp(3) & "','" & cp(4) & "','996'" & _
                     "," & strExc(2) & "," & strExc(1) & ",'" & strUserNum & "',GETNP22)"
                  cnnConnection.Execute strSql, intI
               End If
            End If
         End If
      End If
   End If
   'Add by Morgan 2007/1/30 專利的讓與及專利權讓與發文時,
   '同時更新下一程序非專業部掌控之案件性質(未續辦)的智權人員
   '2012/2/16 MODIFY BY SONIA 改智權人員
   'pub_ChgSalesTargetIsNp pa(1), pa(2), pa(3), pa(4), cp(13)
   pub_ChgSalesTargetIsNp pa(1), pa(2), pa(3), pa(4), PUB_GetAKindSalesNo(pa(1), pa(2), pa(3), pa(4))
   
   'Add by Morgan 2007/10/25 專利權讓與新增年費期限
   If m_bolAddNP = True Then
      'Added by Lydia 2025/10/29
      stNP23 = ""
      If m_bolFMP = False And strSrvDate(1) >= 內專本所約定期限啟用日 Then
          m_strNP08 = PUB_GetPOurDeadline(m_strNP09, pa(1), stNP23, pa(1), m_strNP07)
      End If
      'end 2025/10/29
      lngNP22 = GetNextProgressNo
      '2012/2/16 MODIFY BY SONIA 改智權人員
      'strSql = "INSERT INTO NEXTPROGRESS (NP01,NP02,NP03,NP04,NP05,NP07,NP08,NP09,NP10,NP22) " & _
         "VALUES ('" & strReceiveNo & "','" & pa(1) & "','" & pa(2) & "','" & pa(3) & "','" & pa(4) & "'" & _
         ",'" & m_strNP07 & "'," & m_strNP08 & "," & m_strNP09 & ",'" & cp(13) & "'," & lngNP22 & ")"
      'Modified by Lydia 2025/10/29 +NP23
      strSql = "INSERT INTO NEXTPROGRESS (NP01,NP02,NP03,NP04,NP05,NP07,NP08,NP09,NP10,NP22,NP23) " & _
         "VALUES ('" & strReceiveNo & "','" & pa(1) & "','" & pa(2) & "','" & pa(3) & "','" & pa(4) & "'" & _
         ",'" & m_strNP07 & "'," & m_strNP08 & "," & m_strNP09 & ",'" & PUB_GetAKindSalesNo(pa(1), pa(2), pa(3), pa(4)) & "'," & lngNP22 & "," & CNULL(stNP23, True) & ")"
      cnnConnection.Execute strSql, intI
   End If
   
   'Add by Morgan 2009/3/23
   If pa(9) = 台灣國家代號 Then
      PUB_UpdateDispatch m_CP09s, m_CP123s, m_CP130
      'Add by Amy 2014/09/09 for 台灣案電子化
      If P台灣案電子化啟用日 <= Val(strSrvDate(1)) Then
         If cp(9) < "C" Then
            'Modified by Morgan 2018/8/1
            'strExc(1) = PUB_GetLetterJudge(pa(1), cp(10), , , pa(1), pa(2), pa(3), pa(4))
            strExc(1) = PUB_GetLetterJudgeNew("1", pa(1), cp(10))
            'Modify by Amy 2015/02/13 此固定出客戶通知函,故不考慮未出客戶函 修改判斷條件
            'PUB_AddLetterProgress strReceiveNo, 1, True, strExc(1), False, pa(26), cp(10), pa(75), True
              '1.　電子送件有規費的有收據；無規費的無回執
              '2.非電子送件要計件的有回執；不計件的無回執
            'Mark by Amy 2015/03/06 回執改至PUB_UpdateLP19做
'            If cp(118) = "Y" Then
'                If Val(txtCP84) > 0 Then
'                    PUB_AddLetterProgress strReceiveNo, 1, True, strExc(1), False, pa(26), cp(10), pa(75), True
'                Else
'                    PUB_AddLetterProgress strReceiveNo, 0, True, strExc(1), False, pa(26), cp(10), pa(75), False
'                End If
'            Else
                If Left(m_CP123s, 1) = "Y" Then
                    PUB_AddLetterProgress strReceiveNo, 1, True, strExc(1), False, pa(26), cp(10), pa(75), True
                Else
                    PUB_AddLetterProgress strReceiveNo, 0, True, strExc(1), False, pa(26), cp(10), pa(75), False
                End If
'            End If
'            'end 2015/02/13
            'edn 2015/03/06
         End If
      End If
      'Add by Amy 2015/02/13 更新收據/回執設定
      'Modify by Amy 2015/03/06 +發文日參數
      PUB_UpdateLP19 cp(1), cp(2), cp(3), cp(4), m_CP09s, m_CP123s, Text9
      'end 2014/09/09
      
   'Added by Morgan 2016/5/26 非臺灣案電子化
   ElseIf Left(Pub_StrUserSt03, 1) <> "F" Then
      '客戶通知函
      If 內專全面電子化啟用日 <= Val(strSrvDate(1)) Then
         'Modified by Morgan 2018/8/1
         'strExc(1) = PUB_GetLetterJudge(pa(1), cp(10), , pa(9), pa(1), pa(2), pa(3), pa(4))
         strExc(1) = PUB_GetLetterJudgeNew("1", pa(1), cp(10), pa(9), , , IIf(Left(cp(12), 1) = "F", True, False))
         'Modified by Morgan 2024/3/22 不可傳入原申請人，直接讓函數抓目前案件申請人，否則E化設定會抓錯對象 Ex:P-131992專利權讓
         'PUB_AddLetterProgress strReceiveNo, 0, True, strExc(1), False, pa(26), cp(10), pa(75)
         PUB_AddLetterProgress strReceiveNo, 0, True, strExc(1), , , cp(10), pa(75)
         'end 2024/3/22
      End If
   'end 2016/5/26
   End If
   
   'Add by Morgan 2009/8/17
   If txtChkRltDate <> "" Then
      PUB_UpdateChkResultDate txtChkRltDate, cp, cp(9), cp(10), cp(43)
   End If
   
    cnnConnection.CommitTrans
    Exit Function
ErrorHandler:
    cnnConnection.RollbackTrans
    FormSave = False
End Function

Private Sub Combo2_Click()
'Add By Cheng 2002/07/05
Combo2_Validate False
End Sub

Private Sub Combo2_KeyPress(KeyAscii As Integer)
'Add By Cheng 2002/07/05
KeyAscii = UpperCase(KeyAscii)
End Sub

Private Sub Combo2_Validate(Cancel As Boolean)
   Dim strNo As String, iPos As Integer
   If Combo2.Text = "" Then
      If pa(9) <> 台灣國家代號 Then
         MsgBox "當申請國家非台灣時, 代理人欄不可為空白!!!", vbExclamation
         Cancel = True
         Exit Sub
      End If
      
   ElseIf Not ChgType(12) Then
      Cancel = True
      
   Else
      strNo = Combo2.Text
      
      'Add by Morgan 2008/2/22 加聯絡人判斷
      iPos = InStr(strNo, "-")
      If iPos > 0 Then
         strNo = Left(strNo, iPos - 1)
      End If
      'end 2008/2/22
      
      'Add by Morgan 2004/9/14 檢查客戶/代理人是否不再使用
      If PUB_CheckStatus(strNo) = False Then Cancel = True
   End If
End Sub

Private Sub Form_Activate()
    'Add By Cheng 2003/10/06
    '若有按下變更事項按鈕, 則重新讀取資料
    If m_blnClkChgEvnBtn = True Then
        ReadPatent
        Label2(0) = strReceiveNo
        m_blnClkChgEvnBtn = False
    End If
End Sub

Private Sub Form_Load()
   MoveFormToCenter Me
   intWhere = 國內
   With frm040104_1
      Text1 = .Text1
      Text2 = .Text2
      Text3 = .Text3
      Text4 = .Text4
      strReceiveNo = .Tag
   End With
   'Add by Morgan 2005/7/15
   ReDim pa(TF_PA)
   ReDim cp(TF_CP)
   
   ReadPatent
   
   cp(110) = "" '要清空,否則若重新發文會殘留前次發文資料,當新案有改出名人而本程序未改選將會造成不一致 Added by Morgan 2012/9/7
   
   'Add by Morgan 2005/7/15
   '台灣加出名代理人清單供勾選,原是否出名欄位不顯示
   lstNameAgent.Clear
   If pa(9) = "000" Then
      PUB_SetOurAgent lstNameAgent, pa(), cp(110), , True 'Modified by Morgan 2021/12/15 +傳入bForm2=True
      lstNameAgent.Visible = True
      lblNameAgent.Visible = True
   Else
      lstNameAgent.Visible = False
      lblNameAgent.Visible = False
      If PUB_ChkFileNP(cp(9), "'997','998'") Then MsgBox "下一程序已有一般提申或收達期限，若為重新發文時需要先刪除後才可作業！" 'Add by Morgan 2009/6/1
   End If
   '2005/7/14 END
   
   Label2(0) = strReceiveNo
   m_blnClkChgEvnBtn = False
   
End Sub

Private Sub Form_Unload(Cancel As Integer)
   'Set frm040104_c = Nothing 'Removed by Morgan 2021/12/15 form2.0會有問題，改在呼叫時清除記憶體變數
End Sub

Private Sub ReadPatent()
Dim Lbl As Object, txt As TextBox, i As Integer
Dim m_Fee As String         '銷帳服務費 2012/8/1 add by sonia
Dim m_Official As String    '銷帳規費   2012/8/1 add by sonia
   
   For Each Lbl In Label2
      Lbl = ""
   Next
   pa(1) = Text1
   pa(2) = Text2
   pa(3) = Text3
   pa(4) = Text4
   Select Case pa(1)
      Case "P"
         If ClsPDReadPatentDatabase(pa(), intWhere) Then 'edit by nickc 2007/02/02 不用 dll 了 If objPublicData.ReadPatentDatabase(pA(), intWhere) Then
            For i = 3 To 7
               If pa(i + 23) <> "" Then ChgType (i)
            Next
            
            Text15(0) = pa(5)
            Text15(1) = pa(6)
            Text15(2) = pa(7)

         End If
      Case "FG"
         If ClsPDReadServicePracticeDatabase(pa(), intWhere) Then 'edit by nickc 2007/02/02 不用 dll 了 If objPublicData.ReadPatentDatabase(pA(), intWhere) Then
            
            Text15(0) = pa(5)
            Text15(1) = pa(6)
            Text15(2) = pa(7)
            
         End If
   End Select
   
   'Add By Cheng 2002/07/05
   cp(9) = strReceiveNo
   If PUB_ReadCaseProgressDatabase(cp(), intWhere) Then
   End If
      
   'Modify by Morgan 2005/11/17 加cp89,cp90,cp91,cp92
   strExc(0) = "select cp13,st02,cp06,cp27,cp14,cp64,cp56,cp10,cp89,cp90,cp91,cp92 from caseprogress" & _
      ",staff where cp09='" & strReceiveNo & "' and cp13=st01(+)"
   intI = 1
   Set RsTemp = ClsLawReadRstMsg(intI, strExc(0)) 'edit by nickc 2007/02/05 不用 dll 了 objLawDll.ReadRstMsg(intI, strExc(0))
   With RsTemp
      If intI = 1 Then
         If Not IsNull(.Fields(1)) Then Label2(1) = .Fields(1)
         If Not IsNull(.Fields(2)) Then Label2(2) = TransDate(.Fields(2), 1)
         If Not IsNull(.Fields(3)) Then
            Text9 = TransDate(.Fields(3), 1)
         Else
            Text9 = strSrvDate(2)
         End If
         If Not IsNull(.Fields(4)) Then Text10 = .Fields(4): ChgType (10)
         If Not IsNull(.Fields(5)) Then Text14 = .Fields(5)
         
         'Modify by Morgan 2005/11/18
         'If Not IsNull(.Fields(6)) Then Text6 = .Fields(6): ChgType (8)
         Label3(1) = "": Label3(2) = "": Label3(3) = "": Label3(4) = "": Label3(5) = ""
         If Not IsNull(.Fields("cp56")) Then txtAppNew(1) = .Fields("cp56"): ChgType (13)
         If Not IsNull(.Fields("cp89")) Then txtAppNew(2) = .Fields("cp89"): ChgType (14)
         If Not IsNull(.Fields("cp90")) Then txtAppNew(3) = .Fields("cp90"): ChgType (15)
         If Not IsNull(.Fields("cp91")) Then txtAppNew(4) = .Fields("cp91"): ChgType (16)
         If Not IsNull(.Fields("cp92")) Then txtAppNew(5) = .Fields("cp92"): ChgType (17)
         '2005/11/18 end
         
         If Not IsNull(.Fields(7)) Then cp(10) = .Fields(7)
         'Add By Cheng 2002/07/05
         'Modified by Lydia 2016/10/27 +新案有申請人指定國外代理人檔則預設 => cp(9), pa(9), pa(26)
         AddAgent Combo2, cp, , cp(44), cp(116), cp(9), pa(9), pa(26)
      End If
   End With
   '2012/8/1 add by sonia 若有銷帳則要扣除銷帳規費
   If Val(cp(77)) > 0 Then
      If GetCP77Detail(cp(9), m_Fee, m_Official) = True Then
         cp(17) = cp(17) - m_Official
      End If
   End If
   '2012/8/1 end
   
   'Added by Lydia 2023/06/20 判斷FCP案,寰華案
   'Move by Lydia 2023/07/03 從PUB_ReadCaseProgressDatabase下方移過來
   If Left(cp(12), 1) = "F" And pa(9) <> "000" Then
      m_bolFMP = True
   Else
      m_bolFMP = False
   End If
   m_bolFMP2 = False
   If m_bolFMP = True Then
      m_bolFMP2 = PUB_FMPtoCheck(1, 2, Pub_strUserST05, pa(1), pa(2), pa(3), pa(4))
      '寰華案:承辦人為外專程序時,改為操作人員
      If m_bolFMP2 = True Then
         cp(14) = GetFCPUser(cp(14))
         Text10 = cp(14) 'Added by Lydia 2023/07/03
      End If
   End If
   'end 2023/06/20
   
   'Add by Morgan 2004/8/11
   txtCP84.Tag = cp(17)
   
   'Add by Morgan 2009/8/17
   If Text9 <> "" Then
      PUB_SetChkResultDate pa(1), pa(9), cp(10), Text9, txtChkRltDate, cp, pa(8)
      Text9.Tag = Text9
   End If
   
   'Added by Morgan 2015/7/7
   txtCP118 = ""
   If cp(118) <> "" Then txtCP118 = "Y"
   
   If pa(9) = "020" Then txtCP118 = "Y" 'Added by Morgan 2024/1/19 大陸案預設電子送件--郭
   
   txtPayToday = ""
   If txtCP118 = "Y" And pa(9) = "000" Then
      If Val(ServerTime) <= 153000 Then
         txtPayToday = "Y"
      End If
   End If
   'end 2015/7/7
    'Added by Lydia 2021/05/25
    txtCP113 = ""
    If cp(113) <> "" Then txtCP113 = cp(113)
    'end 2021/05/25
End Sub

Private Function ChgType(i As Integer) As Boolean
 Dim strTempName As String
   ChgType = False
   Select Case i
      Case 0
         '2011/12/8 MODIFY BY SONIA 發文日可輸系統日的下一個工作日
         'If Not ChkDate(Text9) Or Val(Text9.Text) > Val(strSrvDate(2)) Then
         '   MsgBox "發文日期不正確或發文日大於系統日，請重新輸入 !", vbCritical
         If Not ChkDate(Text9) Or DBDATE(Val(Text9.Text)) > DBDATE(PUB_GetWorkDayAfterSysDate(Val(strSrvDate(1)))) Then
            MsgBox "發文日期不正確或發文日大於系統日下一個工作日，請重新輸入 !", vbCritical
         '2011/12/8 END
         Else
            ChgType = True
         End If
      Case 3, 4, 5, 6, 7
         'edit by nickc 2007/02/05 不用 dll 了
         'If objLawDll.LawGetName(pa(i + 23), strTempName) Then
         If ClsLawLawGetName(pa(i + 23), strTempName) Then
            Label2(i) = strTempName
            ChgType = True
         End If
      Case 10
         'edit by nickc 2007/02/02 不用 dll 了
         'If objPublicData.GetStaff(Text10, strTempName) Then
         If ClsPDGetStaff(Text10, strTempName) Then
            Label2(9) = strTempName
            ChgType = True
         End If
      Case 12 '代理人
         strExc(1) = Combo2.Text
         'Add by Morgan 2008/2/22 加判斷是否為聯絡人
         If InStr(strExc(1), "-") > 0 Then
            If ClsPDGetContact(strExc(1), strTempName) Then
               Combo2 = strExc(1)
               Label2(14) = strTempName
               ChgType = True
            End If
         
         ElseIf PUB_GetAgentName(pa(1), strExc(1), strTempName) = True Then
            Combo2.Text = strExc(1)
            Label2(14).Caption = strTempName
            ChgType = True
         Else
            Label2(14).Caption = ""
         End If

      'Modify by Morgan 2005/11/17
'      Case 8 '受讓人1
'         If objLawDll.GetCusCAJnam(Text6.Text, strExc(0), strExc(1), strExc(2)) = True Then
'            For i = 0 To 2
'               Text8(i) = strExc(i)
'            Next
'            ChgType = True
'         End If
      Case 13, 14, 15, 16, 17 '受讓人
         Label3(i - 12) = ""
         If txtAppNew(i - 12).Text <> "" Then
            'edit by nickc 2007/02/05 不用 dll 了
            'If objLawDll.GetCusCAJnam(txtAppNew(i - 12).Text, strExc(0), strExc(1), strExc(2)) = True Then
            If ClsLawGetCusCAJnam(txtAppNew(i - 12).Text, strExc(0), strExc(1), strExc(2)) = True Then
               Label3(i - 12) = strExc(0)
               ChgType = True
            End If
         ElseIf i = 13 Then
            MsgBox "讓與人1不可空白！", vbExclamation
         Else
            ChgType = True
         End If
         
   End Select
End Function

Private Sub Text10_GotFocus()
   TextInverse Text10
End Sub

Private Sub Text10_Validate(Cancel As Boolean)
   If Text10 <> "" Then
      If Not ChgType(10) Then Cancel = True
   End If
End Sub

Private Sub Text14_GotFocus()
   TextInverse Text14
End Sub

Private Sub Text15_GotFocus(Index As Integer)
    'Modify By Cheng 2002/10/28
    Select Case Index
    Case 0
        Me.Text15(Index).SelStart = 0
        Me.Text15(Index).SelLength = 0
    Case Else
        TextInverse Text15(Index)
    End Select
End Sub

Private Sub Text9_GotFocus()
   TextInverse Text9
End Sub

Private Sub Text9_Validate(Cancel As Boolean)
   If Text9 <> "" Then
      If Not ChgType(0) Then
         Cancel = True
      Else
         'Add by Morgan 2009/8/17
         If Text9.Tag <> Text9 Then
            PUB_SetChkResultDate pa(1), pa(9), cp(10), Text9, txtChkRltDate, cp, pa(8)
            Text9.Tag = Text9
            'Added by Morgan 2015/7/7
            '當發文日有改時,電子送件案要人工輸入是否當日扣款
            If txtCP118 = "Y" Then
               txtPayToday.Text = ""
            End If
            'end 2015/7/7
         End If
      End If
   Else
      MsgBox "發文日不可空白 !", vbCritical
      Cancel = True
   End If
End Sub

'Add By Cheng 2002/07/05
Private Function CheckDataIntegrity() As Boolean
Dim Cancel As Boolean
   'add by nickc 2008/05/01
   If IsDebt(pa(9), cp(9)) Then
        MsgBox "未收款且無 預定收款日 請轉告智權同仁！！", vbOKOnly, "警告！禁止發文！"
        GoTo IntegrityOrNot
   End If
Cancel = False

'檢查代理人欄位
Combo2_Validate Cancel
If Cancel = True Then
   Me.Combo2.SetFocus
   GoTo IntegrityOrNot
End If
CheckDataIntegrity = True
Exit Function

IntegrityOrNot:
CheckDataIntegrity = False
End Function

'Add By Cheng 2002/05/22
Private Function TxtValidate() As Boolean
Dim objTxt As Object
Dim ii As Integer
Dim Cancel As Boolean
Dim bolChkPA165IsY As Boolean 'Added by Morgan 2021/2/5

TxtValidate = False

   'Added by Morgan 2021/12/15 檢查畫面輸入欄位是否含有Unicode文字
   If PUB_ChkUniText(Me, , True, "TextBox") = False Then
       Exit Function
   End If
   'end 2021/12/15

If Me.Text10.Enabled = True Then
   Cancel = False
   Text10_Validate Cancel
   If Cancel = True Then
      Me.Text10.SetFocus
      Text10_GotFocus
      Exit Function
   End If
End If

   'Modify by Morgan 2005/11/18
'   If Me.Text6.Enabled = True Then
'      Cancel = False
'      Text6_Validate Cancel
'      If Cancel = True Then
'         Me.Text6.SetFocus
'         Text6_GotFocus
'         Exit Function
'      End If
'   End If
   For ii = 1 To 5
      If Me.txtAppNew(ii).Enabled = True Then
         Cancel = False
         txtAppNew_Validate ii, Cancel
         If Cancel = True Then
            Me.txtAppNew(ii).SetFocus
            txtAppNew_GotFocus ii
            Exit Function
         End If
      End If
   Next
   '2005/11/18 end

If Me.Text9.Enabled = True Then
   Cancel = False
   Text9_Validate Cancel
   If Cancel = True Then
      Me.Text9.SetFocus
      Text9_GotFocus
      Exit Function
   End If
End If

'Add by Morgan 2004/8/11
If txtCP84.Enabled = True Then
   Cancel = False
   txtCP84_Validate Cancel
   If Cancel = True Then
      txtCP84.SetFocus
      txtCP84_GotFocus
      Exit Function
   End If
End If

'Add by Morgan 2004/9/14
If Combo2.Enabled = True Then
   Cancel = False
   Combo2_Validate Cancel
   If Cancel = True Then
      Combo2.SetFocus
      Exit Function
   End If
End If

   'Add by Morgan 2005/7/15
   If lstNameAgent.Visible = True Then
      Cancel = False
      lstNameAgent_Validate Cancel
      If Cancel = True Then
         lstNameAgent.SetFocus
         Exit Function
      End If
   End If
   '2005/7/14 END
   'Add by Morgan 2004/7/22
   If pa(9) = "000" Then
      m_DiscType = ""
      For ii = 1 To 5
         m_DiscType = m_DiscType & txtAD(ii).Text
         If txtAD(ii).Enabled = True Then
            If txtAD(ii).Text = "" Then
               MsgBox "申請人【" & txtAppNew(ii) & "-" & Label3(ii) & "】減免身分不可空白", vbInformation
               txtAD(ii).SetFocus
               txtAD_GotFocus ii
               Exit Function
            '學校
            ElseIf (txtAD(ii).Text = "2") Then
               '變更
               If (txtAD(ii).Tag <> "2" And txtAD(ii).Tag <> "") Then
                  If MsgBox("確定要變更申請人【" & txtAppNew(ii) & "-" & Label3(ii) & "】減免身分為【學校】？", vbInformation + vbYesNo + vbDefaultButton1) = vbNo Then
                     txtAD(ii).SetFocus
                     txtAD_GotFocus ii
                     Exit Function
                  End If
               End If
            '公司
            ElseIf (txtAD(ii).Text = "3") Then
               '新增或變更
               If (txtAD(ii).Tag <> "3") Then
                  If MsgBox("申請人【" & txtAppNew(ii) & "-" & Label3(ii) & "】的減免身分將設定為【中小企業】，確定有【證明文件】存放於本卷？", vbInformation + vbYesNo + vbDefaultButton1) = vbNo Then
                     txtAD(ii).SetFocus
                     txtAD_GotFocus ii
                     Exit Function
                  End If
               End If
            '不可減免
            ElseIf (txtAD(ii).Text = "N") Then
               '身分變更
               If (txtAD(ii).Tag <> "N" And txtAD(ii).Tag <> "") Then
                  If MsgBox("確定要變更申請人【" & txtAppNew(ii) & "-" & Label3(ii) & "】減免身分為【不可減免】？", vbInformation + vbYesNo + vbDefaultButton1) = vbNo Then
                     txtAD(ii).SetFocus
                     txtAD_GotFocus ii
                     Exit Function
                  End If
               End If
            End If
         End If
      Next
      If InStr(m_DiscType, "N") > 0 Then
         cp(81) = "N"
      Else
         cp(81) = "Y"
      End If
      
      '檢查是否可減免退費且未收文
      m_bolAdd919 = False
      If cp(81) = "Y" Then
         If PUB_CheckYearFeeReturn(pa) = True Then
            m_bolAdd919 = True
         End If
      End If
      
   End If
   
   'Added by Morgan 2015/7/7
   If txtCP118 = "Y" And pa(9) = "000" Then
      If cp(22) = "N" Then
         MsgBox "電子送件不可不出名！", vbCritical
         Exit Function
      ElseIf Val(txtCP84) > 0 Then
         If txtPayToday = "" Then
            MsgBox "電子送件請輸入是否當日扣款(Y/N)！", vbExclamation
            txtPayToday.SetFocus
            Exit Function
         End If
      End If
   End If
   'end 2015/7/7

   'Added by Morgan 2021/2/5
   bolSetPA165 = False
   If pa(9) = "000" And (cp(10) = "701" Or cp(10) = "708") Then
      bolChkPA165IsY = True
      If Trim(txtAppNew(1)) <> "" Then
         If PUB_ChkPA165IsY(txtAppNew(1)) = True Then
            If Trim(txtAppNew(2)) <> "" Then
               If PUB_ChkPA165IsY(txtAppNew(2)) = True Then
                  If Trim(txtAppNew(3)) <> "" Then
                     If PUB_ChkPA165IsY(txtAppNew(3)) = True Then
                        If Trim(txtAppNew(4)) <> "" Then
                           If PUB_ChkPA165IsY(txtAppNew(4)) = True Then
                              If Trim(txtAppNew(5)) <> "" Then
                                 If PUB_ChkPA165IsY(txtAppNew(5)) = False Then
                                    bolChkPA165IsY = False
                                 End If
                              End If
                           Else
                              bolChkPA165IsY = False
                           End If
                        End If
                     Else
                        bolChkPA165IsY = False
                     End If
                  End If
               Else
                  bolChkPA165IsY = False
               End If
            End If
         Else
            bolChkPA165IsY = False
         End If
         If bolChkPA165IsY = False Then
            If MsgBox("是否要設定為總委任書案件？", vbYesNo + vbDefaultButton2 + vbInformation) = vbYes Then
               bolSetPA165 = True
            End If
         End If
      End If
   End If
   'end 2021/2/5
   
   'Added by Lydia 2021/05/25 ACS智財顧問專業分配比例管制：有相關卷號(CaseRelation1)為ACS且曾有收文智財顧問112
   If Pub_ChkACS112isNull(pa(1), pa(2), pa(3), pa(4), txtCP113) = True Then
         txtCP113.SetFocus
         txtCP113_GotFocus
         Exit Function
   End If
   'end 2021/05/25
   
   'Added by Morgan 2023/8/1
   If pa(1) = "P" And pa(9) = "020" And (cp(10) = "701" Or cp(10) = "708") Then
      If PUB_CNP701708Check(cp(55) & "," & cp(93) & "," & cp(94) & "," & cp(95) & "," & cp(96), txtAppNew(1) & "," & txtAppNew(2) & "," & txtAppNew(3) & "," & txtAppNew(4) & "," & txtAppNew(5), True) = False Then
         Exit Function
      End If
   End If
   'end 2023/8/1
   
'Added by Morgan 2024/1/19
If pa(9) = "020" And txtCP118 = "" Then
   If MsgBox("請確認本案是否為紙本送件？", vbQuestion + vbYesNo + vbDefaultButton2) = vbNo Then
      txtCP118.SetFocus
      Exit Function
   End If
End If
'end 2024/1/19

TxtValidate = True
End Function

'只有公司可輸入 2,3,N
Private Sub txtAD_KeyPress(Index As Integer, KeyAscii As Integer)
   KeyAscii = UpperCase(KeyAscii)
   'Modified by Morgan 2014/7/15 學校改預設且不可改
   'If Not (KeyAscii = 8 Or KeyAscii = 50 Or KeyAscii = 51 Or KeyAscii = 78) Then
   If Not (KeyAscii = 8 Or KeyAscii = 51 Or KeyAscii = 78) Then
      KeyAscii = 0
   End If
End Sub
Private Sub txtAD_GotFocus(Index As Integer)
   'edit by nickc 2007/07/11 切換輸入法改用API
   'txtAD(Index).IMEMode = 2
   CloseIme
   TextInverse txtAD(Index)
End Sub

Private Sub txtAppNew_Change(Index As Integer)
   SetAD Index
End Sub

'Add by Morgan 2005/11/17
Private Sub txtAppNew_GotFocus(Index As Integer)
   TextInverse txtAppNew(Index)
End Sub

Private Sub txtAppNew_KeyPress(Index As Integer, KeyAscii As Integer)
   KeyAscii = UpperCase(KeyAscii)
End Sub

Private Sub txtAppNew_Validate(Index As Integer, Cancel As Boolean)
   
   If Not ChgType(Index + 12) Then
      Cancel = True
   End If
   '檢查客戶/代理人是否不再使用
   If Cancel = False Then
      If PUB_CheckStatus(txtAppNew(Index).Text) = False Then Cancel = True
   End If
End Sub
'2005/11/17 end

'Added by Lydia 2021/05/25
Private Sub txtCP113_GotFocus()
   TextInverse txtCP113
End Sub

'Added by Lydia 2021/05/25
Private Sub txtCP113_Validate(Cancel As Boolean)
   If txtCP113 <> "" Then
      If Not IsNumeric(txtCP113) Then
         MsgBox "請輸入數字！", vbExclamation
         txtCP113.SetFocus
         txtCP113_GotFocus
         Cancel = True
         Exit Sub
      End If
   End If
End Sub

Private Sub txtCP118_GotFocus()
   TextInverse txtCP118
   CloseIme
End Sub

Private Sub txtCP118_KeyPress(KeyAscii As Integer)
   KeyAscii = UpperCase(KeyAscii)
   If KeyAscii <> 8 And KeyAscii <> Asc("Y") Then
      Beep
      KeyAscii = 0
   End If
End Sub
'Add by Morgan 2004/8/11
Private Sub txtCP84_GotFocus()
   TextInverse txtCP84
End Sub
'Add by Morgan 2004/8/11
Private Sub txtCP84_KeyPress(KeyAscii As Integer)
   '只能輸倒退及數字鍵
   If KeyAscii <> 8 And (KeyAscii < 48 Or KeyAscii > 57) Then
      Beep
      KeyAscii = 0
   End If
End Sub
'Add by Morgan 2004/8/11
Private Sub txtCP84_Validate(Cancel As Boolean)
   '台灣
   If pa(9) = "000" Then
      If Val(txtCP84.Text) <> Val(cp(17)) And Val(txtCP84.Text) <> Val(txtCP84.Tag) Then
         If MsgBox("發文規費【" & txtCP84.Text & "】與收文規費【" & cp(17) & "】不同，確定要繼續！", vbInformation + vbYesNo + vbDefaultButton2) = vbYes Then
            txtCP84.Tag = txtCP84.Text
         Else
            txtCP84_GotFocus
            Cancel = True
         End If
      End If
   End If
End Sub

'Add by Morgan 2005/7/15
'檢查並設定cp110資料
Private Sub lstNameAgent_Validate(Cancel As Boolean)
   Dim ii As Integer, bolCheck As Boolean
   bolCheck = False
   cp(110) = ""
   For ii = 0 To lstNameAgent.ListCount - 1
      If lstNameAgent.Selected(ii) = True Then
         'modify by sonia 2016/10/7 員工編號已可非數字需做轉換
         'cp(110) = cp(110) & "," & lstNameAgent.ItemData(ii)
         'Modified by Morgan 2021/12/15f Forms2.0 改用模組
         'cp(110) = cp(110) & "," & PUB_Num2Id(lstNameAgent.ItemData(ii))
         'Modified by Morgan 2021/12/15f Forms2.0 改用模組
         'cp(110) = cp(110) & "," & PUB_Num2Id(lstNameAgent.ItemData(ii))
         cp(110) = cp(110) & "," & PUB_GetItemData(lstNameAgent.Tag, ii)
         bolCheck = True
      End If
   Next
   If Left(cp(110), 1) = "," Then cp(110) = Mid(cp(110), 2)
   If bolCheck = True Then
      cp(22) = ""
   Else
      cp(22) = "N"
      If MsgBox("未勾選代理人，確定不出名？", vbYesNo + vbDefaultButton2 + vbInformation) = vbNo Then
         Cancel = True
      End If
   End If
End Sub

'Add by Morgan 2005/11/21
Private Sub SetAD(ByVal p_idx As Integer)
   Dim strAD10 As String, strCU15 As String
   txtAD(p_idx).Enabled = False
   txtAD(p_idx).Tag = ""
   txtAD(p_idx).Text = ""
   If txtAppNew(p_idx).Text <> "" And pa(9) = "000" Then
      txtAD(p_idx).Text = PUB_GetAD03(txtAppNew(p_idx).Text, pa(9), strAD10, strCU15)
      txtAD(p_idx).Tag = txtAD(p_idx).Text
      '個人只可設定自然人(1)
      If strCU15 = "0" Then
         txtAD(p_idx).Text = "1"
      'Added by Morgan 2014/7/15 學校也預設--玲玲
      ElseIf strCU15 = "2" Then
         txtAD(p_idx).Text = "2"
      'end 2014/7/15
      '公司
      Else
         If txtAD(p_idx).Text = "Y" Then
            txtAD(p_idx).Text = strAD10
            txtAD(p_idx).Tag = txtAD(p_idx).Text
         End If
         txtAD(p_idx).Enabled = True
      End If
   End If
End Sub
'Add by Morgan 2005/11/21 新增減免退費內部收文並發文
Private Sub AddNewCP()
   Dim Ncp() As String
   ReDim Ncp(TF_CP) As String
   Ncp(1) = cp(1)
   Ncp(2) = cp(2)
   Ncp(3) = cp(3)
   Ncp(4) = cp(4)
   Ncp(5) = strSrvDate(2)
   'Modify by Morgan 2011/2/24 修正百年收文號問題
   'Ncp(9) = "B" & Left(strSrvDate(2), 2)
   Ncp(9) = "B" & CompAutoNumberYear(GetTaiwanThisYear)
   Ncp(10) = "919" '減免退費
   Ncp(11) = "90"
   Ncp(12) = cp(12)
   Ncp(13) = cp(13)
   Ncp(14) = cp(14)
   Ncp(16) = 0
   Ncp(17) = 0
   Ncp(18) = 0
   Ncp(20) = "N"
   Ncp(26) = "N"
   Ncp(27) = strSrvDate(2)
   Ncp(32) = "N"
   Ncp(86) = "Y" 'Add by Morgan 2005/7/4
   'edit by nickc 2007/02/02 不用 dll 了
   'If Not objPublicData.SaveNewCaseProgressDatabase("B", Ncp, intWhere) Then
   If Not ClsPDSaveNewCaseProgressDatabase("B", Ncp, intWhere) Then
      MsgBox "無法產生 ""減免退費"" 內部收文！", vbCritical
   End If
End Sub
'Add by Morgan 2009/8/17
Private Sub lblCaseFee_Click()
   frm12040102_2.txtCF(1) = cp(1)
   frm12040102_2.txtCF(2) = pa(9)
   frm12040102_2.txtCF(3) = cp(10)
   frm12040102_2.Show vbModal
   If Val(Text9) > 0 Then
      PUB_SetChkResultDate pa(1), pa(9), cp(10), Text9, txtChkRltDate, cp, pa(8)
   End If
End Sub

Private Sub lblCaseFee_MouseDown(Button As Integer, Shift As Integer, x As Single, y As Single)
   PUB_LabelMouseDown lblCaseFee, lblCaseFees
End Sub

Private Sub lblCaseFee_MouseUp(Button As Integer, Shift As Integer, x As Single, y As Single)
   PUB_LabelMouseUp lblCaseFee, lblCaseFees
End Sub

Private Sub txtChkRltDate_Validate(Cancel As Boolean)
   If txtChkRltDate <> "" Then
      If ChkDate(txtChkRltDate) = False Then
         Cancel = True
      End If
   End If
End Sub

Private Sub txtPayToday_GotFocus()
   TextInverse txtPayToday
   CloseIme
End Sub

Private Sub txtPayToday_KeyPress(KeyAscii As Integer)
   KeyAscii = UpperCase(KeyAscii)
   If KeyAscii <> 8 And KeyAscii <> Asc("Y") And KeyAscii <> Asc("N") Then
      KeyAscii = 0
      Beep
   End If
End Sub

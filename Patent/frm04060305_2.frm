...
Begin VB.Form frm04060305_2 
   ...
   End
End
Attribute VB_Name = "frm04060305_2"
...
Private Sub ExecuteQuery()
   ...
   If IsEmpty(m_DateFrom) = False Then
      ...
      strSubSQL = strSubSQL & "TPG03 >= " & ChangeTStringToWString(m_DateFrom) & " "
   End If
   If IsEmpty(m_DateTo) = False Then
      If strSubSQL <> Empty Then: strSubSQL = strSubSQL & "AND "
      strSubSQL = strSubSQL & "TPG03 <= " & ChangeTStringToWString(m_DateTo) & " "
   End If
   If IsEmpty(m_DateFrom) = False Or IsEmpty(m_DateTo) = False Then
      pub_QL05 = pub_QL05 & ";" & frm04060305_1.Label1 & frm04060305_1.text01_01 & "-" & frm04060305_1.text01_02 'Add By Sindy 2010/12/2
   End If
   If IsEmpty(m_CountryBegin) = False Then
      If strSubSQL <> Empty Then: strSubSQL = strSubSQL & "AND "
      strSubSQL = strSubSQL & "TPG06 >= '" & m_CountryBegin & "' "
   End If
   If IsEmpty(m_CountryEnd) = False Then
      If strSubSQL <> Empty Then: strSubSQL = strSubSQL & "AND "
      strSubSQL = strSubSQL & "TPG06 <= '" & m_CountryEnd & "' "
   End If
   If IsEmpty(m_CountryBegin) = False Or IsEmpty(m_CountryEnd) = False Then
      pub_QL05 = pub_QL05 & ";" & frm04060305_1.Label2 & frm04060305_1.text02_01 & "-" & frm04060305_1.text02_02 'Add By Sindy 2010/12/2
   End If
   If strSubSQL <> Empty Then
      strSql = strSql & " WHERE " & strSubSQL
   End If
   strSql = strSql & " ORDER BY TPG06"
...
Private Sub ListData()
   ...
   strCountry = Empty
   If IsNull(m_Recordset.Fields("TPG06")) = False Then
      strCountry = m_Recordset.Fields("TPG06")
   End If
   ' 清除代理人事務所串列
   ClearAgentList
   ...
   Do While m_Recordset.EOF <> True
      strCmp = Empty
      If IsNull(m_Recordset.Fields("TPG06")) = False Then
         strCmp = m_Recordset.Fields("TPG06")
      End If
      ...
      strAgentCode = Empty
      If IsNull(m_Recordset.Fields("TPG07")) = False Then
         strAgentCode = m_Recordset.Fields("TPG07")
      End If
      strAgentName = GetAgentCompany(strAgentCode)
      
      nType = 0
      'Modify by Morgan 2010/12/28 申請案號改碼數
      'Select Case Mid(m_Recordset.Fields("TPG01"), 3, 1)
      Select Case Mid(m_Recordset.Fields("TPG01"), 4, 1)
         Case 1: nType = 1
         Case 2: nType = 2
         Case 3: nType = 3
      End Select

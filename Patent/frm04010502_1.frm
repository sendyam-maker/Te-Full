...
Begin VB.Form frm04010502_1 
   ...
   End
End
Attribute VB_Name = "frm04010502_1"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'Memo by Lydia 2022/02/18 改成Form2.0 ; MSHFlexGrid1改字型=新細明體-ExtB
...
Public m_DeadLine As String 'Added by Morgan 2014/8/4
...
Private Sub Command1_Click()
   ...
   If Me.Option1(0).Value Then
      If Text7 = "" Then
         ...
         Exit Sub
      End If
      ' 90.06.28 modify by louis 大陸案件可以以申請案號來搜尋
      ...
      '   "nvl(cp37,nvl(cp38,cp38)) as Name,nvl(cp37,nvl(cp38,cp39)) as RName,''," & _
      '   "cp01,cp02,cp03,cp04,'' from caseprogress where CP01='P' AND " & _
      '   "cp36='" & Text7 & "' and (cp01,cp02,cp03,cp04) not in " & _
    ...
    '  strExc(0) = "select " & ChgPatent("", 1) & " as No,nvl(pa05,nvl(pa06,pa07)) as Name," & _
         ...
         "select distinct(" & ChgCaseprogress("", 1) & "||'N') as No," & _
         "nvl(cp37,nvl(cp38,cp38)) as Name,nvl(cp37,nvl(cp38,cp39)) as RName,''," & _
         "cp01,cp02,cp03,cp04,pa09 from caseprogress,patent where CP01='P' AND " & _
         "cp36='" & Text7 & "' and (cp01,cp02,cp03,cp04) not in " & _
         "(select pa01,pa02,pa03,pa04 from patent where PA01='P' AND pa11='" & Text7 & "' " & _
      ...
      '設別名f0,+FMP2openSQL =>f2
        ...
        '抓對造資料
         strExc(0) = strExc(0) & " union select distinct(" & ChgCaseprogress("", 1) & "||'N') as No," & _
            "nvl(cp37,nvl(cp38,cp38)) as Name,nvl(cp37,nvl(cp38,cp39)) as RName,''," & _
            "cp01,cp02,cp03,cp04,pa09 from caseprogress f2,patent where CP01='P' AND cp36='" & Text7 & "' " & _
            FMP2openSQL & " and (cp01,cp02,cp03,cp04) not in " & _
            "(select pa01,pa02,pa03,pa04 from patent where PA01='P' AND pa11='" & Text7 & "' " & _
            ") and pa01(+)=cp01 and pa02(+)=cp02 and pa03(+)=cp03 and pa04(+)=cp04 "
   ...
   Else
      ...
      If Me.Text1.Text = "P" Then
         ...
         If rsA.RecordCount > 0 Then
            'Modified by Morgan 2012/6/29 申請寄存核准時不一定申請案已發文
            'If "" & rsA("PA09") = 台灣國家代號 Then
            If "" & rsA("PA09") = 台灣國家代號 And IsNull(rsA("pa11")) = False Then
...
'         strExc(0) = "select " & ChgPatent("", 1) & " as No,nvl(pa05,nvl(pa06,pa07)) as Name," & _
            ...
            "select distinct(" & ChgCaseprogress("", 1) & "||'N') as No," & _
            "nvl(cp37,nvl(cp38,cp38)) as Name,nvl(cp37,nvl(cp38,cp39)) as RName,''," & _
            "cp01,cp02,cp03,cp04,pa09 from caseprogress f0,patent where CP01='" & Me.Text1.Text & "' AND CP02='" & Me.Text2.Text & "' AND CP03='" & IIf(Me.Text3.Text = "", "0", Me.Text3.Text) & "' AND CP04='" & IIf(Me.Text4.Text = "", "00", Me.Text4.Text) & "' " & _
         ...
         '設別名f0,+FMP2openSQL
         strExc(0) = "select " & ChgPatent("", 1) & " as No,nvl(pa05,nvl(pa06,pa07)) as Name," & _
            "'' as RName,'',pa01,pa02,pa03,pa04,pa09 cty from patent f0 " & _
            "where PA01='" & Me.Text1.Text & "' AND PA02='" & Me.Text2.Text & "' AND PA03='" & IIf(Me.Text3.Text = "", "0", Me.Text3.Text) & "' " & _
            "AND PA04='" & IIf(Me.Text4.Text = "", "00", Me.Text4.Text) & "' " & FMP2openSQL
         strExc(0) = Replace(strExc(0), "f0.CP", "f0.PA")
        'Add by Lydia 2014/10/31 先判斷外專程序人員權限。
        If FMP2open = True And FMP2openSQL <> "" Then
           If PUB_FMPtoCheck(0, 1, Pub_strUserST05, "CHANGE_SQL", strExc(0)) = False Then
        ...
        '抓對造資料
         strExc(0) = strExc(0) & " union select distinct(" & ChgCaseprogress("", 1) & "||'N') as No," & _
            "nvl(cp37,nvl(cp38,cp38)) as Name,nvl(cp37,nvl(cp38,cp39)) as RName,''," & _
            "cp01,cp02,cp03,cp04,pa09 from caseprogress f2,patent " & _
            "where CP01='" & Me.Text1.Text & "' AND CP02='" & Me.Text2.Text & "' AND CP03='" & IIf(Me.Text3.Text = "", "0", Me.Text3.Text) & "' AND " & _
            "CP04='" & IIf(Me.Text4.Text = "", "00", Me.Text4.Text) & "'" & FMP2openSQL & _
            " and (cp01,cp02,cp03,cp04) not in (select pa01,pa02,pa03,pa04 from patent where PA01='" & Me.Text1.Text & "' AND PA02='" & Me.Text2.Text & "' " & _
            "AND PA03='" & IIf(Me.Text3.Text = "", "0", Me.Text3.Text) & "' AND PA04='" & IIf(Me.Text4.Text = "", "00", Me.Text4.Text) & "' " & _
            ") and pa01(+)=cp01 and pa02(+)=cp02 and pa03(+)=cp03 and pa04(+)=cp04 "
         strExc(0) = Replace(strExc(0), "f0.CP", "f2.CP")
      ...
      Else
         ...
         strExc(0) = " select " & ChgService("", 1) & " as No,nvl(sp05,nvl(sp06,sp07)) as Name," & _
            "'' as RName,'',sp01,sp02,sp03,sp04,sp09 cty From ServicePractice f0 where SP01='" & Me.Text1.Text & "' " & _
            "AND SP02='" & Me.Text2.Text & "' AND SP03='" & IIf(Me.Text3.Text = "", "0", Me.Text3.Text) & "' AND SP04='" & IIf(Me.Text4.Text = "", "00", Me.Text4.Text) & "' " & FMP2openSQL
        ...
        '抓對造資料
         strExc(0) = strExc(0) & " union select distinct(" & ChgCaseprogress("", 1) & "||'N') as No," & _
            "nvl(cp37,nvl(cp38,cp38)) as Name,nvl(cp37,nvl(cp38,cp39)) as RName,''," & _
            "cp01,cp02,cp03,cp04,pa09 from caseprogress f2 where CP01='" & Me.Text1.Text & "' AND CP02='" & Me.Text2.Text & "' " & _
            "AND CP03='" & IIf(Me.Text3.Text = "", "0", Me.Text3.Text) & "' AND CP04='" & IIf(Me.Text4.Text = "", "00", Me.Text4.Text) & "' " & FMP2openSQL & _
            " and (cp01,cp02,cp03,cp04) not in (select Sp01,Sp02,Sp03,Sp04 from ServicePractice " & _
            "where SP01='" & Me.Text1.Text & "' AND SP02='" & Me.Text2.Text & "' " & _
            "AND SP03='" & IIf(Me.Text3.Text = "", "0", Me.Text3.Text) & "' AND SP04='" & IIf(Me.Text4.Text = "", "00", Me.Text4.Text) & "' " & ")"
         strExc(0) = Replace(strExc(0), "f0.CP", "f2.CP")
      End If
   End If
...
Private Function TxtValidate() As Boolean
   ...
   'Add by Morgan 2009/2/20 直接按Enter時若日期錯會沒檢查到
   Else
...
'2011/7/11 CANCEL BY SONIA 改至第二畫面再檢查
'   ' 90.07.10 modify by louis (非台灣案件不需檢查來函記錄檔)
...
'         If IsEmptyText(rsTmp.Fields("PA09")) = False Then
'            If rsTmp.Fields("PA09") < "010" Then
...
'   Set rsTmp = Nothing
   ...
   '2017/12/27 END
   'Add By Sindy 2016/10/5
   frm04010502_2.m_strIR01 = m_strIR01
   frm04010502_2.m_strIR02 = m_strIR02
   frm04010502_2.m_strIR03 = m_strIR03
   frm04010502_2.m_strIR04 = m_strIR04
   '2016/10/5 END
   frm04010502_2.Show

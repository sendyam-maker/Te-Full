...
Private Sub Form_Load()
   MoveFormToCenter Me
   FormClear
   If CheckUse("frm060111", strPrint, False) = True Then
      ...
      Do While Not .EOF
         If IsNull(.Fields("cp27")) Then
            strSql = "Update CaseProgress Set CP27=" & DBDATE(txtCP27) & _
               " where cp09='" & .Fields("cp09") & "' and cp10='413' and cp57 is null and cp27 is null"
            cnnConnection.Execute strSql, intI
         Else
            strSql = "Update CaseProgress Set CP27=null" & _
               " where cp09='" & .Fields("cp09") & "' and cp10='413' and cp57 is null and cp27=19221111"
            cnnConnection.Execute strSql, intI
         End If
         
         strSql = "update nextprogress set np06='N' where np01='" & .Fields("cp43") & "' and np06 is null and np07='202'"
         cnnConnection.Execute strSql, intI
...
Private Function GetRst(p_iType As Integer, p_iRlt As Integer) As ADODB.Recordset
   ...
   Select Case p_iType
      Case 1 '未收文
         strExc(0) = "select pa01,pa02,pa03,pa04,cp09,cp110 from patent,caseprogress a" & _
            " where pa01='FCP' and '" & stCustNo & "' in (pa26,pa27,pa28,pa29,pa30)" & _
            " and cp01(+)=pa01 and cp02(+)=pa02 and cp03(+)=pa03 and cp04(+)=pa04 and cp10='928' and cp27>20070000" & _
            " and exists(select * from nextprogress where np01=cp09 and np07||''='202' and np06 is null)" & _
            " and not exists(select * from caseprogress b where b.cp43=a.cp09 and b.cp10='413')"
      
      Case 2 '已收文未發文
         strExc(0) = "select cp09,cp27,cp43 from patent,caseprogress a" & _
            " where pa01='FCP' and '" & stCustNo & "' in (pa26,pa27,pa28,pa29,pa30)" & _
            " and cp01(+)=pa01 and cp02(+)=pa02 and cp03(+)=pa03 and cp04(+)=pa04 and cp10='413' and cp27 is null" & _
            " and exists(select * from caseprogress b where b.cp09=a.cp43 and b.cp10='928')"
      
      Case 3 '已發文
         strExc(0) = "select 1 from patent,caseprogress a" & _
            " where pa01='FCP' and '" & stCustNo & "' in (pa26,pa27,pa28,pa29,pa30)" & _
            " and cp01(+)=pa01 and cp02(+)=pa02 and cp03(+)=pa03 and cp04(+)=pa04 and cp10='413' and cp27>20070000" & _
            " and exists(select * from caseprogress b where b.cp09=a.cp43 and b.cp10='928')"
      ...
      Case 4 '案件清單
         strExc(0) = "select decode(cp27,null,2,1) C1,substr(cp09,1,1) C2,decode(pa16,'1',1,2) C3,cp05,s1.st02 S1,cp27,pa75,na03" & _
            ...
            " where pa01='FCP' and '" & stCustNo & "' in (pa26,pa27,pa28,pa29,pa30)" & _
            " and cp01(+)=pa01 and cp02(+)=pa02 and cp03(+)=pa03 and cp04(+)=pa04 and cp10='413'" & _
      ...
      Case 5 '整批發文(發文日=19221111的要清空)
         strExc(0) = "select cp09,cp27,cp43 from patent,caseprogress a" & _
            " where pa01='FCP' and '" & stCustNo & "' in (pa26,pa27,pa28,pa29,pa30)" & _
            " and cp01(+)=pa01 and cp02(+)=pa02 and cp03(+)=pa03 and cp04(+)=pa04 and cp10='413' and (cp27 is null or cp27=19221111)" & _
            " and exists(select * from caseprogress b where b.cp09=a.cp43 and b.cp10='928')"
...
Private Sub PrintAppForm()
   ...
   If intI = 1 Then
      With rsData
         Do While Not .EOF
            EndLetter "01", .Fields("cp09"), "03", strUserNum
            NowPrint .Fields("cp09"), "01", "03", False, strUserNum, , , , , 2
            .MoveNext
         Loop
         PUB_BatchPrint "5"
      End With
...
Private Sub PrintAppList(Optional p_AppDate As String)
   ...
   strExc(0) = "SELECT LPAD(CP01||'-'||CP02||'-'||CP03||'-'||CP04,15,' ') C01, 0 C02, 0 C03" & _
            ",RPAD(NVL(PA11,' '),12,' ') C04,'自請撤回(重新委任)' C05, RPAD(NVL(CU04,' '),20,' ') C06" & _
            ",RPAD(PA05,40,' ') C07" & _
...
Private Sub PrintHead()
      ...
      strTitle = "外專 " & ChangeTStringToTDateString(strSrvDate(2)) & " 重新委任自請撤回送件清單"
      Printer.Print
      Printer.FontSize = 16

...
Begin VB.Form frm06010612 
   ...
   Begin VB.ComboBox Combo2 
      Height          =   300
      ItemData        =   "frm06010612.frx":0000
      Left            =   630
      List            =   "frm06010612.frx":0019
      Style           =   2  '單純下拉式
      TabIndex        =   42
      Top             =   2520
      Width           =   735
   ...
   Begin VB.Frame Frame4 
      ...
      Begin VB.ComboBox cboReason 
         Height          =   276
         ItemData        =   "frm06010612.frx":0057
         Left            =   600
         List            =   "frm06010612.frx":0059
         Style           =   2  '單純下拉式
         TabIndex        =   57
         Top             =   0
         Width           =   315
...
Public m_strFullFileName As String
Public m_LstEmp As String
Dim m_WorkEmpList As String '工作代理 Add By Sindy 2022/9/5
Const m_strSysCode As String = ",FCT,CFT,CFC,S,T,TF" 'Modify By Sindy 2023/9/5 周雅嫺提需做TF歸卷
      ...
      For i = 1 To GRD1.Rows - 1
         If GRD1.TextMatrix(i, 0) = "V" Then
            For j = 0 To intTotList
               ...
               '可以輸入轉寄內容: 非外專承辦組，程序組人員，CFT人員
               If PUB_IPDept_IsCFMail(strChkEmp) <> True Then
...
On Error GoTo ErrHand
   ...
   Label6.Visible = False: Label13.Visible = False
   ...
   strSql = "select '' V,IR23 符號,GetInputRecordReply(ir01,ir02,ir03) 確,sqldatet(Ii12)||' '||sqltime6(Ii13) 收信日期時間,decode(Ii23,null,'',Ii23||'-'||Ii24||'-'||Ii25||'-'||Ii26) 本所案號,Ii17 主旨" & _
            ",'' 收受者,s2.st02||'-'||decode(ii29," & 外專信件處理結果 & ",ii29) 處理人員" & _
   ...
   Else
      ...
      Label9.Visible = True: Label12.Visible = True
      Combo2.Visible = True
      ...
      strSql = "select '' V,IR23 符號,GetInputRecordReply(ir01,ir02,ir03) 確,sqldatet(Ii12)||' '||sqltime6(Ii13) 收信日期時間,decode(Ii23,null,'',Ii23||'-'||Ii24||'-'||Ii25||'-'||Ii26) 本所案號,Ii17 主旨,'' 收受者" & _
               ",decode(ir16,'3',s3.st02,decode(ii29||ir16,null,decode(ir15,'Y',decode(length(ir03),5,decode(substr(ir03,1,1),'T','TM','P','Patent','IPDept'),'IPDept'),s1.st02),s2.st02))||decode(sign(instr('3,8',ir16)),1,decode(ir16," & 信件處理狀態 & ",ir16),decode(ii29," & 外專信件處理結果 & ",ii29)) 處理或轉寄者" & _
               ",decode(ii29,null,sqldatet(Ii08)||' '||sqltime6(Ii09),sqldatet(ir17)||' '||sqltime6(ir18)) 處理或轉寄日期時間" & _
               ",sqldatet(IR05)||' '||sqltime6(IR06) 讀取日期時間" & _
               ",IR01,IR02,Ii18,IR04,Ii06,Ii14,Ii03 檔名,Ii08,Ii09,ir11,ir12,Ii12,Ii05,IR16,decode(IR21,null,ii19,IR21) 總收文號,decode(FO02,null,IR20,FO02) 處理原因,ir24,ir19,getmailbox(Ii01,Ii03) 信箱來源,ir21" & _
...
Dim strIR16nm As String 'Add By Sindy 2023/8/15
...
'         End If
   ...
   Else
      ...
      'Modify by Sindy 2023/8/15 +strIR16nm
   ...
   For i = 1 To GRD1.Rows - 1
      ...
      If GRD1.TextMatrix(i, 0) = "V" And GRD1.TextMatrix(i, 23) <> "9" Then '9.回信
         ...
         If bolOK = False Then '3.退回
            ...
            strExc(0) = "update IPDeptInput set ii29=null" & _
         ...
         Else
            ...
            'Modify By Sindy 2023/9/22 此段不適用,會沖到不處理其他人未沖銷的信件
'            '未處理的都自沖(上確認日期時間人員)
...
'   If GRD1.Rows - 1 >= 1 And GRD1.TextMatrix(1, 16) = "" Then Exit Sub
   ...
   Else
      ...
      If txtPI18 <> "" Then '有輸入案號代表要歸卷，所以要檢查案號和文號資料
         'Modify By Sindy 2023/7/24 不處理,不檢核系統別
         If ChkCaseNo(False, True) = False Then
            'Exit Sub
         ElseIf txtRecvNo.Text <> "" Then
            bolUptCaseNo = True
...
On Error GoTo ErrHand
   ...
   For i = 1 To GRD1.Rows - 1
      If GRD1.TextMatrix(i, 0) = "V" Then
         ...
         If txtPI18 & txtPI19 & txtPI20 & txtPI21 <> "" Then
         ...
         '外商信件沖銷機制
         If PUB_IPDeptEMailF1UptRec(GRD1.TextMatrix(i, 10), GRD1.TextMatrix(i, 11), _
...
'檢查本所案號和總收文號
'Modify By Sindy 2023/7/24 + , Optional ByVal bolNotChkSysCode As Boolean
Public Function ChkCaseNo(Optional bolChkRecv As Boolean = True, Optional ByVal bolNotChkSysCode As Boolean = True) As Boolean
Dim strCP12 As String

   ChkCaseNo = False
   Screen.MousePointer = vbHourglass 'Add By Sindy 2025/6/19
   
   'Modify By Sindy 2023/7/19
   'If txtPI18 <> "FCT" And txtPI18 <> "CFT" And txtPI18 <> "CFC" And txtPI18 <> "S" Then
   If InStr(m_strSysCode, "," & txtPI18) = 0 Then
   '2023/7/19 END
      'Add By Sindy 2023/7/24
      If bolNotChkSysCode = True Then
...
On Error GoTo ErrHand
   ...
   For i = 1 To GRD1.Rows - 1
      If GRD1.TextMatrix(i, 0) = "V" Then
         '檢查是否有處理狀態
         If PUB_CheckIRStatus(GRD1.TextMatrix(i, 10), GRD1.TextMatrix(i, 11), _
                          ChgSQL(GRD1.TextMatrix(i, 16)), ChgSQL(GRD1.TextMatrix(i, 13)), strIR16) = True Or _
            InStr(GRD1.TextMatrix(i, 7), "回信") > 0 Then
            If strIR16 = "" Then
               If InStr(GRD1.TextMatrix(i, 7), "回信") > 0 Then
                  strIR16 = "回信"
               End If
            End If
            MsgBox "此封信件已有人操作【" & strIR16 & "】，請畫面更新！", vbExclamation, "警告！"
...
'            End If
            ...
            'If MsgBox("沖銷碼已複製【" & stPwd & "】，確定回信了嗎？" & vbCrLf & vbCrLf & "注意：先回信後，再按下「是」因為信件關閉，後續才能一併歸卷！", vbExclamation + vbYesNo + vbDefaultButton1, "重要訊息！") = vbNo Then
            If MsgBox("沖銷碼已複製【" & stPwd & "】，請回貼於主旨後方。" & vbCrLf & vbCrLf & _
                      "是：已使用此沖銷碼進行沖銷" & vbCrLf & vbCrLf & _
                      "否：取消此動作" & vbCrLf & vbCrLf & "注意：回掉訊息，就會清除沖銷碼。（以防貼至其他信件）", vbExclamation + vbYesNo + vbDefaultButton1, "重要訊息！") = vbNo Then
               Clipboard.Clear '清除沖銷碼
               Exit Sub
'            Else
'               If PUB_ChkFileOpening2(m_strFullFileName, "後續才能一併歸卷！") = True Then
'                  Exit Sub
...
'      sqlB = sqlB & " union "
      sqlB = "select '' V," & SQLDate("CP05") & " as 收文日,cp09 as 總收文號,decode(tm10,'000',cpm03,cpm04) as 案件性質,s1.st02 as 承辦人,s2.st02 as 智權人員," & SQLDate("CP27") & " as 發文日,cp05,cp66,cp67,cp09 " & _
...
'                                 CNULL(IIf(Trim(Left(Combo1, 6)) <> strUserNum, Trim(Left(Combo1, 6)), "")) & _
'                                 ",'" & IIf(InStr(ArrStr(2), "(cc)") > 0, "Y", "") & "')"
'            cnnConnection.Execute strExc(0), intI
'
'            '非外專承辦和外專程序和CFT人員收件人員
'            If PUB_GetST03(strIR04) <> "F22" And _
'               PUB_GetST03(strIR04) <> "F23" And _
'               PUB_IPDept_IsCFMail(IIf(Pub_GetSpecMan(CStr(ArrStr(1))) <> "", Pub_GetSpecMan(CStr(ArrStr(1))), strIR04)) <> True Then
'
'               '再檢查一次信件檔案是否存在
'               If strFileName = "" Then strFileName = Mid(GRD1.TextMatrix(i, 15), InStrRev(GRD1.TextMatrix(i, 15), "/") + 1)
...
'               '是否為副本
'               If InStr(ArrStr(2), "(cc)") > 0 Then
'                  If strCC <> "" Then strCC = strCC & ";"
'                  '檢查是否為特殊設定
'                  If Pub_GetSpecMan(CStr(ArrStr(1))) <> "" Then
'                     strCC = strCC & Pub_GetSpecMan(CStr(ArrStr(1))) '群組
'                  Else
'                     strCC = strCC & strIR04
'                  End If
'               Else
...
'                  '檢查是否為特殊設定
'                  If Pub_GetSpecMan(CStr(ArrStr(1))) <> "" Then
'                     strTo = strTo & Pub_GetSpecMan(CStr(ArrStr(1))) '群組
'                  Else
'                     strTo = strTo & strIR04
'                  End If
...
'                  IIf(TextContext.Enabled = True And Trim(TextContext) <> "", TextContext, vbCrLf & "信件內容參附件！"), , strFullFileName, , , , strCC, , , , , , , , , , , , , , , , , , "1"
...
'
'         '外商承辦組
'         If PUB_GetST03(Trim(Left(Combo1, 6))) = "F11" Then
...
'               cnnConnection.Execute strExc(0), intI
'               '更新主檔處理結果(尚是空白或歸卷)
'               strExc(0) = "update IPDeptInput set ii29='8'" & _
'                           " where ii01=" & GRD1.TextMatrix(i, 10) & _
'                             " and ii02=" & GRD1.TextMatrix(i, 11) & _
...
Private Sub Form_Load()
   ...
   '備註/原因
   cboReason.Clear
   'Modify By Sindy 2024/2/2 May提出新原因清單
   cboReason.AddItem "已回覆信件"
   cboReason.AddItem "已通知客戶"
   cboReason.AddItem "已以Outlook處理"
   cboReason.AddItem "帳單已結"
   cboReason.AddItem "屬確收信函"
   cboReason.AddItem "交由收件者之承辦人處理"
   cboReason.AddItem "已轉寄他部門"
   cboReason.AddItem "例行通知/開拓信函"
   '2024/2/2 END
'   cboReason.AddItem "已處理:"
'   cboReason.AddItem "  已回覆信件"
'   cboReason.AddItem "  待來函，暫無後續"
'   cboReason.AddItem "  已輸期限(含通知客戶)"
'   cboReason.AddItem "  已通知客戶"
'   cboReason.AddItem "  已以OutLook處理" 'Add By Sindy 2023/7/12
'   cboReason.AddItem "  其他"
'   cboReason.AddItem "" 'Add By Sindy 2023/7/11
'   cboReason.AddItem "不處理:"
'   cboReason.AddItem "  屬確收信函"
'   cboReason.AddItem "  尚有接續來函"
'   cboReason.AddItem "  已轉寄他部門"
'   cboReason.AddItem "  例行通知"
'   cboReason.AddItem "  開拓信函"
'   cboReason.AddItem "  回覆本所詢問"
'   cboReason.AddItem "  交由收件者之承辦人處理"
'   cboReason.AddItem "  非本人應處理案件" 'Add By Sindy 2023/7/11
   cboReason.AddItem "其他"
   cboReason.AddItem "非本人應處理案件" 'Modify By Sindy 2024/2/2 琬姿要保留在其他之後
...
Private Sub SetCmdStatus()
   Option1(0).Value = True '卷宗區
   cmdDelRow.Visible = False '刪除
...
'      End If
'   End If
'   cmdDelRow.Tag = cmdDelRow.Enabled 'Add By Sindy 2021/1/22 記錄刪除鍵原狀況
End Sub
...
Dim Rs As New ADODB.Recordset
   ...
   If Left(Pub_StrUserSt03, 2) = "F1" Then
      '抓CF案人員
      '2級主管可以幫組員操作
      strSql = "Select st03,st01,st02,st05,Sl02,st16 組別代號,decode(st16,'2','英文組','4','日文組','6','CF案',st16) 組別,st70 小組,st04" & _
               " From Staff, Staff_Level" & _
               " Where st03 Like 'F1%' and st05=sl01(+)" & _
               " and (st16 in('6') or st01 in('80030','78011'))" & _
               " and st04='1'" & _
               " and st52='" & strUserNum & "'" & _
               " order by st01 asc"
      intI = 1
      Set RsTemp = ClsLawReadRstMsg(intI, strSql)
      If intI = 1 Then
         With RsTemp
            .MoveFirst
            Do While Not .EOF
               If Not IsNull(RsTemp.Fields("st01")) Then
                  strText = Trim(RsTemp.Fields("st01")) & " " & GetPrjSalesNM(Trim(RsTemp.Fields("st01")))
                  For ii = 0 To Combo1.ListCount - 1
                     If InStr(Combo1.List(ii), Trim(RsTemp.Fields("st01"))) = 1 Then
                        Exit For
                     End If
                  Next
   ...
   If Left(Pub_StrUserSt03, 2) <> "F1" Then
      MsgBox "注意：此沖銷信件是屬外商人員操作，一定要用該單位人員ID，因程式判斷上是用單位做相關的處理！", vbCritical
      Combo1.Clear
   Else
      ...
      If iRow <> GRD1.MouseRow Or iCol <> GRD1.MouseCol Then
         If GRD1.MouseCol = 5 Or GRD1.MouseCol = 6 Or GRD1.MouseCol = 7 Or GRD1.MouseCol = 8 Then
            If GRD1.TextMatrix(GRD1.MouseRow, GRD1.MouseCol) <> "" Then
               ...
               iCol = GRD1.MouseCol
            End If
         ElseIf GRD1.MouseCol = 3 And GRD1.TextMatrix(GRD1.MouseRow, 10) <> "" Then
...
'      strCP13 = PUB_GetAKindSalesNo(txtPI18, txtPI19, txtPI20, txtPI21) '目前智權人員
      ...
      strExc(0) = "select tm12,tm15,tm10,'T' as sys_type from trademark" & _
                  ...
                  " where sp01='" & txtPI18 & "'" & _
                    ...
                    " and sp04='" & txtPI21 & "'"
'                    & _
'                  " union select pa11,pa22,pa09,'P' as sys_type from patent" & _
...
'                    " and pa04='" & txtPI21 & "'"
      ...
      If intI = 1 Then
         ...
         m_RegNo = "" & RsTemp.Fields("tm15") '號數
         m_Nation = "" & RsTemp.Fields("tm10") '申請國家
   ...
   If m_Nation = "000" Then '申請國家=000:FC案
   ...
   With GRD1
      For m_iRow = 1 To .Rows - 1
         ...
         If .TextMatrix(.row, 0) = "V" And .RowHeight(.row) > 0 Then
            ...
            m_AppNo = "": m_RegNo = ""
            'Modify By Sindy 2023/7/19
            'Modify By Sindy 2023/9/6 + Or txtPI18 = "TF"
            If txtPI18 = "T" Or txtPI18 = "TF" Then
               MsgBox txtPI18 & "系統別無輸入功能！", vbExclamation, "警告！"
               Exit Sub
            '2023/7/19 END
            ElseIf txtPI18 <> "" Then
               If ChkCaseNo(False) = False Then
            ...
            End If
            '檢查是否有處理狀態
            If PUB_CheckIRStatus(m_strIR01, m_strIR02, ChgSQL(m_strIR03), ChgSQL(m_strIR04), strIR16) = True Or _
               InStr(GRD1.TextMatrix(.row, 7), "輸入") > 0 Then
               If strIR16 = "" Then
                  If InStr(GRD1.TextMatrix(.row, 7), "輸入") > 0 Then
                     strIR16 = "輸入"
                  End If
               End If
               MsgBox "此封信件已有人操作【" & strIR16 & "】，請畫面更新！", vbExclamation, "警告！"
...
'Modify By Sindy 2023/8/15 + , pIR16nm As String
Private Function IsReKeyInCase(pCP09 As String, pIR16nm As String) As Boolean
   Dim strSql As String, intQ As Integer
   Dim rsQuery As ADODB.Recordset
   
   'If Pub_StrUserSt03 = "F22" Then 'Added by Morgan 2023/4/18
      'Modify By Sindy 2023/8/15 + And pIR16nm = "輸入"
      If Left(pCP09, 1) = "C" And pIR16nm = "輸入" Then
         intQ = 1
         strSql = "select cp07 from caseprogress where cp09='" & pCP09 & "' and cp07>0"

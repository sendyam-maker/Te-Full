...
Begin VB.Form frm04060308 
   ...
   End
End
Attribute VB_Name = "frm04060308"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'Memo By Lydia 2021/11/17 Form2.0已檢查 (無需修改的物件)
'Create By Lydia 2017/01/05 專利公報市場占有率比較
Option Explicit
...
Dim m_Tot1 As Double, m_Tot2 As Double, m_Tot3 As Double '國內、大陸、國外總計(今年度)
Dim b_Tot1 As Double, b_Tot2 As Double, b_Tot3 As Double '國內、大陸、國外總計(前一年)
Dim m_TopNum As Integer '前X名
Dim strMid As String, strMid2 As String
Dim xlsSalesPoint As New Excel.Application
Dim wks4638 As New Worksheet
  ...
  'end 2022/01/13
   ...
   For mCall = 1 To 2
        ...
        strSql = "select decode(substr(tpb06,1,1),'A','國內',decode(tpb06,'C0020','大陸','國外')) title,count(*) cnt from (" & _
                 strMid & strMid2 & ") group by decode(substr(tpb06,1,1),'A','國內',decode(tpb06,'C0020','大陸','國外')) "
        intI = 1
        Set RsTemp = ClsLawReadRstMsg(intI, strSql)
        If intI = 1 Then
           RsTemp.MoveFirst
           Do While Not RsTemp.EOF
              Select Case "" & RsTemp.Fields("title")
                  Case "國內"
                      If mCall = 1 Then
                         m_Tot1 = Val("" & RsTemp.Fields("cnt"))
                      Else
                         b_Tot1 = Val("" & RsTemp.Fields("cnt"))
                      End If
                  Case "大陸"
                      If mCall = 1 Then
                         m_Tot2 = Val("" & RsTemp.Fields("cnt"))
                      Else
                         b_Tot2 = Val("" & RsTemp.Fields("cnt"))
                      End If
                  Case "國外"
                      If mCall = 1 Then
                         m_Tot3 = Val("" & RsTemp.Fields("cnt"))
                      Else
                         b_Tot3 = Val("" & RsTemp.Fields("cnt"))
                      End If
              End Select
              RsTemp.MoveNext
           Loop
   ...
   For mCall = 1 To 2
       For iCall = 1 To 3
           ...
           strSql = "select decode(substr(tpb06,1,1),'A','1',decode(tpb06,'C0020','2','3')) ord1," & _
                     ...
                     strMid & strMid2 & " ) where decode(substr(tpb06,1,1),'A','1',decode(tpb06,'C0020','2','3')) = " & CNULL(Format(iCall, "0")) & " and nvl(tpb08,' ') <> ' ' " & _
           ...
           '---------------開啟xls檔
              If mCall = 1 And iCall = 1 Then
                  ...
                  Set wks4638 = xlsSalesPoint.Worksheets(1)
                  wks4638.PageSetup.Orientation = xlPortrait '直印
                  '抬頭
                   wks4638.PageSetup.PrintTitleRows = "$1:$3"
                   
                   wks4638.Columns("a:a").ColumnWidth = 10 '項目
                   wks4638.Columns("b:b").ColumnWidth = 10
                   For intI = Asc(startX) To Asc(startX) + m_TopNum - 1
                       wks4638.Columns(Chr(intI) & ":" & Chr(intI)).ColumnWidth = 13
                       wks4638.Columns(Chr(intI) & ":" & Chr(intI)).HorizontalAlignment = xlCenter
                       wks4638.Columns(Chr(intI) & ":" & Chr(intI)).VerticalAlignment = xlBottom
                   Next
                   ...
                   wks4638.Range("a1").Value = strExc(0) & " " & Me.Caption
                   With wks4638.Range("a1:" & Chr(Asc(startX) + m_TopNum - 1) & "1")
                     .WrapText = False
                     .MergeCells = True
                  ...
                  For intI = 1 To m_TopNum
                     wks4638.Range(Chr(Asc(startX) + intI - 1) & xRows).Value = intI
                  Next
                  xRows = xRows + 1
              ...
              With m_rs
                  ...
                  inJ = 1
                    ...
                    If strTemp <> .Fields("title") Then
                       ...
                       wks4638.Range("a" & xRows).Value = IIf(mCall = 1, Mid(txtDate(0), 1, 3) & "年", Val(Mid(txtDate(0), 1, 3)) - 1 & "年")
                       wks4638.Range("b" & xRows).Value = "" & .Fields("title")
                       wks4638.Range("b" & xRows + 1).Value = "筆數"
                       wks4638.Range("b" & xRows + 2).Value = "占有率"
                  ...
                  Do While inJ <= m_TopNum
                     '事務所名稱
                     wks4638.Range(Chr(Asc(startX) + inJ - 1) & xRows).Value = "" & .Fields("tpb08")
                     '筆數
                     wks4638.Range(Chr(Asc(startX) + inJ - 1) & xRows + 1).Value = Val("" & .Fields("cnt"))
                     '無代理人且為本所案件,若最後發文的AB類程序為不出名則算台一案件
                     If "" & .Fields("tpb08") = "台一國際" Then
                         strExc(7) = Replace(strMid2, "TPB", "R0")
                         Select Case "" & .Fields("ord1")
                            Case "1": strExc(7) = strExc(7) & " and substr(R006,1,1)='A' "
                            Case "2": strExc(7) = strExc(7) & " and R006='C0020' "
                            Case "3": strExc(7) = strExc(7) & " and substr(R006,1,1)<>'A' and R006<>'C0020' "
                         End Select
                         ...
                         If intI = 1 Then
                            If Val("" & RsTemp.Fields("add1")) > 0 Then
                                wks4638.Range(Chr(Asc(startX) + inJ - 1) & xRows + 1).Value = Val(wks4638.Range(Chr(Asc(startX) + inJ - 1) & xRows + 1).Value) + Val("" & RsTemp.Fields("add1"))
                            End If
                         End If
                     End If
                     ...
                     '占有率
                     If mCall = 1 Then
                         wks4638.Range(Chr(Asc(startX) + inJ - 1) & xRows + 2).Value = "=$" & Chr(Asc(startX) + inJ - 1) & xRows + 1 & " / " & IIf(iCall = 1, m_Tot1, IIf(iCall = 2, m_Tot2, m_Tot3))
                     Else
                         wks4638.Range(Chr(Asc(startX) + inJ - 1) & xRows + 2).Value = "=$" & Chr(Asc(startX) + inJ - 1) & xRows + 1 & " / " & IIf(iCall = 1, b_Tot1, IIf(iCall = 2, b_Tot2, b_Tot3))
                     End If
                     wks4638.Range(Chr(Asc(startX) + inJ - 1) & xRows + 2).NumberFormatLocal = "##0.00%"
                     inJ = inJ + 1
                     .MoveNext
   ...
   Next mCall
   '----------------切換今年度和前一年
'Added by Lydia 2019/05/16 增加合計排名(國內+大陸+國外)by楊特助
   For mCall = 1 To 2 '-------------切換今年度和前一年
        If mCall = 1 Then '今年度
           strMid2 = " and TPB03 >=" & CNULL(strVol1_S) & " and TPB03 <=" & CNULL(strVol1_E)
        Else              '前一年
           strMid2 = " and TPB03 >=" & CNULL(strVol2_S) & " and TPB03 <=" & CNULL(strVol2_E)
        ...
        xRows = xRows + 3
        wks4638.Range("a" & xRows).Value = IIf(mCall = 1, Mid(txtDate(0), 1, 3) & "年", Val(Mid(txtDate(0), 1, 3)) - 1 & "年")
        wks4638.Range("b" & xRows).Value = "合計"
        wks4638.Range("b" & xRows + 1).Value = "筆數"
        wks4638.Range("b" & xRows + 2).Value = "占有率"
        
        strSql = "select tpb08,count(*) cnt from (" & strMid & strMid2 & ") where nvl(tpb08,' ') <> ' ' group by tpb08 order by cnt desc,tpb08 "
        intI = 1
        ...
        If intI = 1 Then
           With m_rs
             ...
             For inJ = 1 To m_TopNum
               ...
               End If
               '占有率
               wks4638.Range(Chr(Asc(startX) + inJ - 1) & xRows + 2).Value = "=$" & Chr(Asc(startX) + inJ - 1) & xRows + 1 & " / " & m_Tot1 + m_Tot2 + m_Tot3
               wks4638.Range(Chr(Asc(startX) + inJ - 1) & xRows + 2).NumberFormatLocal = "##0.00%"
               .MoveNext
             Next inJ
           End With

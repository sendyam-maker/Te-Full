...
Begin VB.Form Frmacc7170 
   ...
   End
End
Attribute VB_Name = "Frmacc7170"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'Add by Lydia 2014/10/3 分所每日收款明細表-智權人員繳款
...
'Add by Lydia 2015/01/15 +留分所,範圍+1
'Dim PLeft(0 To 20) As Integer '20=(0~13)14+(0~4)5+1(左邊界)
'Dim SPLeft(0 To 14) As Integer '14=總計欄位+1
'Dim strTemp(0 To 21) As String
'Dim m_sColumn(0 To 18) As String '明細欄位名稱
'Dim Sm_sColumn(1 To 13) As String '總計欄位名稱
'Dim lngSubTot() As String, lngTot(1 To 13) As Double  '小計,合計

    'Modified by Lydia 2015/07/16 設定欄位數
    Private Const LR1 = 14 '13 '第1行的欄位數(從0開始)
    Private Const LR2 = 4  '第2行的欄位數(從0開始)
    Private Const LRt = 14 '尾頁總計的欄位數(從0-不顯示的業務代號開始)
    ...
    Dim mArea As Double '留分所
    'end 2015/07/16
    
Dim mTName As String '所別名稱
'end 2015/01/15

'列印用
Dim iPrint As Integer, iPage As Integer, iRec As Integer
Dim M7170 As New ADODB.Recordset
...
Dim mChk4401 As String, mChk4402 As Long, mChk4403 As Long
...
'Add by Lydia 2015/01/14預設報表抬頭所別
mTName = IIf(pub_strUserOffice = "1", "北所", IIf(pub_strUserOffice = "2", "中所", IIf(pub_strUserOffice = "3", "南所", IIf(pub_strUserOffice = "4", "高所", "其他所"))))
...
'   m_sColumn(9) = "補扣繳"
'   m_sColumn(10) = "外幣"
'   m_sColumn(11) = "留" & mTName 'Add by Lydia 2015/01/14
'   m_sColumn(12) = "會計簽名"
'   m_sColumn(13) = "出納備註"
'   m_sColumn(14) = "收據號碼"
'   m_sColumn(15) = "客戶名稱"
...
Dim TmpStr As String
   ...
   TmpStr = "確認日期|繳款人|票據金額|北所電匯|分所電匯|現金|抵暫收款|其他|溢收款|手續費|補扣繳|外幣|留" & mTName & "|會計簽名|出納備註"
...
'   Sm_sColumn(11) = "合計"
'   Sm_sColumn(12) = "點數"
'   Sm_sColumn(13) = "留" & mTName 'Add by Lydia 2015/01/14
Dim TmpArr As Variant
Dim TmpStr As String

   TmpStr = "繳款人|票據金額|北所電匯|分所電匯|現金|抵暫收款|其他|溢收款|手續費|補扣繳|外幣|合計|點數|留" & mTName
   TmpArr = Split(TmpStr, "|")
...
Private Sub PrintLine(mKind As Integer)
   ...
   If mKind = 2 Then
     'Modified by Lydia 2015/07/16
     'iNo = (PLeft(20) - ciStartX) \ Printer.TextWidth("=")
...
'主檔只計算一次金額,明細點數需要累計
       ...
       For m_In = 0 To m_Ix - 1
          ...
          If LTrim(RTrim(lngSubTot(m_In, 0))) = LTrim(RTrim(sChk4401)) Then
            lngSubTot(m_In, LRt - 1) = Val(lngSubTot(m_In, LRt - 1)) + mDot '個人小計-點數
            lngTot(LRt - 1) = lngTot(LRt - 1) + mDot '總計-點數
            m_Rwrite = True
          Else
...
Else
    ...
    If m_SState = 1 Then '第一個智權人小計
      ...
      'Modified by Lydia 2015/07/16
      'ReDim lngSubTot(0 To m_Ix - 1, 0 To 13)
...
'        lngSubTot(0, 13) = strTemp(21) 'Add by Lydia 2015/01/14
        lngSubTot(0, LRt - 2) = m_Stot
        lngSubTot(0, LRt - 1) = mDot
        lngSubTot(0, LRt) = mArea
    Else
       m_Rwrite = False
       m_Rnum = 0
       For m_In = 0 To m_Ix - 1
...
'            lngSubTot(m_In, 13) = Val(lngSubTot(m_In, 13)) + Val(strTemp(21)) 'Add by Lydia 2015/01/14
            lngSubTot(m_In, LRt - 2) = Val(lngSubTot(m_In, LRt - 2)) + m_Stot
            lngSubTot(m_In, LRt - 1) = Val(lngSubTot(m_In, LRt - 1)) + mDot
            lngSubTot(m_In, LRt) = Val(lngSubTot(m_In, LRt)) + mArea
            m_Rwrite = True
          Else
            If Len(lngSubTot(m_In, 1)) = 0 Then '查無記錄->寫入新陣列
               m_Rnum = m_In
...
'         lngSubTot(m_Rnum, 13) = strTemp(21) 'Add by Lydia 2015/01/14
         lngSubTot(m_Rnum, LRt - 2) = m_Stot
         lngSubTot(m_Rnum, LRt - 1) = mDot
         lngSubTot(m_Rnum, LRt) = mArea
       End If
...
'      If m_TIn = 13 Then lngTot(m_TIn) = lngTot(m_TIn) + Val(strTemp(21)) '留分所 'Add by Lydia 2015/01/14
       Select Case LRt - m_TIn
          ...
          Case 0 '留分所
                 lngTot(m_TIn) = lngTot(m_TIn) + mArea
       End Select
    End If
   Next m_TIn
...
strSql = ""
    ...
    If Me.Text2.Text = "" Then   'A4404註記變更和條件判斷
        strSql = strSql & " And A4404 Is Null "
    End If
...
iRec = tmpRec!rr1
    ...
    If Option1(0).Value = True Then '注意排序A4401+A4402+A4403(單據號碼)
        strSql = strSql & " order by st06,a4401,a4413,a4402,a4403,axd04 " '--智權人,確認日期
    Else
        strSql = strSql & " order by a4413,a4401,a4402,a4403,axd04 "  '--確認日期
    End If
...
strA = "select st06,A4413,A4401,ST02,A4402,A4403,A4404,A4415, " & _
       ...
       "nvl(A4411,0) as A4411,nvl(A4422,0) as A4422,NVL(A4426,0) as A4426,NVL(A4429,0) as A4429,NVL(A4430,0) as A4430,AXD04,A0K04,nvl(a0k09, 0) as A0K09,a0j02, " & _
...
If M7170.State = 1 Then M7170.Close
M7170.CursorLocation = adUseClient
M7170.Open strA, cnnConnection, adOpenStatic, adLockReadOnly
iPage = 0
'Added by Lydia 2015/07/16
mDiff = 8
ReDim strTemp(0 To 19) As String '列印明細
GetPleft
...
If Not M7170.EOF And Not M7170.BOF Then
    With M7170
       M7170.MoveFirst
       
       PrintHeader '列印表頭
       
       Do While Not M7170.EOF
           iPage = iPage + 1
           '明細
'           strTemp(0) = ChangeTStringToTDateString(M7170!a4413 - 19110000)  '確認日期
'           strTemp(1) = Trim(M7170!st02) '繳款人(智權人員)
'           strTemp(18) = RTrim(LTrim(M7170!A4401)) '員工代號
'           strTemp(19) = Val(M7170!A4402) '繳款時間
'           strTemp(20) = Val(M7170!A4403) '繳款日期
'           strTemp(2) = Val(M7170!A4405) '--  票據金額
'           strTemp(3) = Val(M7170!A4406) '--  北所電匯
'           strTemp(4) = Val(M7170!A4407) '--  分所電匯
'           strTemp(5) = Val(M7170!A4408) '--  現金
'           strTemp(6) = Val(M7170!A4409) '--  抵暫收款
'           strTemp(7) = Val(M7170!A4410) '--  溢收款
'           strTemp(8) = Val(M7170!A4411) '--  手續費
'           strTemp(9) = Val(M7170!A4422) '--  補扣繳
'           strTemp(10) = Val(M7170!A4426) '--  外幣
'           strTemp(21) = Val(M7170!A4429) '--  留分所 'Add by Lydia 2015/01/14
'           '增加明細資料要注意PrintDetailRun欄位的對應關係
'           strTemp(11) = "" '--  會計簽名(留白)
'           strTemp(12) = "" & StrConv(LeftB(StrConv(LTrim(RTrim(M7170!A4415)), vbFromUnicode), 24), vbUnicode) '--  出納備註
'           strTemp(13) = "" & M7170!axd04 '--  收據號碼
'            If M7170!a0k09 > 0 Then '繳款後, 出納(acc0k0)作廢
'               strTemp(13) = strTemp(13) & "(廢)"
'            End If
'           strTemp(14) = "" & StrConv(LeftB(StrConv(LTrim(RTrim(M7170!A0K04)), vbFromUnicode), 24), vbUnicode) '取中文混雜指定長度(中文2,英文1 byte)
'           strTemp(15) = "" & Trim(M7170!A0J02) & StrConv(LeftB(StrConv(LTrim(RTrim(M7170!案件性質)), vbFromUnicode), 26), vbUnicode) & Trim(str(M7170!A0j09 + M7170!A0J10))
'
'           strTemp(16) = Val(M7170!addot) '--  點數
'           strTemp(17) = Val(M7170!AXD08) '--  扣繳金額
           
           sChk4401 = RTrim(LTrim(M7170!A4401)) '員工代號
           sChk4402 = Val(M7170!A4402) '繳款時間
           sChk4403 = Val(M7170!A4403) '繳款日期
           idR = 0
           strTemp(idR) = ChangeTStringToTDateString(M7170!a4413 - 19110000)  '確認日期
           idR = idR + 1: strTemp(idR) = Trim(M7170!st02) '繳款人(智權人員)
           idR = idR + 1: strTemp(idR) = Val(M7170!A4405) '--  票據金額
           idR = idR + 1: strTemp(idR) = Val(M7170!A4406) '--  北所電匯
           idR = idR + 1: strTemp(idR) = Val(M7170!A4407) '--  分所電匯
           idR = idR + 1: strTemp(idR) = Val(M7170!A4408) '--  現金
           idR = idR + 1: strTemp(idR) = Val(M7170!A4409) '--  抵暫收款
           idR = idR + 1: strTemp(idR) = Val(M7170!A4430) '--  其他
           idR = idR + 1: strTemp(idR) = Val(M7170!A4410) '--  溢收款
           'mDiff = idR '記錄溢收款位置
           idR = idR + 1: strTemp(idR) = Val(M7170!A4411) '--  手續費
           idR = idR + 1: strTemp(idR) = Val(M7170!A4422) '--  補扣繳
           idR = idR + 1: strTemp(idR) = Val(M7170!A4426) '--  外幣
           idR = idR + 1: strTemp(idR) = Val(M7170!A4429) '--  留分所
           mArea = Val(M7170!A4429)
           '增加明細資料要注意PrintDetailRun欄位的對應關係
           idR = idR + 1: strTemp(idR) = "" '--  會計簽名(留白)
           idR = idR + 1: strTemp(idR) = "" & StrConv(LeftB(StrConv(LTrim(RTrim(M7170!A4415)), vbFromUnicode), 24), vbUnicode) '--  出納備註
           idR = idR + 1: strTemp(idR) = "" & M7170!axd04 '--  收據號碼
            If M7170!a0k09 > 0 Then '繳款後, 出納(acc0k0)作廢
               strTemp(idR) = strTemp(idR) & "(廢)"
            End If
           idR = idR + 1: strTemp(idR) = "" & StrConv(LeftB(StrConv(LTrim(RTrim(M7170!A0K04)), vbFromUnicode), 24), vbUnicode) '取中文混雜指定長度(中文2,英文1 byte)
           idR = idR + 1: strTemp(idR) = "" & Trim(M7170!A0J02) & StrConv(LeftB(StrConv(LTrim(RTrim(M7170!案件性質)), vbFromUnicode), 26), vbUnicode) & Trim(str(M7170!A0j09 + M7170!A0J10))

           idR = idR + 1: strTemp(idR) = Val(M7170!addot) '--  點數
           mDot = Val(M7170!addot)
           idR = idR + 1: strTemp(idR) = Val(M7170!AXD08) '--  扣繳金額
           
           If .AbsolutePosition = 1 Then
              Call WriteSSum(1, iRec) '首筆重設陣列
           Else
              Call WriteSSum(2, iRec)
           End If
            
            PrintDetailRun '列印明細
            
             'A4404註記變更和條件判斷
            adoTaie.Execute "Update ACC440 Set A4404='*' Where A4401='" & M7170!A4401 & "' And A4402=" & M7170!A4402 & "  And A4403=" & M7170!A4403 & "  "
...
strPTmp = mTName & IIf(Me.Option1(0).Value = True, " " & Left(Me.Text1.Text, Len(Me.Text1.Text) - 2) & " 年 " & Right(Me.Text1.Text, 2) & " 月 份", " " & Me.MaskEdBox1.Text & " ~ " & Me.MaskEdBox2.Text & " ") & "每日收款明細表-智權人員繳款"
...
'Modified by Lydia 2015/07/16
'Printer.Print mTName & IIf(Me.Option1(0).Value = True, " " & Left(Me.Text1.Text, Len(Me.Text1.Text) - 2) & " 年 " & Right(Me.Text1.Text, 2) & " 月 份", " " & Me.MaskEdBox1.Text & " ~ " & Me.MaskEdBox2.Text & " ") & "每日收款明細表-智權人員繳款"
...
'Add by Lydia 2015/01/14
'For pa1 = 0 To 12
'  If pa1 >= 2 And pa1 <= 10 Then
'Modified by Lydia 2015/07/16
'For pa1 = 0 To 13
'  If pa1 >= 2 And pa1 <= 11 Then '金額類-置右
For pa1 = 0 To LR1
...
Else
    If Len(mChk4401) > 0 And mChk4402 > 0 And mChk4403 > 0 Then
      ...
      End If
    End If
'Add by Lydia 2015/01/14 +留分所strTemp(21)
'        For aP1 = 0 To 12
'        If aP1 >= 2 And aP1 <= 10 Then '金額類-置右
          'pB = Format(strTemp(aP1), DDollar2)
        'Modified by Lydia 2015/07/16
'        For aP1 = 0 To 13
'        If aP1 >= 2 And aP1 <= 11 Then '金額類-置右
'          If aP1 = 11 Then
'             pB = Format(strTemp(21), DDollar2)
'          Else
'             pB = Format(strTemp(aP1), DDollar2)
'          End If
        For aP1 = 0 To LR1
...
For aP1 = LR1 + 1 To LR1 + LR2 + 1
  If aP1 >= LR1 + LR2 Then  '金額類-置右
    ...
    Else
      pB = Format(strTemp(aP1), DDollar2)
    End If
     'Add by Lydia 2015/01/14 上一行+留分所strTemp(21) 位置後退一格
'    Printer.CurrentX = PLeft(aP1 + 1) - Printer.TextWidth(pB) - ciColGap
    'Modified by Lydia 2015/07/16
    'Printer.CurrentX = PLeft(aP1 + 2) - Printer.TextWidth(pB) - ciColGap
    Printer.CurrentX = PLeft(aP1 + 1) - Printer.TextWidth(pB) - ciColGap
    Printer.CurrentY = iPrint
    Printer.Print pB
  Else
...
'Add by Lydia 2015/01/14
'For sa1 = 1 To 12
'  If sa1 > 1 And sa1 <= 12 Then
'Modified by Lydia 2015/07/16
'For sa1 = 1 To 13
'  If sa1 > 1 And sa1 <= 13 Then '金額類-置右
For sa1 = 1 To LRt
...
Dim aP2 As Integer, pSB As String
...
Erase PLeft
'Add by Lydia 2015/01/15 +留分所(放外幣後面) 19=>20

   PLeft(0) = ciStartX
   'Modified by Lydia 2015/07/16
   'PLeft(20) = lngPageWidth - ciStartX * 2  '出納備註的邊界=列印右邊界
'For intI = 1 To 19
'   Select Case intI
'   '    Case 1 To 12, 17, 18
'    Case 1 To 7, 12, 13, 18, 19
'        PLeft(intI) = PLeft(intI - 1) + Printer.TextWidth(String(4, "　")) + ciColGap
'
'    Case 8 To 11 'Add by Lydia 2015/01/15縮小溢收款，手續費，補扣繳，外幣
'        PLeft(intI) = PLeft(intI - 1) + Printer.TextWidth(String(3, "　")) + ciColGap
''    Case 13 '收據號碼起
'    Case 14 '收據號碼起-換行
'        PLeft(intI) = PLeft(1)
'
      ...
      Else
        Select Case intI - LR1 - 1
         ...
         Case 2
             PLeft(intI) = PLeft(intI - 1) + Printer.TextWidth(String(12, "　")) + ciColGap
         Case 3
             PLeft(intI) = PLeft(intI - 1) + Printer.TextWidth(String(14, "　")) + ciColGap * 2
        End Select
      End If
   Else
       '縮小-其他、溢收款、手續費、補扣繳、外幣
      If intI > mDiff And intI <= mDiff + 4 Then
         PLeft(intI) = PLeft(intI - 1) + Printer.TextWidth(String(3, "　")) + ciColGap
      Else
         PLeft(intI) = PLeft(intI - 1) + Printer.TextWidth(String(4, "　")) + ciColGap
      End If
...
For intI = 1 To LRt
   '縮小-姓名、其他、溢收款、手續費、補扣繳、外幣
    If intI = 1 Or (intI >= mDiff - 1 And intI < mDiff + 4) Then
       SPLeft(intI) = SPLeft(intI - 1) + Printer.TextWidth(String(3, "　")) + ciColGap + 100
    Else
       SPLeft(intI) = SPLeft(intI - 1) + Printer.TextWidth(String(5, "　")) + ciColGap
    End If
Next intI

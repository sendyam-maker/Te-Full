      ...
      Case 1 '案件資料及進度查詢
        ...
        'end 2021/12/16
         If CheckUse("frm100101_1", strExec) Then
            frm100101_1.Show
         End If
      'Modify  by Amy 2014/04/30 申請人查詢分兩個項目(與其他系統順序不同)
      Case 2 '以申請人查詢
        'Added by Lydia 2021/12/16 配合改Form2.0Unload可能沒有下此指令改在呼叫前執行以避免前次變數值殘留
...
'      'Add By Sindy 2022/10/4 配合接洽單電子收文
'      mnu0101(4).Visible = False '待建檔區
'      If Pub_StrUserSt03 = "M51" Or _
'         UCase(pub_DbTerminalName) <> Pub_GetSpecMan("正式資料庫電腦名稱") Or _
'         strSrvDate(1) >= 接洽單電子收文啟用日 Then
'
'         mnu0101(4).Visible = True '待建檔區
'      End If
'      '2022/10/4 END
...
Select Case Index
             ...
             Case 8     '主管機關來函查詢
                        ...
                        frm010008.Show
             Case 9     '主管機關來函收文簿
                        If CheckUse("frm010009", strExec) = False Then
                           Exit Sub
                        End If
                        frm010009.Show
             Case 10     '主管機關來函資料刪除作業
                        If CheckUse("frm010010", strExec) = False Then
                           Exit Sub
                        End If
                        frm010010.Show
            ...
            'Add by Morgan 2004/1/27
            Case 12     '分所收文量查詢
                        If CheckUse("frm010013_1", strExec) = False Then
                           Exit Sub
                        End If
                        frm010013_1.Show
                        frm010013_1.UnloadChild
            'add by nickc 2007/06/13
            Case 13     '櫃檯每日信件輸入
                        If CheckUse("frm010016", strExec) = True Then
                           frm010016.Show
                        End If
            Case 14     '櫃台每日作業統計數輸入
                        If CheckUse("frm010017", strExec) = True Then
                           frm010017.Show
                        End If
            Case 15     '總務處工作報告列印
                        If CheckUse("frm010019", strExec) = True Then
                           frm010019.Show
                        End If
            'add by nickc 2007/11/01
            Case 16     '每月收文統計明細查詢
                        If CheckUse("frm010021", strExec) = True Then
                           frm010021.Show
                        End If
            ...
            Case 18 '價目表查詢
               If CheckUse("frm210143", strExec) = True Then
                  frm210143.Show
               End If
            'Added by Lydia 2015/11/30
            'Modified by Lydia 2019/11/14 更名: 各國年費預估報價
...
End Select
End Sub
Private Sub mnu0101_Click(Index As Integer)
   ToolHide
   If CheckUse("frm010001", strExec) = False Then
      Exit Sub
   End If
   'Added by Lydia 2015/11/12 新增查名單電子化-收文組群
   Set frm010001.Tmpfrm090130 = frm090130
   
   frm010001.intChoose = 0
   frm010001.intReceiveKind = 0
   If Index <= 3 Then 'Added by Lydia 2023/05/17 排除非收文的選項,因為會影響回到前一畫面Where01ToGo
       frm010001.intModifyKind = Index - 1
   End If
   Select Case Index
      Case 1
            '2005/8/10 ADD BY SONIA
            If CheckUse("frm010001", strAdd) = False Then
               Exit Sub
            End If
            '2005/8/10 END
            frm010001.Caption = "接洽紀錄單－新增"
      Case 2
            '2005/8/10 ADD BY SONIA
            If CheckUse("frm010001", strEdit) = False Then
               Exit Sub
            End If
            '2005/8/10 END
            frm010001.Caption = "接洽紀錄單－修改"
      Case 3
            frm010001.Caption = "接洽紀錄單－查詢"
      'Add By Sindy 2022/8/4
      Case 4
            frm210148.m_FlowType = "A" '櫃檯接洽單
            frm210148.Caption = "待建檔區"
            frm210148.Show
      '2022/8/4 END
...
Private Sub mnu0102_Click(Index As Integer)
ToolHide
If CheckUse("frm010001", strExec) = False Then
   Exit Sub
End If
...
Select Case Index
             Case 1
                        ...
                        'frm010002.lblRecieveCode.Caption = "D" + GetTaiwanThisYear
...
On Error Resume Next 'Add By Sindy 2010/3/8
...
Public Function GetForm(pFormName As String) As Form
   ...
   '2020/5/29 END
   'Added by Lydia 2021/04/29
   Case "frm010006_1"
         Set GetForm = frm010006_1
   'end 2021/04/29

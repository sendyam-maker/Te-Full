...
Private Sub cmdok_Click(Index As Integer)
   Select Case Index
      Case 0
        If txt1(0) = "" Then
            MsgBox "獎金年度不可以空白！", vbInformation, "操作錯誤！"
            txt1(0).SetFocus
            Exit Sub
        End If
        If txt1(0) <> "" Then
        ...
        ElseIf txt1(1) = "2" Then
            'modify by sonia 2016/1/7 留職停薪人員改現金發放,故SD05改DECODE(SD02,'S','1',SD05)
            'm_StrSQL = m_StrSQL & " Order by SD05,SD01 "
            m_StrSQL = m_StrSQL & " Order by DECODE(SD02,'S','1',SD05),SD01 "
        End If
        StrMenu1
...
Dim dblTotCnt As Double
   ...
   m_str = "SELECT SD01,ST02,DECODE(SD02,'S','1',SD05) SD05,SD06,T.*,a0802 " & _
           "FROM Staff,SalaryData,( " & _
           "SELECT substr(YB02,1,2)||replace(substr(YB02,3,1),'A','0')||substr(YB02,4) as T1,nvl(YB05,0)+nvl(YB06,0)+nvl(YB26,0)+nvl(YB08,0)-nvl(YB15,0)-nvl(YB16,0)-nvl(YB17,0)-nvl(YB25,0) as T2,YB24 as T3,YB02 as T4 " & _
           "From YearBonus " & _
           "WHERE YB01='" & strYM & "') T,acc080 " & _
           "Where ST01 = t1 AND SD01=T1 AND T3=a0801(+) " & m_StrSQL
...
Sub PrintTitle()
   ...
   Printer.CurrentX = Printer.ScaleWidth / 2 - (Printer.TextWidth("年終獎金入帳明細") / 2)
   Printer.CurrentY = iLine * 300
   Printer.Print "年終獎金入帳明細"
   ...
   Printer.CurrentX = Printer.ScaleWidth / 2 - (Printer.TextWidth("獎金年度：" & txt1(0) & "  年") / 2)
   Printer.CurrentY = iLine * 300
   Printer.Print "獎金年度：" & txt1(0) & "  年"
   Printer.CurrentX = Printer.ScaleWidth - Printer.TextWidth("列印日期：" & ChangeTStringToTDateString(strSrvDate(2))) - 1000
   Printer.CurrentY = iLine * 300

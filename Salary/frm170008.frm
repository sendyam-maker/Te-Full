...
Begin VB.Form frm170008 
   BorderStyle     =   1  '單線固定
   Caption         =   "兼職人員每月工作時數輸入"
   ClientHeight    =   5055
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8160
   ...
   Begin MSComctlLib.ImageList ImageList1 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170008.frx":0000
            Key             =   ""
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170008.frx":031C
            Key             =   ""
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170008.frx":0638
            Key             =   ""
         EndProperty
         BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170008.frx":0814
            Key             =   ""
         EndProperty
         BeginProperty ListImage5 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170008.frx":0B30
            Key             =   ""
         EndProperty
         BeginProperty ListImage6 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170008.frx":0E4C
            Key             =   ""
         EndProperty
         BeginProperty ListImage7 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170008.frx":1168
            Key             =   ""
         EndProperty
         BeginProperty ListImage8 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170008.frx":1484
            Key             =   ""
         EndProperty
         BeginProperty ListImage9 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170008.frx":17A0
            Key             =   ""
         EndProperty
         BeginProperty ListImage10 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170008.frx":1ABC
            Key             =   ""
         EndProperty
         BeginProperty ListImage11 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170008.frx":1DD8
            Key             =   ""
         EndProperty
      EndProperty
   End
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabHeight       =   520
      TabCaption(0)   =   "單筆資料"
      TabPicture(0)   =   "frm170008.frx":20F4
      ...
      Tab(0).Control(5)=   "txtPH(3)"
      Tab(0).Control(5).Enabled=   0   'False
      Tab(0).Control(6)=   "txtPH(1)"
      Tab(0).Control(6).Enabled=   0   'False
      Tab(0).Control(7)=   "txtPH(2)"
      Tab(0).Control(7).Enabled=   0   'False
      Tab(0).ControlCount=   8
      TabCaption(1)   =   "多筆瀏覽"
      TabPicture(1)   =   "frm170008.frx":2110
...
Dim stCon As String
   ...
   stCon = ""
   If txt1(0) <> "" Then
      stCon = stCon & " and ph01>=" & Val(txt1(0)) + 191100
   End If
   If txt1(1) <> "" Then
      stCon = stCon & " and ph01<=" & Val(txt1(1)) + 191100
   End If
   If txt1(2) <> "" Then
      stCon = stCon & " and ph02>='" & txt1(2) & "' "
   End If
   If txt1(3) <> "" Then
      stCon = stCon & " and ph02<='" & txt1(3) & "' "
   End If
   strExc(0) = "SELECT ph01-191100 工作年月,ph02 員工代號,ST02 姓名,ph03 工作時數 FROM PTHour,staff " & _
               " where ph02=st01(+) " & stCon & " order by ph01,ph02"
   intI = 1
...
Private Sub Form_Unload(Cancel As Integer)
   Set frm170008 = Nothing
End Sub

' 初始化欄位陣列
Private Sub InitialField()
   strExc(0) = "select * from PTHour where rownum<1"
   intI = 1
   Set RsTemp = ClsLawReadRstMsg(intI, strExc(0))
   If intI <> 2 Then
      With RsTemp
      TF_PH = .Fields.Count
      ReDim m_FieldList(TF_PH) As FIELDITEM
      For Each oText In txtPH
         idx = oText.Index
         m_FieldList(idx).fiName = "PH" & Format(idx, "00")
...
Dim adoRst As New ADODB.Recordset
   ...
   stKey01 = Val(txtPH(1)) + 191100
   stKey02 = txtPH(2)
   
   Select Case p_iWay
      Case 0
         strExc(0) = "SELECT * FROM PTHour" & _
            " WHERE ph01 = '" & stKey01 & "' and ph02= '" & stKey02 & "'"
      Case -2
         strExc(0) = "SELECT * FROM PTHour order by 1 ASC"
      Case -1
         strExc(0) = "SELECT * FROM PTHour" & _
            " WHERE ph01||ph02 <'" & stKey01 & stKey02 & "' order by 1 DESC"
      Case 1
         strExc(0) = "SELECT * FROM PTHour" & _
            " WHERE ph01||ph02 >'" & stKey01 & stKey02 & "' order by 1 ASC"
      Case 2
         strExc(0) = "SELECT * FROM PTHour order by 1 DESC"
   End Select
   intI = 1
   adoRst.MaxRecords = 1
   ...
   If lCurRow > 0 Then
      If GRD1.TextMatrix(lCurRow, 0) <> "" Then
         If TBar1.Buttons(4).Enabled = True Then
            ...
            If txtPH(1).Locked = False Then
               txtPH(1).Text = GRD1.TextMatrix(lCurRow, 0)
               txtPH(2).Text = GRD1.TextMatrix(lCurRow, 1)
...
Private Sub txtPH_GotFocus(Index As Integer)
   CloseIme
   TextInverse txtPH(Index)
End Sub
...
Private Sub UpdateCtrlData(ByRef p_Rst As ADODB.Recordset)
   ...
   If .RecordCount > 0 Then
      ...
      If ClsPDGetStaffN(txtPH(2), strExc(1), , True) Then
         lblDsp(1) = strExc(1)
      End If
...
Private Sub UpdateToolbarState()
   Select Case m_EditMode
      Case 0 ' 無任何動作
         ...
         End If
         If m_bUpdate And txtPH(1) <> "" Then
            TBar1.Buttons(2).Enabled = True
         Else
            TBar1.Buttons(2).Enabled = False
         End If
         If m_bDelete And txtPH(1) <> "" Then
...
Dim bCancel As Boolean
   ...
   '維護
   Else
      If txtPH(1) = "" And txtPH(1).Locked = False Then
         ShowMsg "請輸入工作年月 !"
         txtPH(1).SetFocus
         txtPH_GotFocus 1
         GoTo EscPoint
      End If
      If txtPH(2) = "" And txtPH(2).Locked = False Then
         ShowMsg "請輸入員工代號 !"
         txtPH(2).SetFocus
         txtPH_GotFocus 2
         GoTo EscPoint
      End If
      If Val(txtPH(3)) = 0 And txtPH(3).Locked = False Then
         ShowMsg "請輸入工作時數 !"
         txtPH(3).SetFocus
         txtPH_GotFocus 3
         GoTo EscPoint
...
On Error GoTo ErrHand
   ...
   stSQL = "INSERT INTO PTHour (" & stCols & ") Values (" & stValues & ")"
...
On Error GoTo ErrHand
   ...
   stSQL = "begin user_data.user_enabled:=1; UPDATE PTHour SET "
   stSet = ""
   For Each oText In txtPH
      idx = oText.Index
   ...
   If bDifference = True Then
      stSet = Mid(stSet, 2)
      stSQL = stSQL & stSet & " where ph01=" & Val(txtPH(1)) + 191100 & " and ph02='" & txtPH(2) & "'; end; "
...
Private Sub txtPH_KeyPress(Index As Integer, KeyAscii As Integer)
   ...
   Select Case Index
      ...
      Case 2  '2011/8/3 add by sonia 員工編號可輸英文字母
      Case Else
...
Private Sub txtPH_Validate(Index As Integer, Cancel As Boolean)
   
   If m_EditMode = 1 Or m_EditMode = 2 Then
      Select Case Index
         Case 1
            If txtPH(Index) <> "" Then
               If Right(txtPH(Index), 2) > 12 Then
                  ShowMsg "工作月份不可超過12 !"
                  Cancel = True
               End If
            End If
         Case 2
            If txtPH(Index) <> "" Then
               If ChkStaffID(txtPH(Index)) = True Then
                  Cancel = True
               End If
               '2011/9/1 modify by sonia A0025於100/9/1離職,100/8無法輸入
               'If Cancel = False And ClsPDGetStaff(txtPH(Index), strExc(1)) = False Then
               '   Cancel = True
               'Else
               '   lblDsp(1) = strExc(1)
               'End If
               If Cancel = False Then
                  lblDsp(1) = GetStaffName(txtPH(Index), False)   '先抓在職員工姓名
                  If lblDsp(1) = "" Then  '非在職再抓離職員工並判斷離職日
                     strExc(0) = "select st02,nvl(st51,0) st51 from staff where st01='" & txtPH(Index) & "'"
                     intI = 1
                     Set RsTemp = ClsLawReadRstMsg(intI, strExc(0))
                     If intI = 1 Then
                        If Val(Left(RsTemp.Fields("st51"), 6) - 191100) < txtPH(1) Then
                           ShowMsg MsgText(9149)
                           Cancel = True
                        ElseIf Val(Right(RsTemp.Fields("st51"), 2)) = 1 And Val(Left(RsTemp.Fields("st51"), 6) - 191100) = txtPH(1) Then
                           ShowMsg MsgText(9149)
                           Cancel = True
                        Else
                           lblDsp(1) = "" & RsTemp.Fields("st02")
                        End If
                     Else
                        ShowMsg MsgText(9149)
                        Cancel = True
               ...
               '2011/9/1 END
               If Cancel = False Then
                  strExc(0) = "select sd02 from salarydata where sd01='" & txtPH(Index) & "'"
                  intI = 1
                  Set RsTemp = ClsLawReadRstMsg(intI, strExc(0))
                  If intI = 1 Then
                     If RsTemp.Fields("sd02") <> "P" And RsTemp.Fields("sd02") <> "F" Then
                        ShowMsg "非兼職人員不可輸入工作時數 !"
                        Cancel = True
                     End If
                  End If
               End If
      ...
      If Cancel = True Then TextInverse txtPH(Index)
...
On Error GoTo ErrHand
   ...
   '刪除
   stSQL = "delete from PTHour where ph01=" & Val(txtPH(1)) + 191100 & " and ph02='" & txtPH(2) & "'"
   Pub_SeekTbLog stSQL
   cnnConnection.Execute stSQL, intI

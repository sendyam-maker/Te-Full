...
Begin VB.Form frm170030 
   ...
   Begin MSComctlLib.ImageList ImageList1 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170030.frx":0000
            Key             =   ""
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170030.frx":031C
            Key             =   ""
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170030.frx":0638
            Key             =   ""
         EndProperty
         BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170030.frx":0814
            Key             =   ""
         EndProperty
         BeginProperty ListImage5 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170030.frx":0B30
            Key             =   ""
         EndProperty
         BeginProperty ListImage6 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170030.frx":0E4C
            Key             =   ""
         EndProperty
         BeginProperty ListImage7 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170030.frx":1168
            Key             =   ""
         EndProperty
         BeginProperty ListImage8 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170030.frx":1484
            Key             =   ""
         EndProperty
         BeginProperty ListImage9 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170030.frx":17A0
            Key             =   ""
         EndProperty
         BeginProperty ListImage10 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170030.frx":1ABC
            Key             =   ""
         EndProperty
         BeginProperty ListImage11 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm170030.frx":1DD8
            Key             =   ""
         EndProperty
      EndProperty
   End
   Begin TabDlg.SSTab SSTab1 
      ...
      TabHeight       =   520
      TabCaption(0)   =   "單筆資料"
      TabPicture(0)   =   "frm170030.frx":20F4
      ...
      Tab(0).Control(5)=   "txtHR(2)"
      Tab(0).Control(5).Enabled=   0   'False
      Tab(0).Control(6)=   "txtHR(3)"
      Tab(0).Control(6).Enabled=   0   'False
      Tab(0).Control(7)=   "txtHR(1)"
      Tab(0).Control(7).Enabled=   0   'False
      Tab(0).Control(8)=   "txtHR(4)"
      Tab(0).Control(8).Enabled=   0   'False
      Tab(0).Control(9)=   "textCUID"
      Tab(0).Control(9).Enabled=   0   'False
      Tab(0).ControlCount=   10
      TabCaption(1)   =   "多筆瀏覽"
      TabPicture(1)   =   "frm170030.frx":2110
...
Dim stCon As String
   ...
   stCon = ""
   If txt1(0) <> "" Then
      stCon = stCon & " HR01>='" & txt1(0) & "' "
   End If
   If txt1(1) <> "" Then
      stCon = stCon & " and HR01<='" & txt1(1) & "' "
   End If
   strExc(0) = "SELECT HR01 類別代碼,HR02||'%' 補助比率,HR03 補助上限,HR04 說明 FROM HiReduce " & _
               " where " & stCon & " order by HR01"
   intI = 1
...
Private Sub Form_Unload(Cancel As Integer)
   Set frm170030 = Nothing
End Sub

' 初始化欄位陣列
Private Sub InitialField()
   strExc(0) = "select * from HiReduce where rownum<1"
   intI = 1
   Set RsTemp = ClsLawReadRstMsg(intI, strExc(0))
   If intI <> 2 Then
      With RsTemp
      TF_HR = .Fields.Count
      ReDim m_FieldList(TF_HR) As FIELDITEM
      For Each oText In txtHR
         idx = oText.Index
         m_FieldList(idx).fiName = "HR" & Format(idx, "00")
...
Dim adoRst As New ADODB.Recordset
   ...
   Select Case p_iWay
      Case 0
         strExc(0) = "SELECT * FROM HiReduce" & _
            " WHERE HR01 = '" & stKey01 & "'"
      Case -2
         strExc(0) = "SELECT * FROM HiReduce order by 1 ASC"
      Case -1
         strExc(0) = "SELECT * FROM HiReduce" & _
            " WHERE HR01 <'" & stKey01 & "' order by 1 DESC"
      Case 1
         strExc(0) = "SELECT * FROM HiReduce" & _
            " WHERE HR01 >'" & stKey01 & "' order by 1 ASC"
      Case 2
         strExc(0) = "SELECT * FROM HiReduce order by 1 DESC"
   End Select
   intI = 1
   adoRst.MaxRecords = 1
   ...
   If lCurRow > 0 Then
      If GRD1.TextMatrix(lCurRow, 0) <> "" Then
         If TBar1.Buttons(4).Enabled = True Then
            ...
            If txtHR(1).Locked = False Then
               txtHR(1).Text = GRD1.TextMatrix(lCurRow, 0)
...
Private Sub txtHR_GotFocus(Index As Integer)
   If Index = 4 Then
      OpenIme
   Else
      CloseIme
   End If
   TextInverse txtHR(Index)
End Sub
...
Private Sub UpdateToolbarState()
   Select Case m_EditMode
      Case 0 ' 無任何動作
         ...
         End If
         If m_bUpdate And txtHR(1) <> "" Then
            TBar1.Buttons(2).Enabled = True
         Else
            TBar1.Buttons(2).Enabled = False
         End If
         If m_bDelete And txtHR(1) <> "" Then
...
Private Function CheckBeUsed() As Boolean
   strSql = "select 1 c0 from staff where st50='" & txtHR(1) & "' and rownum<2" & _
      " union select 1 c0 from hirelationlog where hl05='" & txtHR(1) & " and rownum<2'"
   intI = 1
   Set RsTemp = ClsLawReadRstMsg(intI, strSql)
   If intI = 1 Then
      CheckBeUsed = True
      MsgBox "該補助類別代號已被使用，不可刪除！"
   End If
End Function
...
Dim bCancel As Boolean
   ...
   '維護
   Else
      If txtHR(2) = "" And txtHR(2).Locked = False Then
         ShowMsg "請輸入補助比率 !"
         txtHR(2).SetFocus
         txtHR_GotFocus 2
         GoTo EscPoint
      End If
      If txtHR(4) = "" And txtHR(4).Locked = False Then
         ShowMsg "請輸入說明 !"
         txtHR(4).SetFocus
         txtHR_GotFocus 4
         GoTo EscPoint
...
On Error GoTo ErrHand
   ...
   stSQL = "INSERT INTO HiReduce (" & stCols & ") Values (" & stValues & ")"
...
On Error GoTo ErrHand
   ...
   stSQL = "begin user_data.user_enabled:=1; UPDATE HiReduce SET "
   stSet = ""
   For Each oText In txtHR
      idx = oText.Index
   ...
   If bDifference = True Then
      stSet = Mid(stSet, 2)
      stSQL = stSQL & stSet & " where HR01='" & txtHR(1) & "'; end; "
...
Private Sub txtHR_KeyPress(Index As Integer, KeyAscii As MSForms.ReturnInteger)
...
Private Sub txtHR_Validate(Index As Integer, Cancel As Boolean)
   If m_EditMode = 1 Or m_EditMode = 2 Then
      Select Case Index
         Case 2
            If txtHR(Index) <> "" Then
               If Val(txtHR(Index)) > 100 Then
                  MsgBox "補助比率不可大 100 ！", vbCritical
                  Cancel = True
               End If
            End If
      End Select
      
      If Cancel = True Then TextInverse txtHR(Index)
   End If
End Sub
...
On Error GoTo ErrHand
   ...
   '刪除
   stSQL = "delete from HiReduce where HR01='" & txtHR(1) & "'"
   Pub_SeekTbLog stSQL
   cnnConnection.Execute stSQL, intI
